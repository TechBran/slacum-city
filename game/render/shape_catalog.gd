...
## Every variant shape in the manifest, as `{"archetype/variant": shape}`. For
## the tests and the census; the renderer asks `shape_of`.
func variant_shapes() -> Dictionary:
	return _shape_by_variant.duplicate()
