...
## …and doc 05's per-VARIANT answer, for `cmd_place_water_component`. A water
## shell's ladder is its variant's, not doc 02's `water_facility` column (doc 93
## §BE4), so a tank and a pump reserve different ground.
static func water_reservation(sim: CitySim, variant: String) -> Vector2i:
	return sim.water_lot_for(variant)
