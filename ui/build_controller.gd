...
func _water_headroom_params(sim_id: String, b: Building,
	...
	return {
		...
		# Doc 05 §2.3's per-tile factor at the building's tile, so the sentence can
		# say the number a main would change (Wave 30 merge; it had one reader, a test).
		"tile_factor": float(verdict.get("tile_factor", 0.0)),
