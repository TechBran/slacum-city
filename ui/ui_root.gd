...
## (Merged 2026-09-05: the water lane's one-seam rule and the power lane's
## `asked_about` land together — S18 is asked first WITH the building the row
## was about, and S19 is the fall-through for an id the grid does not have.)
...
func _serve_transformer_fix(fix_target: Dictionary) -> bool:
	...
	if not show_transformer(target, action.get("needs", {})):
		return false
