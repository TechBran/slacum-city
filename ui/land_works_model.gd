...
## without reading. Its two consumers are the ones a smaller invoice owes, and
## neither is in this file: the event-log row (`data/ui.json.event_log.events`,
## so the number is findable an hour later) and the land panel's own `Yard
## materials −$1,160` line under the phase it came off, which
## `LandPanelModel._phases` builds as `stockpile_offset_text` and
## `ui/land_panel.gd` draws. (The merge removed a `phase_text` helper and an
## `EVENT_YARD` constant this comment used to credit with that line; nothing
## but a test ever called them — A91-D-19's shape, caught by the verifier.)
