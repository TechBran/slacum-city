...
- `ramp` is exactly doc 03's stated behaviour — a fresh building fills from 0.35 to 1.00 over **36 game-hours**, linearly. `age_b` is `(sim_time_minutes − built_at_minutes) / 60`.
  - **Inert as shipped (report 98 RR-204, 2026-09-04).** `CitySim._population_inputs` hands every building `POPULATION_INPUT_AGE_HOURS` (48) instead of its age, so `ramp` has read 1.00 for every building in every settle the game has ever run; `tools/measure_population_lag.gd` prints the ramp the formula would give beside the 1.00 the sim uses. Doc 93 §AX2 asks for a ruling — delete the ramp (recommended: the aggregate has no per-arrival channel to show a fill) or wire the real age and pay for it with a doc 12 fill surface. Until ruled, this line describes an intent, not the shipped game.
