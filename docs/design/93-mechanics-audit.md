...
1.00 on `standard` — and doc 92 §31.1 proves it three ways.*

### M1. `E_roads_repair` takes ONE difficulty knob, and it is `M_repair`
...
4. **It is free on the default preset.** Both knobs are 1.00 on `standard`, so
   ...
   published `standard` table stands. Doc 92 §31.1 proves it on the two state
...
every seed, which is gate 29's actual assertion. Doc 92 §31.5 has both columns.
...
### M2. `M_rev` is the TAX multiplier, and §2.9's table row is corrected to say so
...
4. **The re-open condition is written down rather than implied.** When doc 04
   ...
   different answer. It is ranked in doc 92 §31.7, not buried here.
...
### M3. `cmd_install_backup_generator` is doc 05's INTERFACE CALL, not a player verb
...
moves from *open* to *ruled*, and doc 92 §31.6 carries it.

### M4. `Balanced`'s reserve floor is a fraction of the founding purse — and it was never the thing that froze `crisis`
...
purse — the floor never entered the maximum. Measured after §M1, the two terms
are $5,760 and $16,452, and the payroll still wins. What actually unfroze the
agent was **§M1**: `crisis`'s founding net moved −$18.70/gh → +$44.47/gh, so the
gap between purse and reserve stopped being permanent, and `balanced` went from
**0 buildings on all three seeds** to placing. Doc 92 §31.4 has the arm.
...
own trigger was "if crisis still founds negative", and after §M1 it founds
positive. The coverage ratio is the better test and it belongs to whoever rules
on the purse; doc 92 §31.7 ranks it first.
