...
### K1. An event that describes a PLAYER-VISIBLE state change needs a consumer or a written exemption; everything else needs one line, and a RENDERER counts as a consumer
...
are unchanged (report 98 RR-48). **If an event's payload is the wrong shape to
