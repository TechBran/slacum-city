...
### P1. It was the same dollar, and only one of the two was ever real

**Ruled: `reward_base` moves to doc 03; `POLICE_FINE_PER_RESOLVED_INCIDENT` and `CitySim.HELD_FINE_RATE` are retired; the `fines` ledger line is replaced by a live `city_services` line.** Report 98 RR-77 is the binding text.
...
### P2. A subsidy is honest when it is a published constant on a clock

**Ruled: the early-income retune is two state grants, and neither may be a fraction of anything the player controls.** Report 98 RR-78.
...
### P3. A stock measures how much an agent chose not to spend

**Ruled: gate 4's money column moves from `treasury_end` to `net_mean_per_hour`.** Report 98 RR-79.
