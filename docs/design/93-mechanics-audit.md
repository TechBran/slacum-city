...
### O1. The founding purse is a RESERVE, not an operating budget — `crisis`'s 0.729 game-days is the ruled knife-edge
...
### O2. A SECTION rung is sufficient when the body's shape holds — and `city.section_version` records what the CITY section owns
