...
```jsonc
{
  ...
  "archetypes": {
    "<archetype_id>": {
      ...
      "levels": [                          // exactly 5, index 0 == level 1
        {
          "level": 1,                      // int 1..5
          ...
          "upgrade_time_hours": 0.0,       // absent at level 5
...
**Data integrity**
1. `test_catalog_loads` — 12 archetypes, 5 ordered levels each, all loader invariants from §3.1 hold, `variants` present only on `water_facility`.
...
24. `test_fire_load_ladder` — `fire_load` doubles per level for all 60 rows; `house` L1 = 20 and `high_rise` L5 = 1120 (56×); doc 06's `S_req_base(20) == 0.50` and `S_req_base(1120) == **3.06** ±0.01` by the §2.7 formula — **3.06 is canonical, the report's old "≈3.6" gloss is void** *(RR-9)*; `s_req_base_exponent == 0.45`.
...
Two files. `data/buildings.json` is generated from the seed rows by the rules in §2.2 and fully listed in §2.3–2.4 (60 rows); its loader-facing schema is §3.1. Below is `data/building_rules.json` — every global constant this system introduces — plus the seed block that regenerates the table.
