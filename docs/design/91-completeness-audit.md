...
| **A91-D-149** | **Medium** — OPEN, **for the lane that next touches doc 05 §2.5's share split** | **A pump that is not running still holds its slice of the upstream capacity, and the pumps that could use it never see it.** §2.5 splits `upstream_cap = min(Σ source, Σ treatment)` among a zone's pumps *"in proportion to `rated_flow_m3h`"*, and `WaterTopology._resolve_supply_chain` computes that split on TOPOLOGY change while `_solve_zone` decides whether a pump runs on every TICK. A pump that is dark, tripped or in its restart lockout is `is_live()` and therefore keeps `share_m3h`, so its share is subtracted from what the surviving pumps may draw: a zone with a 40 m³/h pump and a 240 m³/h pump on 100 m³/h of upstream gives the small one 14.3 and the big one 85.7, and if the big one trips the zone delivers **14.3** where the chain could carry **40**. `WaterSystem.supply_chain` publishes it as `stranded_m3h` and `tools/measure_water_chain.gd --losses` prints it as the one row in the volume ledger with **no section that authorises it** — every other loss in that ledger cites §2.5 or §2.4. **Measured on the player's save: 0.1 m³/h**, which is why it is filed rather than fixed: the founding city's two pumps are the same size, so the split is even and the loss is a rounding artefact of the condition term. It is a real hole on a city with a big pump and a small one, which is what a player builds. **The fix, and it wants a measurement:** re-split the upstream at tick time over the pumps that are actually running, which is O(pumps in zone) and hash-affecting. Doc 92 §69.4. |
