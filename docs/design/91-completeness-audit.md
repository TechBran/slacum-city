...
| Suite | `tests/run_tests.gd` | ✅ ~~109 files / 1,909 tests / 505,294 asserts / 0 failed~~ **112 files / 1,965 tests / 508,216 asserts / 0 failed / 0 SILENT at `28b9550`.** The last column is new and is a gate: a test method that finished without asserting anything now fails the run (report 98 RR-50), and the runner takes a per-process `user://` so two worktrees can run it at once |
