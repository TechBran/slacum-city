...
| 18 | **Find what `roads` restores differently after a day boundary** | **M** | **A91-D-30** | *added 2026-08-20.* A determinism violation on the founding city at a save point a player reaches in one sitting, and the one item on this list that constitution §5 makes non-negotiable. The repro is exact (report 98 §24 RR-52), the leading suspect is eliminated, and the fix is a `RoadNetwork.SECTION_VERSION` bump plus a ladder rung — so it wants a wave that is allowed to move the bytes, not a hash-neutral one |
| ~~19~~ | ~~**The loading veil, and the catch-up veil with it**~~ **DONE 2026-08-20 (Wave 13, doc 12 §2.20 / D-60)** | **S** | doc 13 §2.9 / §2.9.1's own pseudocode | **Shipped whole on the UI side**: `ui/veil_model.gd` + `ui/loading_veil.gd` on a new `VeilLayer` above every other layer, two preview states, thirteen model tests, `data/ui.json.veil`, and six `ui_veil_*` strings. It is exactly the one `ColorRect`, one label and one progress bar this row asked for, plus the catch-up sentence. **One deviation** (argued in doc 12 §2.20 item 2 and doc 13 §2.9.1): a stepped bar with its unit named under it — `Step 7 of 11` — rather than doc 13's spinner, because a spinner is the one animation A8's `reduce_motion` would suppress and a suppressed loading animation is indistinguishable from a hung app. **One thing it left behind**, filed as **A91-D-31**: the catch-up loop in `game/main.gd` is synchronous, so the catch-up half of the veil is a message rather than an animation until `advance_coarse_sliced` is wired. Original filing: *added 2026-08-20.* Doc 13 has assumed `veil.show()` since it was written and the shell has never had one. The restore is now eleven resumable steps and the catch-up has always been sliceable, so **both levers are built and neither has a surface**; the title door standing in for it (report 98 §24) covers CONTINUE and covers nothing else. One `ColorRect`, one label, one progress bar, and a `SCREENS` entry so the sweep can see it |
