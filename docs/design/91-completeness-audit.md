...
| **D-15** | **High** | **The sim step does not scale to the benchmark city.** `tools/profile_sim.gd --city=res://tests/fixtures/bench_city.json`: **22.00 ms per fine tick** (vs 1.58 ms on the starter, 13.9×) and **259.18 ms per coarse step** (vs 8.20 ms, 31.6×), debug headless. Two consequences. (a) A fine tick costs more than a whole 16.7 ms frame, so at 1,500 buildings every SimTick is a dropped frame no render optimisation can recover; the largest terms are `roads_congestion` 5.17 ms, `water` 4.39, `power` 3.99. (b) Doc 01's 2 s / 12 h catch-up budget is missed at 3.11 s, and doc 13 §2.9's veil table has to be re-read against 259 ms/step — `max_coarse_hours` (report C-21's `ceil(2000/measured_ms)`) resolves to **7**, not 720. The coarse step is 52% `incidents` and 24% `hourly`, so both are single-system problems rather than a diffuse one. |
...
| **D-15** | Medium | **Doc 06's `traffic_accident` generator has no candidate source.** Same shape as D-14: `IncidentWorld.road_intersections()` returns `[]` (`sim/incidents/incident_world.gd:302`), unoverridden. Doc 10's `RoadNetwork` has `{id, tile, congestion_index, signalised, signal_powered, condition_hazard_mult}` for every intersection it knows about. Between them D-14 and D-15 are a third of doc 06's generator surface and the reason two of the five founding departments (water repair, construction) have no ambient work to answer — doc 92 §18.2. |
