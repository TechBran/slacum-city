...
| 2.6 | Disaster Director v1 | ~~SHIPPED~~ **PARTIAL 2026-09-01 (A91-D-59 / A91-D-80)** → **SHIPPED 2026-09-02 (Wave 18)** | **The first grade was wrong and it was wrong for seventeen waves, which is the interesting part of this row.** Every gate, every score and every weight in §2.6 was implemented and unit-tested, and the class was graded on that. What no test asked was whether the Director ever ran a SECOND pair of events: nothing called `on_event_resolved`, so `active_events` never emptied and the two-in-flight gate refused every later schedule — measured at the Wave-17 fork, `tools/probe_director.gd`, balanced / seed 4242 / 60 game-days: **2 events started, 0 ended, the last one on game-day 7.5**. Five of the eight catalog rows could not become anything at all (A91-D-80). The re-grade is the honest record of a class that computed correctly and did not RUN; the second grade is earned by the same instrument reading **23 started, 23 ended, the last on game-day 47.3**, plus `test_balance_gates.gd` gate 33 and `tests/test_director_resolution.gd`. Report 98 §49 RR-135/RR-136/RR-137; `99-PA` PA-04 / PA-25. |
| 2.7 | MVP severe thunderstorm | ~~SHIPPED~~ **PARTIAL 2026-09-02 (A91-D-87 / A91-D-88)** → **SHIPPED 2026-09-02 (Wave 18)** | Same shape one section down, and it never showed because the storm could not be REACHED past the stall above. The generator, the targeter and the choreography caps were live and tested; §2.7.7's six preparation actions had no verb and no door, §2.7.6's Storm Report was never built and its Storm Ready payout never paid, and the one window a prep action could be taken in could not open (A91-D-87). Closed with `CitySim.cmd_storm_prep_action`, S17 (doc 12 §2.24) and the report at the storm's resolution; `tests/test_storm_prep.gd` walks schedule → warning → prep → storm → report → payout. `99-PA` PA-26. |
...
| 2.11 | Overlay mechanism — **and `MM_blob`, the per-building contact shadow** | SHIPPED | `RenderStateModel.set_overlay_channel`. **The section's second half shipped 2026-09-01**: `game/shaders/blob_shadow.gdshader` + `CityView._upload_blob`, ONE MultiMesh city-wide and **+1 draw call at every pose** on Performance (the tier whose `shadows` knob is `false`), zero on the other two; `tests/test_render_polish.gd` section 6. `blob_shadow.enabled_presets` had named this layer since the preset table was written and nothing read it. §2.11's own "one call per NEAR/MEDIUM chunk" is **corrected in the same change** — the shape it prices measured 36 at Z0 on the bench city, because its worked example models 3 + 3 chunks and the census is 12 + 24. Report 98 RR-96, doc 93 §X2, `A91-D-42`. |
| 2.12 | Vehicles | SHIPPED | `VehicleView` + `VehicleMotion`. Every colour on this layer — instance AND vertex — is decoded exactly once as of 2026-09-01 (`A91-D-36`, closed). |
...
| # | Severity | Defect |
|---|---|---|
| **A91-D-36** | ~~**Low**~~ **✅ CLOSED 2026-08-21 (Wave 15, street polish)** | **Every authored livery in the renderer is lifted, because a MultiMesh instance colour is LINEAR and nothing on that path converts it.** A shader uniform hinted `source_color` is converted from sRGB for free, and so is `StandardMaterial3D.albedo_color`; **`MultiMesh.set_instance_color` is neither.** So an authored `#F25242` arrives in the shader as linear `(0.95, 0.32, 0.26)` and displays at roughly sRGB `(250, 165, 150)` — the same hue, washed out by about the gamma curve. `VehicleView` (`vehicle_view.gd:580`, `v.paint`) and `ConstructionVehicleView` (`_write`, `pose.tint`) both do this with palettes authored as hex in `data/render.json` and `data/vehicles.json`; the street-life layer did too until this wave, where it was found by a screenshot of a marker that was supposed to be a strong red and came out the colour of a plaster. **Filed rather than fixed**, for one honest reason: on a SHADED surface the lift is much less visible than on an unshaded billboard, the result reads as a deliberately chalky palette rather than as a bug, and re-saturating a shipped fleet and a shipped plant hire is an art call on two layers a street-life branch does not own. Closing it is `Color.srgb_to_linear()` at each palette's read point plus one screenshot pass to re-judge the values — a couple of hours, not a design. Report 98 §31 RR-81; the pattern that made it visible is in `StreetLifeModel.coat_linear`, which converts once at `configure()` and says why. **CLOSED.** `Color.srgb_to_linear()` now sits at both seams — `VehicleView._paint_for` (once per vehicle) and `ConstructionActivity._plant_paint` plus `_stock_linear` (once per site, once at `_init`) — rather than per instance per frame, because the conversion allocates a `Color` and both call sites write one colour per instance per frame. **And the palettes were re-judged with it, which is the half the filing said was the real work.** Screenshots: `profile_frame --traffic=14 --units=4 --sites=2` at Z1, hour 13 and hour 21, bench city, with no UI layer in the frame (the harness gained `--traffic` / `--units` for exactly this — it had always BUILT `VehicleView` and never fed it, so every picture it had ever taken was of an empty street). Four hexes moved because they had been fitted by eye against the lift and, corrected, landed on the value of the asphalt they were driving over: `CIV_PAINT` `#4A5157`→`#5B646C` and `#2C3237`→`#3B434B`; `LIVERY` `#2F6E52`→`#3E8C69` and `#3D6B92`→`#4C82AE`. The other eight civilian paints, both remaining liveries and all six department colours survive the correction unchanged. `StreetLifeView` never had the defect — its model bakes every coat and marker tint at `configure` — which is the audit answer this row asked for. Report 98 §36 RR-91; doc 93 §V4; `tests/test_vehicle_view.gd` and `tests/test_construction_living.gd` pin the seam. **One half deliberately left open and filed with a named consumer:** the same latent lift is on every VERTEX colour in every procedural mesh here, and several of those constants are used both as vertex colours and as instance tints (`ConstructionRigMesh.GRAVEL` is the dump truck's load and the yard's gravel heap), so converting the constant would move the mesh half at the same time and the mesh half has never been judged against a picture. ~~**awaiting_consumer:** the next render pass, over `construction_rig_mesh.gd`, `vehicle_mesh.gd` and `street_life_mesh.gd`, with a screenshot per mesh family.~~ **THAT HALF IS CLOSED TOO — 2026-09-01, Wave 17, report 98 RR-95 / doc 93 §X1, and this row is now fully closed.** The named consumer arrived and did the named work. `srgb_to_linear` sits at ONE seam per builder — `ConstructionRigMesh._push`, which `StreetLifeMesh` inherits, and `VehicleMesh._push` — decoding once per vertex at BUILD time and **never at the constant**, because the dual-use trio (`SAND`, `GRAVEL`, `REBAR`) is already decoded once by `ConstructionActivity._stock_linear` and converting the constant would have decoded them twice (gravel at linear 0.049 instead of 0.223 — black). **One of the dual-use warnings in the filing turned out to be inert and the correction is recorded rather than quietly dropped:** `GRAVEL` as "the dump truck's load" never reached the screen at all — the load carries `SURF_STOCK` and `construction_rig.gdshader` replaces the vertex colour there with the `stock_color` `source_color` uniform, which is decoded for free. **Re-judged with the pictures, per family, as the filing required** (`profile_frame --traffic=14 --units=4 --sites=2 --street-life=6 --street-gm=600 --anim-step=0.016667`, bench city, `--focus=48,48`, Z0 and Z1, hour 13 and 21, no UI): **two hexes of twenty moved** — `DARK` `#1D2022`→`#424548` and `TYRE` `#161618`→`#333336`, in both mesh files — because decoded from their authored values they land at linear 0.0125 and 0.0069, **below the 0.02-linear shaded carriageway**, so an excavator's track band became a hole in the road and a tyre went blacker than fresh asphalt and took the tread ribs with it. The other eighteen survive, which is the expected shape: the fix is a darkening and a darkening is what it is for. **And the audit answer this row was owed was a GREP, which the closing note above did not run (report 98 RR-95 (continued)):** `_push(` across `game/render/` and `set_instance_color(` across `game/` found two more vertex builders and three more instance seams. Fixed with pictures: `ConstructionSiteView.PropMesh._push` (hoarding posts, crane, scaffold, skips — **and doc 04's transformer pad, which is built with the same class and read raw by `power_pad.gdshader`**), the hoarding panels and posts (`fence_paint` / `post_paint`, instance colours), and doc 12 §2.5's traffic band (`RoadOverlayView._build_band_paint`, on arithmetic — the showcase has no overlay layer to photograph; the band is now the legend chip's colour by construction). **`CobraHeadMesh` was recorded here as "checked and deliberately left — its vertex colour is a grime ramp and its colour is `albedo_color`", and BOTH HALVES OF THAT WERE WRONG; it is the fifth builder and it is fixed, 2026-09-02.** The ramp MULTIPLIES two authored tints (`COWL_TINT`, `LENS_TINT`) and those tints reach the shader through `ARRAY_COLOR`, not through `albedo_color`. The ruling is a SPLIT: the authored tint is decoded, the grime ramp is not — a ramp is a reflectance multiplier and belongs in linear, and decoding the product would put it through a 2.4 power and take the foot of the mast from linear 0.41 to 0.18. Neither constant moved (both are near-white; 0.90 decodes to 0.787 and white is a fixed point), which is the difference between this file and `DARK`/`TYRE`. **And the guard is now a CENSUS rather than a list**: `test_every_procedural_mesh_decodes_its_authored_vertex_colour` greps `game/render/` for `Mesh.ARRAY_COLOR` and requires `srgb_to_linear` in every file that has it — because a file being missed by a hand-kept list is exactly what happened here, twice. **The re-judge, measured rather than described** (`profile_frame --preset=high --hour=13 --focus=52,44 --poses=z0 --sites=3 --street-life=8 --traffic=14 --units=2 --shots`, bench city, 1920x1080, before/after with nothing else changed): the conversion moves **1.36 % of the frame by more than 2/255 and 0.60 % by more than 32/255, peak 87/255**, and over the moved pixels mean luma falls **151.1 -> 99.3**; whole-frame mean luma moves 94.59 -> 94.15, i.e. this is a targeted correction and not a global dimming. **The count of pixels below 20/255 luma is IDENTICAL in the two frames (1,956 either side), and below 12/255 it is zero in both** — the re-judged `DARK` and `TYRE` are what bought that, and it is the number that says no part became a hole. **Still open, with a named consumer:** `PathGhostView.tint_for` (`game/ui/`), the road-drawing ghost's instance tint — one `srgb_to_linear()` at its return and one screenshot mid-drag, on the next UI pass. No site or pad hex moved: the accent hoarding panel became safety orange and the cabinet became olive, which is what those hexes were authored to be. `tests/test_render_polish.gd` section 7 pins all four builders, the fence seam, the stock trio's single decode and the cobra head's tint/ramp SPLIT (the row that used to read 'the cobra head's exemption'). |
...
| **A91-D-43** | ~~**High**~~ **✅ CLOSED 2026-09-02 (Wave 17)** | **The city sold the player a repair on two hundred buildings it does not own, and drew the button on every one of them.** `cmd_repair_building` priced and sold a repair for any building at all, and `ui/build_controller.gd.repair_view` drew the REPAIR row for anything under condition 1.00 — measured at **260 private / 21 civic** buildings in one 21-game-day `balanced` city and **98 / 10** in a `curriculum` one (`tools/measure_repair_burden.gd`, new in this wave). Private repair *trips* were 58–80 % of all repair trips while private repair *dollars* were only 16–28 % of the spend: a player counts trips, which is why the 2026-09-01 playtest called a line worth 5 % of net "way too aggressive". Meanwhile `data/notifications.json` has **no** building-condition binding at all and never had one, so the interruption was never an alert to silence — it was an affordance to stop drawing, and every prior pass had looked for it in the wrong channel. Closed by report 98 RR-99 / doc 93 §Y1: `E_OWNER_MAINTAINED` refuses private stock at any condition and folds into "nothing to buy", so the row is not drawn (0 private on every strategy); the owner holds the building at `condition.band_worn` while the city serves it, so it is never damaged by wear, never destroyed by wear and always still upgradable; and the recovery is an UPGRADE, 20.7 % cheaper in the same wave. **The half of the first draft that did NOT survive is worth as much as the half that did**: retiring `E_building_maint` alongside it put `do_nothing` on `standard` at game-day 176 against gate 29's ruled 69 and left `casual` immortal inside 200, so the line stays and is named for what it is — the city's cost of *serving* a building. Founding gross is bit-identical across the shipped change (1047.184374 both sides) and the net moves −0.18 %, inside every anchor's tolerance. |

| **A91-D-44** | ~~**Medium**~~ **✅ CLOSED 2026-09-02 (Wave 17)** | **`building_repaired` has been emitted since doc 02 §2.6 shipped and consumed by nothing, so a repaired building kept its soot.** `grep -rn "building_repaired" game/ ui/` → nothing; `render_state_model.gd:1194` handles `building_damaged` (writing the damage channel and an `OVERLAY_WARNING` tint) and there was no branch that ever cleared either. A city crew's repair therefore left the world looking exactly as it did before the player paid for it, and the only thing that could clean a building up was an unrelated `building_completed` / `building_upgraded`. It survived because the *sim* state was right in every test and the renderer's is asserted by none — the `game/` tree's 4.8 asserts per 100 lines (PA-38). **Doc 02 §2.6a promotes it from cosmetic to visible**: a private building the city left dark now goes `damaged` and then repairs *itself* when the lights come back, with no player action, so nothing else is coming to clear the soot. Closed by report 98 RR-99's wave: `RenderStateModel` grows the `building_repaired` branch, reading `condition` off the event where the sim sends it and deriving the damage from it otherwise, exactly as the `building_damaged` branch above it does. `game/main.gd`'s render-id translation must name the event for it to arrive — a one-word addition to the match list at `:650`, handed to the lead as a snippet because that file is the lead's. |

| **A91-D-56** | **Critical** | **Standing water is cosmetic: the flood band table's road, asset and incident consequences all have zero readers, and the push copy says traffic is being turned back.** `grep -rn 'add_closure(' sim/` finds construction causes and the water-main path only; `road_closed_flood`/`flood_level_changed` (`flood_field.gd:120/128`) reach `data/notifications.json` and `data/ui.json` and no sim file; `weather.json flood.thresholds[]`'s `road_speed_mult`, `stall_incident_p_10min`, `asset_trip_p_10min`, `asset_forced_offline`, `asset_condition_delta` have no reader though `RoadNetwork.set_edge_speed_mult` exists at `:830`. Probe: a block driven to 351.5 mm closes and slows zero edges after a game-minute. A91-D-26 / RR-53 closed the SURFACE half of the flood; this is the sim half. `99-production-audit.md` PA-01 (lane A). |
| **A91-D-57** | **Critical** | **Placement checks transformer COVERAGE and never capacity.** `cmd_place_building`'s only grid gate is `grid.would_serve(origin)` (`sim/city_sim.gd:2055`); `headroom` appears in `city_sim.gd` at `:2123` (upgrade) and `:2998` (water node) only, though doc 12 §2.7 lists power capacity as a placement requirement. On the fine curriculum path (`tools/playtest.gd --strategies=curriculum --mode=fine --days=21`) seed 1337 records 148 `transformer_failure` of 178 incidents and 44,778 blackout-minutes, starting on day 8–10 when the sheet's police station, pump and infill land on a green ghost. `99-production-audit.md` PA-02 (lane C). |
| **A91-D-58** | **Critical** | **A cold launch never runs the offline catch-up and never reconciles the alarms the last pause armed.** `CatchUpPlanner.plan` is called once, `game/main.gd:1630` inside `_on_app_resumed()`, reached only through `android_lifecycle.resumed`; `_paused_wall` (`android_lifecycle.gd:90`) is in-memory and never seeded from the loaded save, so after process death, a swipe from recents or a reboot the city resumes frozen at the pause with no income, no construction and no away report, while `NotificationCenter.kt replayAfterBoot` re-arms the pause-time alarms. Doc 13 §2.3's cold-start implementation does not exist. Sits beside A91-D-31 (the veil ships; the cold path does not). `99-production-audit.md` PA-03 (lane E). |
| **A91-D-59** | **Critical** | **The Disaster Director stalls permanently after its first two events, in every save.** `disaster_director.gd:518` refuses any schedule while `scheduled.size() + active_events.size() >= 2`; `on_event_resolved` (`:698`) is the only eraser of `active_events` and has no caller outside tests. Measured coarse online: balanced 21 d schedules uid 1 at h113 and uid 2 at h189 and then holds `dir_active=2` every day with `tp_pool` climbing to 60 unspent; starter 30 d and bench 14 d identical. `active_events` is serialized, so existing saves carry the stall; doc 07 §2.7's thunderstorm is unreachable after ~day 5–8. Doc 91 grades doc 07 §2.6 SHIPPED — re-grade PARTIAL. `99-production-audit.md` PA-04 (lane B). |
| **A91-D-60** | **Critical** | **The building panel's `Fix this →` is dead on two of its seven checklist rows.** POWER CAPACITY: `ui/build_controller.gd:1426` sets `fix_target_id = sim.grid.attachment_of(sim_id)` — a transformer id — and `game/main.gd:1791-1793` looks it up in `sim.buildings` and returns. `E_AVENUE`: its params carry no `fix_target_id`, `requirement_formatter.gd:304-307` emits `id == ""`, `main.gd:1780` discards it, and `building_panel.gd:414-416` draws the button on `kind != FIX_NONE` alone. No haptic, no toast; `main.gd` has zero test references. The POWER CAPACITY half is with the Wave-17 power lane — verify at merge that a `T-nn` id moves the camera. `99-production-audit.md` PA-05 (lane D; lane L supplies the params). |
| **A91-D-61** | **High** | **The Graphics preset and the perf governor are inert on every engine-side axis.** No file sets `scaling_3d_scale`, `msaa_3d` or `screen_space_aa`; `environment_controller.gd:66/79` read the `balanced` row at every preset; `main.gd:266-267` hardcodes High's shadow distance for all three; the governor's `render_scale` rung (`perf_governor.gd:26`) is computed and never applied. PIL mean |diff| of the Performance and High Z2 frames = 0.05/255. Doc 11 §2.13's table promises all of it per tier. `99-production-audit.md` PA-06 (lane P; the Performance blob shadow is with the Wave-17 MM_blob lane). |
| **A91-D-62** | **High** | **Doc 04's `CapacityWarning` is authored and never emitted.** `sim/power/*.gd` emit 14 event names; `CapacityWarning`, `PowerOutage`, `MajorOutage`, `TotalBlackout` and `GenerationDeficit` (doc 04 §4) are none of them, so the only cue for a cooking transformer is its failure — 83 % of the fine curriculum city's interrupting banners from day 9. `99-production-audit.md` PA-07 (lane C). |
| **A91-D-63** | **High** | **Auto-reclose never resolves.** `PowerGrid._process_reclose_timers()` (`power_grid.gd:1052-1061`) closes the breaker and sets `pending_reclose_check`; `evaluate_reclose()` (`:1066`), the only reader, the only place `reclose_attempts` increments and the only emitter of `AutoReclosedOK`/`AutoRecloseLockout`, has zero callers. A chronically overloaded component re-closes every 90 game-seconds forever; `incident_system.gd:1715`'s repair spawner, gated on the lockout, never fires. Doc 04 §7 test 8 was never written; §2.5 is graded SHIPPED. `99-production-audit.md` PA-08 (lane C). |
| **A91-D-64** | **High** | **A total water outage moves nothing but revenue.** `city_sim.gd:4619` passes literal `1.0` for `service_uptime_day` and `condition_mean` (two of doc 09 §2.10.3's five happiness inputs); `district_registry.gd:254 update_water_reliability` has no caller; `water_system.gd:807 abandonment_rate_per_hour` has no consumer. Probe (starter, pump tripped, 48 gh): happiness +0.09, stability unchanged, population unchanged. Side effect: the happiness floor is ~74 and L3's `reach_happiness 70` completes at entry. Doc 91 grades doc 05 §2.11 and doc 09 §2.10 SHIPPED — re-grade PARTIAL. `99-production-audit.md` PA-09 (lane H). |
| **A91-D-65** | **High** | **Water node failures are repaired by a phantom `AUTO-WATER-1` truck inside `WaterSystem` in 1–3 gh at $0 and never become incidents.** `water_system.gd:74 auto_dispatch_water = true`, `:654-655 repairs.assign(job_id, "AUTO-WATER-1", …)` on every failure, the job clock at `:571`; `lifetime_repairs: 0` after 56 days; `water_incident_raised` has no consumer in `city_sim.gd`. Doc 05 §2.9/§2.12 hand failures to doc 06; `notifications.json:189` still binds the P2 alert. `99-production-audit.md` PA-10 (lane H). |
| **A91-D-66** | **High** | **Upgrade headroom is judged on the current hour's load.** `power_grid.gd:1409-1429 can_upgrade_power` compares `load_kw + delta` (instantaneous, `:804`) against `0.90 × cap_eff`; `time.json`'s residential channel runs 0.67 at 05:00 and 1.46 at 20:00, so an upgrade approved in the morning runs at 2.2× that load in the evening and the 1.15 margin cannot cover it. Possibly with the Wave-17 power lane's model audit — confirm at merge. `99-production-audit.md` PA-11 (lane C if not). |
| **A91-D-67** | **High** | **The building panel quotes a required-kW figure 15 % below the number the sim's gate demands.** `city_sim.gd:2123` gates on `delta_kw × 1.15`; `ui/build_controller.gd:1424` publishes `"required_kw": delta_kw`; `ui/water_actions.gd:52/183` applies the margin. The only test naming the field hand-builds it. `99-production-audit.md` PA-12 (lane L). |
| **A91-D-68** | **High** | **`data/building_rules.json` is a mirror, not a source.** Every `condition.*` and `construction.*` key and `headroom_safety` is validated for presence (`building_catalog.gd:76, 517-527`), asserted by `tests/test_building_catalog.gd:635/651-652`, and read by nothing: `building.gd:13-19` and `:151-156` hardcode the constants, `building.gd:208` gates on `const MIN_CONDITION_TO_UPGRADE := 0.55`, the 1.15 lives at `city_sim.gd:2123`, `:2998` and `water_actions.gd:52`, `construction_queue.gd:20-21/164-169` hardcode crews and refunds, and `set_site_mult` is called only by a test. **The Wave-17 decay retune must land the readers or its JSON edits change nothing and the suite stays green** — A91-D-19's shape, with the validator approving the dead numbers. `99-production-audit.md` PA-13 (lane C sim half; construction block with the Wave-17 construction lane). |
| **A91-D-69** | **High** | **`POST_NOTIFICATIONS` is never requested.** `PermissionFlow.note_trigger/request_rationale/accept/decline` have no caller (`main.gd:144-146` connects only `permission_result → confirm`); no rationale strings, no settings row; `native_notification_sink.gd:108-121` returns false when `not can_post()`. On a targetSdk-33+ device nothing in the notification stack can post — doc 13 §11.10's "never fired" has a cause. Extends A91-D-27. `99-production-audit.md` PA-14 (lane G). **✅ CLOSED 2026-09-02 (Wave 18, lane G).** Five seams: the trigger (`Main._note_permission_trigger` on `upgrade_started_sim` / `incident_resolved`), the idle frame (`_pump_permission_prompt`), the modal (`ui/permission_sheet.gd`), the two answers (`_on_permission_answered` → `accept()`/`decline()`, with BACK spending neither), and S10's four-state `notification_permission` row. Counters persist in `user://settings.cfg` section `permission`, not in a save, because Android's two dismissals are per install. `tests/test_android_notifications.gd` 30–32 + `tests/test_ui_settings.gd` (4 methods). Report 98 §52 RR-146, doc 93 §AK, doc 13 §2.7 as-built. **Still unverified on hardware** — `dumpsys notification` after a pause is owed. |
| **A91-D-70** | **High** | **`user://settings.cfg` is never written or read.** `ui/settings_model.gd:452-453` defines the path and nothing calls it; settings ride inside each city's save (`ui_root.gd:1770-1796`) and are restored only when `_resumed_slot >= 0` (`main.gd:963-964`), so the title door, New City and every other slot come up at data defaults — including the notification master switch doc 08 §2.13.4 says must survive city deletion. Extends A91-D-27 (the budget's reset is this file's absence). `99-production-audit.md` PA-15 (lane G). **✅ CLOSED 2026-09-02 (Wave 18, lane G).** `game/device_settings.gd` owns the file as a section registry (`settings`, `permission`), merge-read-write, tmp+rename. The precedence rule lives at the end of `SettingsModel.restore_state()` rather than at the four call sites that restore a `ui` block, so "the device copy wins" is true for the resumed save, a mid-session load and New City's empty block alike; `_on_title_new_game` restores `{}` for exactly that reason. `tests/test_ui_settings.gd` (6 methods, including the audit's own acceptance test: change text scale, delete the city, relaunch, the scale survives). Report 98 §52 RR-145, doc 12 §3.2 as-built. |
| **A91-D-71** | **High** | **Crash recovery whose ladder fails the gate silently founds a fresh tutorial city and the next autosave commits it over the player's slot.** `main.gd:165-172` shows no door on the unclean path and leaves `_resumed_slot = -1` on a refused load; `:989-990` starts onboarding; `save_enabled` stays true so the 5-minute timer writes the founding city to slot 0; `save_manager.gd:386-389` evicts quarantine to three files. Doc 08 §2.9's total-failure path (`crash_report.json`, the two-button choice) does not exist. `99-production-audit.md` PA-16 (lane E). |
| **A91-D-72** | **High** | **An unparseable `manifest.json` restarts the ladder at generation 1 and sweeps every retained generation; neither atomic writer checks its bytes landed.** Probe: three saves, truncate the manifest, one more save → `gen_000001.sav` + manifest, `history: []`. `save_manager.gd:149-150` reads `next_generation` from the empty manifest, `_sweep()` deletes what it does not name; `store_string()`'s return is never tested (`:445`, `:456`) and no `get_error()` call exists. `99-production-audit.md` PA-17 (lane F). |
| **A91-D-73** | **High** | **Lifecycle and timer saves fail silently, and a failed pause save leaves the crash flag armed.** Nothing connects `SaveService.failed`; `main.gd:1949-1953` discards the timer outcome; `_on_app_paused` (`:1597-1603`) only marks clean exit on success. The manual S8 path surfaces failure; the two saves a phone relies on do not. `99-production-audit.md` PA-18 (lane E). |
| **A91-D-74** | **High** | **Backgrounding during the catch-up veil drains the rest of the plan synchronously on resume** (`main.gd:1621-1628`: up to 720 coarse steps in one frame at the docs' 6.3–190 ms per step, ×3–5 on the Fold, against Android's 5 s ANR line) **and the pause in between overwrites the away report's "before"** (`:1604-1609`). Timings quoted from doc 13 §2.9, not re-measured. `99-production-audit.md` PA-19 (lane E). |
| **A91-D-75** | **High** | **`NOTIFICATION_OS_MEMORY_WARNING` is emitted into a void.** `android_lifecycle.gd:140-141` emits `memory_warning`; the only other reference is a test asserting it does NOT save. Docs 11 and 13 both specify the cache shed (chunks below MEDIUM to `SIM_ONLY`, LOD0 cache cleared, one governor step) and doc 13 D-12 is its device test. `99-production-audit.md` PA-20 (lane D). |
| **A91-D-76** | **High** | **Two `data/ui.json.event_log` bindings name payload keys the sim never emits.** `city_level_changed` binds `{"level": "level"}` where `progression_system.gd:92` emits `{from, to}`; `block_ready` binds `key "block_id"` where `development_controller.gd:166` emits `{block}`; `event_log_model.gd:316-333` drops the unresolved sentence, so both rows render a bare glyph. The push bindings for the same events are correct. A91-D-26's matrix proves the NAME is wired; nothing proves the SHAPE. `99-production-audit.md` PA-21 (lane K). |
| **A91-D-77** | **High** | **The building panel's Water / Police / Fire coverage tiles are hard-wired to `✕ —` on every building** (`ui/build_controller.gd:1323-1342`, comment "docs 05/06 have no per-building coverage query yet") **while `coverage_police/fire` (`city_incident_world.gd:133/137`) and `pressure_at` (`water_system.gd:742`) publish all three**; `tests/test_build_controller.gd:368-371` enshrines the stale state. Doc 91 row 2.9 records the publisher closing on 2026-08-19; the reader was never updated. `99-production-audit.md` PA-22 (lane L). |
| **A91-D-78** | **High** | **A refused placement explains itself only in a tooltip a touch screen never shows, and has no `Fix this →`.** `ui/build_sheet.gd:870-879` puts the sentence on `tooltip_text`; no long-press-to-tooltip path exists in `game/touch_input.gd`; the `ui_placement_issues` popover string has no caller; the commit-refusal fallback writes into a sheet that `_on_card_pressed` (`:629`) closed. Doc 12 §2.7 calls the row's `Fix this →` "the single most important teaching device in the game"; the building and land panels have it, placement does not. `99-production-audit.md` PA-23 (lane L). |
| **A91-D-79** | **High** | **`E_WATER_HEADROOM` has no copy and no checklist row.** `city_sim.gd:2126-2129` appends it (a real gate, `water_system.gd:824-837`); `build_controller.gd:61-64 UPGRADE_CHECKS` has no water row and `requirement_formatter.gd` no table entry, so `upgrade_view` draws six ✓, `blocked_by == {}`, and `building_panel.gd:385-389` prints "Every requirement met." over a disabled UPGRADE. `99-production-audit.md` PA-24 (lane L). |
| **A91-D-80** | **High** | **Five of the Director's eight catalog events are unreachable.** `target_provider` (`disaster_director.gd:28`) is never assigned anywhere, so `_choose_target` returns `{}` and `DirectorIncidentSink` (`city_sim.gd:4418-4433`) refuses the empty target; `traffic_pileup`, `transformer_explosion`, `crime_surge` and `major_structure_fire` are not `IncidentCatalog` types. Each dead pick still debits `tp_pool`, arms the cooldown and burns the offline-hazard slot. Extends A91-D-19 / RR-48; doc 07 line 959's "8 catalog events" is three. `99-production-audit.md` PA-25 (lane B). |

#### Closed by Wave 18, lane B (2026-09-02)

*Both rows keep their original numbers and their original filing above; this is
the evidence they are closed by, not a re-filing.*

**A91-D-59 — ✅ CLOSED.** Every committed event is stamped at commit with `resolve_after_min` and `expire_at_min`; `CitySim._sweep_director_events` joins that clock to the `_director_links` book on every REPORT tick; save section rung **7 → 8** stamps every row of every existing save so a ghost pair can die. `tools/probe_director.gd` (balanced / 4242 / 60 d): **2 → 23** events started, **0 → 23** ended, `active_end` **2 → 0**, last start game-day **7.5 → 47.3**. Gate 33 + `tests/test_director_resolution.gd` (11). Report 98 §49 RR-135 / RR-136.

**A91-D-80 — ✅ CLOSED.** `data/director.json`'s five incident rows gain an `incident_kind {type, subtype, source}` column naming doc 06's own type; `CitySim.director_targets` is bound to `target_provider` at boot and draws every roster from doc 06's own candidate source through `CityIncidentWorld`; the sink resolves by domain and picks for itself when a request carries no ref. `tests/test_director_targets.gd` (9) forces one of each of the five and asserts a linked incident. Report 98 §49 RR-137.

#### New rows, Wave 18 (2026-09-02) — `A91-D-87`, `A91-D-88` (found under `99-PA` PA-26)

| # | Severity | Defect |
|---|---|---|
| **A91-D-87** | **High** ✅ **CLOSED 2026-09-02 (this wave)** | **The storm preparation window could never open, so §2.7.7's six actions were unreachable from inside `sim/` as well as from the shell.** `DisasterDirector.storm_prep_action` gated on `storm.active` **and** `−90 ≤ now − t0 ≤ −20`; `SevereThunderstorm.begin()` is called from `_start_event` at IMPACT and sets `t0 = now`, so while the storm is active `now − t0 ≥ 0` and the second test is false for every minute of every storm the game can produce. The one existing test that exercised the verb (`test_weather_director.gd::test_29`) constructed the storm BY HAND with a future `t0`, which is why a dead branch stayed green for seventeen waves. This is the shape A91-D-19 and A91-D-80 have: a test that builds the state the code cannot reach. Closed — the window is now measured against the SCHEDULED row (`storm_prep_window()`), which is the row F7's warning went out for, and `tests/test_storm_prep.gd` opens it from a real `debug_force_director_event` and walks it shut. **`test_29` went red on the way**, which is the confirmation and not a casualty: a test standing on an unreachable branch fails the moment the branch becomes reachable. It is re-pointed at a real `director.scheduled` row at T−50 and keeps every assertion it had. |
| **A91-D-88** | **Medium** ✅ **CLOSED 2026-09-02 (this wave)** | **§2.7.6's Storm Ready check was an attendance test, because the resilience half of it had no writer.** `storm_ready_earned()` requires `prep_actions.size() ≥ 3` **and** `outage_customer_minutes < 250 × population/1000`; `grep -rn outage_customer_minutes sim/ game/ ui/` returned `_empty_metrics()` and that one comparison — nothing anywhere incremented it. Three taps would therefore have earned the 15 % reimbursement however dark the city got, which inverts the whole point of the reward (doc 07: *"this makes preparation profitable, not merely less painful"* — it would have made ATTENDANCE profitable). Closed: `CitySim._accrue_storm_outage` counts residents in dark buildings at REPORT, on the same tick and the same `dt` as the Director sweep, so the fine and coarse paths integrate the same quantity. |

**Rows A91-D-56 … A91-D-80, filed 2026-09-01 by the production audit** (`99-production-audit.md`, whose ledger `PA-01 … PA-100` carries every finding and the 25 promoted here — RR-125 says which). Numbers 41 … 55 of the `A91-D` sequence were left to the Wave-17 lanes by the lead and may fill after this fork; a gap in the sequence is deliberate and `tools/check_doc_refs.py` tolerates it (RR-124). Severities follow RR-123's map (P0 Critical, P1 High). None of the 25 is fixed on the branch that filed it — the fixes are Wave 18's lanes (`99-production-audit.md` §3), named per row.

#### New rows, Wave 17 (2026-08-21) — `A91-D-47`, `A91-D-48` (the rush verb, sim half)

| # | Severity | Defect |
|---|---|---|
| **A91-D-47** | **Medium** | **Every `Vector2i` a `ConstructionQueue` job payload carries becomes the TEXT `"(3, 4)"` the first time the city is saved, and no reader has noticed because no reader existed.** The `construction` save section is serialised through `JSON.stringify` like every other section, and `JSON` has no `Vector2i`: it stringifies one to its `print()` form and `JSON.parse_string` hands back a `String`. Doc 10's road jobs put live `Vector2i` in the payload (`{"roads_kind", "tiles": [...], "road_class", "cost"}`), so a save taken with a road job in flight restores a job whose `tiles` are text. **Nothing has ever broken, and that is the whole reason it survived**: `RoadNetwork` keeps its OWN record of the same job (`_serialize_jobs` writes `[x, y]` pairs and `_deserialize_jobs` rebuilds `Vector2i`), and every live consumer — `on_job_completed(job_id)`, `_emit_construction_stages`'s `payload.sim_id`, `on_construction_completed`'s `payload.repair_target`, `DevelopmentController`'s `payload.block_id` — reads either the id or a scalar. The payload's copy of the geometry is dead weight that has been quietly rotting. **Wave 17 is the first branch to want it**: `construction_overview()` needs a representative tile per row, and reading `payload.tiles[0]` would have shipped a roster whose road rows point at `Vector2i(-1, -1)` — but only after a save, which is the worst possible failure shape. Worked around correctly rather than papered over: `RoadNetwork.job_record(job_id)` is the new read-only accessor and the roster goes to doc 10's own record, with `test_the_roster_survives_a_save_and_still_points_at_real_tiles` holding the seam from the other side. **Filed, not fixed, because the fix is a payload-encoding rule for doc 02 §2.13's save section** — either the queue encodes `Vector2i` as `[x, y]` on the way out (a section rung), or `submit()` refuses a non-JSON-round-trippable payload at the door, which is the guard that would have caught this on the day the road jobs landed. The second is cheaper and strictly better: a payload the save cannot carry is a bug whether or not anything currently reads it. |
| **A91-D-48** | **Low** | **A development phase is the only job in the game whose PRICE is not on its own record, so every downstream question about what it cost has to re-derive it — and a re-derivation is not an answer.** `cmd_place_building`, `cmd_upgrade_building`, `cmd_repair_building` and doc 10's three road verbs all stamp `{"cost": n}` into the job payload at submit. Doc 09's six phases do not: `DevelopmentController._submit_phase` records the phase in `_pending_charges` and `CitySim._charge_development_phases` prices it *afterwards*, in the same tick, through `_development_phase_cost(block_id, phase_index)`. The controller owns no money by design (its header says so) and that split is right — but the *result* of the pricing is then thrown away. **Two live consumers already have to re-derive it, and they can disagree with the invoice.** (a) Doc 03 §2.13(f)'s rush quote, new this wave, re-runs `_development_phase_cost` at tap time; if the block's inputs moved since the phase started — a road built next door raises `arterial_connections`, which is a *discount* — the quote is a different number from the charge. That case is defensible and §2.13(f) defends it (a rush is a new purchase, quoted today, exactly as `cmd_start_development`'s preview quotes the next phase today). (b) §2.10's refund table is the case that is **not** defensible in the same way: `_queue_refund_fraction` returns a FRACTION and doc 03 converts it to dollars, so a cancelled phase would be refunded a fraction of a price nobody recorded. **Nothing is wrong today** — the MVP has no `cmd_cancel_job` on the player surface, so arm (b) is unreachable and arm (a) is ruled — which is precisely why it should be written down before a wave builds cancellation and refunds a number the treasury never took. The fix is one field: `_submit_phase` stamps the phase price onto the payload once `_charge_development_phases` has it, or the charge pass moves into the submit. Filed against doc 09 §2.3 / doc 03 §2.8. |

#### New rows, Wave 17 (2026-09-01) — `A91-D-49`, `A91-D-50` (queue-surface fork)

| # | Severity | Defect |
|---|---|---|
| **A91-D-49** | ~~**High**~~ **✅ CLOSED 2026-09-01 (this wave, UI half; the sim half lands with its sibling)** | **The construction queue had no surface at all.** `sim/construction/construction_queue.gd` has carried jobs, crews, `progress()` and `eta_game_minutes()` since doc 02 shipped; `CitySim` submits to it from four places (`cmd_place_building`, `cmd_upgrade_building`, `cmd_repair_building`, `DevelopmentController`) and the roads seam a fifth; doc 12 §4.4 listed `reorder_project` and `cancel_project` as UI commands with **no door**, and §2.2's screen map had no screen for any of it. A player who placed a fire station learned it was finished by noticing the crane had gone; a player whose upgrade had no crew learned nothing, ever. **The pattern is A91-D-28's, one system over** — a sim capability with no state the screen sweep could open — and it survived because every event the queue raises (`construction_completed`, `building_completed`) *was* wired, so the game never looked broken, only mute. Closed by doc 12 §2.22 / D-66: `ui/construction_queue_model.gd` + `ui/construction_queue_sheet.gd` (S16), the S5 inline block, the `⚒ n` chip on rung 3 of the corner rail, four preview states and `tests/test_ui_construction_queue.gd` (22 tests), all built against the seam CONTRACT through a provider `Callable` so the screen was swept before `CitySim.construction_overview()` existed. **What stays open, by name:** the sim half (`construction_overview()`, `cmd_rush_construction()`, the `construction_rushed` event) and the four `game/main.gd` lines — doc 12 §2.22's deferral table. `reorder_project` and `cancel_project` are **not** in this screen: a drag-reorder list and a destructive verb are each their own ruling, and neither has a sim door either. |
| **A91-D-50** | ~~**Medium**~~ **✅ CLOSED 2026-09-01 (this wave)** | **`tools/ui_preview.gd --screen=<one>` audited on its first frame, before any sibling chip had processed, and reported overlaps the whole-deck sweep never saw.** A parent's `_process` runs before its children's and the first frame's `delta` carries the boot, so the 0.12 s settle window was already satisfied on frame one — the frame `_apply()` opened the panel in, before `AlertsCenter`, `EventLog` and `IncidentDrawer` had run the `_process` that yields the edge to an open sibling. Measured on a tree `--screen=all` called clean in all eighteen sweep cells: three states × the six gate boxes read **46** findings, every one of them an `overlapping_targets` between an open panel's rows and a corner affordance that had not yet stood down — worst `--screen=alerts` at 360 × 800 / 880 × 400 / 1280 × 720 with **7** apiece, and `--screen=queue` at 640 × 340 with **4**. **Why it survived:** every gate — doc 12 item 27, §19's matrix, D-58 — runs `--screen=all`, where each state after the first gets its full settle window *after* the previous audit frame; the single-state form is the one a developer reaches for first and no gate ever ran it. Closed by report 98 RR-113(c): `MIN_FRAMES_BEFORE_MEASURE = 2` whole frames after `_apply()`, in addition to the settle window; all eighteen single-state runs read **0** afterwards and the 18-cell sweep is unchanged at zero. |

#### New rows, Wave 17 (2026-09-01) — `A91-D-53` … `A91-D-55` (power fork)

| # | Severity | Defect |
|---|---|---|
| **A91-D-53** | ~~**Medium**~~ **✅ CLOSED 2026-09-01 (this wave)** | **A zero-delta upgrade was refused for want of headroom, so the one grid node doc 04 §2.2 sells hardest could never be bought.** `PowerGrid.can_upgrade_power` opened by reading `_attachments[building_id]` and answering `UNSERVED` when it was empty. A `substation` shell draws nothing at any level (`data/buildings.json` `power_demand_kw 0.0`) and is attached to no transformer — **it IS the grid, it is not a customer of it** — so it took that branch on every call, and `cmd_upgrade_building("SUB-A")` carried `E_POWER_HEADROOM` and nothing else. Measured on the starter city at 6 game-hours before the fix. What that cost: doc 04 §2.2 sells the substation L1→L2 job as "6,000 → 14,000 kW **and the third feeder slot**", and the third slot is the only answer to `E_NO_SLOT`, which is the refusal `cmd_route_feeder` gives on the shipped starter city (doc 09 authors both of SUB-A's two slots full). **The single most important purchase in the late grid was unreachable, and it looked like a power problem.** Fixed by returning `ok` for `delta_kw <= 0.0` before the attachment is read — a building that adds no load cannot overload anything — with `tests/test_power_operations.gd::test_a_a_zero_delta_upgrade_needs_no_headroom_so_a_substation_can_be_upgraded` driving the job to completion and asserting the 14,000 kW and the free third slot. |
| **A91-D-54** | ~~**High**~~ **✅ CLOSED 2026-09-01 (this wave)** | **`Fix this →` on the requirement a player meets most often moved nothing, for three waves.** Doc 12 §2.7 calls that affordance "the single most important teaching device in the game". `RequirementFormatter.CODE_TABLE` gave `POWER_CAPACITY` the fix kind `FIX_BUILDING`; `game/main.gd::_on_fix_requested` answers `FIX_BUILDING` by `camera_state.focus_on()`-ing the building — **the building whose panel the player is reading the row in**. So the row taught the player that the button does nothing. The verb that would have answered it (`cmd_route_feeder`, and after this wave `cmd_upgrade_grid_component`) existed and was reachable only from the BUILD sheet's drag cards, which is not where a refusal is read. Closed with a new fix kind `FIX_POWER`, performed in the panel exactly as `FIX_REPAIR` is: `CitySim.cmd_fix_power_capacity` quotes the **cheapest single purchase that clears the serving path** — a rung on the transformer, heavier copper, a new feeder run, a parallel transformer or the substation shell — and a two-tap confirm strip shows the price and buys it. The other `FIX_BUILDING` rows keep focus-only **deliberately**: `E_STATE` and `E_NO_SLOT` name a different building from the one on screen. |
| **A91-D-55** | ~~**High**~~ **✅ CLOSED 2026-09-01 (this wave)** | **The player was given a city built out of parts the player could not buy, and the wall was always one of them.** Three separate roster gaps, one shape. (a) `data/grid_components.json` offered `placeable_levels [1,2,3]` (400 kW top) and `conductor_classes [1,2]` (3,000 kW top) while `data/starter_city.json` authors class-2 trunks and `tests/fixtures/bench_city.json` authors **L5 transformers (2,500 kW) and class-3 feeders (7,500 kW)** — so `cmd_upgrade_grid_component` could not climb to the rung the city was already standing on. (b) `CitySim._best_feeder_source_for` walked `roster_ids()`, so it could only find a substation the PLAYER had built; the benchmark's six AUTHORED substations are grid components with no doc-02 shell, and `_route_line` therefore answered **`E_NOT_CONNECTED` — "there is no network here" — on a map with six substations and 36 feeders**. (c) `can_upgrade_power` returned a deficit and no id, so the checklist row could not name which hop was full. **Measured with `tools/audit_power.gd` on the benchmark city at the fork:** every one of the **140** `POWER_CAPACITY` blockers bound at a **transformer**, none at a feeder or a substation; the bulk pool sat at **r 0.562 with 105,198 kW spare**; placing a second `plant_gas` moved `system_supply_kw` by exactly its 8,000 kW rating and cleared **0** of them (it made 14 more, by adding its own shell's service draw); and **70 of the 140 had no purchase that cleared them at all**. That is the user's report, exactly: *"how power stations add to the overall grid capacity … doesn't seem to be working well at all."* The model was right; the shop was empty. Closed by opening the roster to the authored range (doc 04 §6.1 — a data edit, at doc 03 §2.13(b)'s existing prices, C-07 untouched), by asking the GRID rather than the roster for substations, and by having the gate name its binder (`{at, kind, r_after, path}`). |

| **A91-D-81** | ~~**High**~~ **◐ HALF-CLOSED 2026-09-01 (Wave 17): the declaration ships; the device A/B is owed** | **The game has capped its own frame rate since the governor landed and has never once told the display about it.** `PerfGovernor.target_fps()` answers 60 / 45 / 30, `game/main.gd:1935` writes it into `Engine.max_fps`, and three searches say the rest: `grep -rn "max_fps" game/ ui/` finds that line and `game/showcase.gd:138` and nothing else; `grep -n "max_fps" project.godot` finds nothing, so a fresh launch runs **uncapped** until the governor first steps — and the 2026-08-21 Fold capture held `knob = 0` through its whole settled window at **99.5–112.4 fps**, i.e. the reference device has never run this game at a declared rate at all; and `git show HEAD:…/SlacumNative.kt | grep -c 'setFrameRate\\|preferredRefreshRate\\|preferredDisplayModeId'` returns **0**. `vsync_mode=1` is a swapchain property and Swappy is a *consumer* of the refresh rate, so on the reference device — a Galaxy Z Fold 6 whose inner panel is **1856 × 2160 LTPO, 1–120 Hz adaptive** — the platform's only input to its mode policy was the app's observed cadence, and every workload change was a candidate mode change with nothing asking for the switch to be seamless. **Why nothing caught it:** there is no wrong value anywhere. Every number in the frame-cap path is correct, tested and applied; what is missing is a *statement*, and a suite cannot fail an assertion nobody wrote. It is A91-D-19's shape once more — a plausible, complete-looking path with a whole channel outside it — except that the channel here is the panel rather than the ledger. **The player found it from the outside**, in the only terms a player has, after days of play: *"tearing only happens in the sub menus."* That sentence is the finding: world play is clean, and a sheet over a still world is a workload step with no camera motion to hide a re-time (doc 93 §AE). **Half-closed here, and the halves are on purpose.** The declaration ships — `game/render/refresh_pin.gd` + `SlacumNative.set_frame_rate()`, the smallest-supported-multiple mode rule, a `--refresh=` lever and doc 12 D-75's settings row — and it is table-tested, idempotent, re-cast on surface recreation and gated against a stale AAR. **What is NOT closed is whether it removes the bands**: that is a scanout event, no workstation instrument can see it, and report 98 §46 has the one-binary A/B the lead runs on the phone. Report 98 RR-126 / RR-127 / RR-128; doc 11 §2.13 and doc 13 §2.8 carry the note; **doc 11 §2.13's open presentation-corruption band stays open until that session reports.** |
| **A91-D-82** | ~~**Medium**~~ **✅ CLOSED 2026-09-01 (Wave 17)** | **`tools/check_doc_refs.py` could not see a two-letter doc-93 section, and failed in the exact way it exists to catch.** Its id pattern was `[A-Z][0-9]*` in both halves. Doc 93's ladder ran past `Z` this wave, and a single letter is wrong twice over: a `## AE.` header matched nothing and registered as **no target at all**, so the section could be deleted or renumbered with every pointer to it still resolving; and a `93 §AE` reference matched its first letter and resolved, **silently, to section `A`**. §37's own words for that case are *"a pointer that resolves to the wrong section is a lie with a footnote"* — the validator was producing one. **Why nothing caught it:** the tool reports totals and problems, and the failure mode is a *silent success*; a run with a mis-resolving reference prints `all resolving; no id assigned twice` and exits 0, which is what every branch of this wave would have seen. It was found by being the first branch to be assigned a two-letter section. Fixed by widening both patterns to `[A-Z]+[0-9]*` — safe because doc 93 carries no all-caps multi-word header for the greedy form to swallow (`grep -nE "^#{2,3} [A-Z]+[0-9]*[.— ]"` finds no row the single-letter form did not already match). Measured across the tree: doc-93 targets **75 → 79**, and RR-94(d)'s validator can see this wave's own sections for the first time. |

#### New rows, Wave 17 (2026-09-01) — `A91-D-83`, `A91-D-84` (the first complete device matrix)

*Both were found by a session that went looking for three A/B deltas, both are
`game/`'s, and both are worth more than the three deltas. Report 98 §47 carries
the measurements; these are the rows.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-83** | **High** | **Doc 13 §2.8's frame cap is written only when the governor changes a knob, so a device that never trips a rung never gets capped — and the Fold 6 has never been capped.** `game/main.gd:1935` sets `Engine.max_fps = perf_governor.target_fps()` **inside** the `if perf_governor.update(delta):` body, which is the branch that runs when a rung moves. `project.godot:45` ships `window/vsync/vsync_mode=1` and no `max_fps`, so until a rung moves the frame rate is the panel's. **Measured on device, 2026-09-01: 106.5, 107.8 and 109.2 fps** at the three daylight captures (`tools/device_results/log_z*_h13.txt`), `knob = 0` and `preset = balanced` in every line — a preset whose `target_fps` is **60**. Doc 13 §2.8 calls capping at 60 on a 120 Hz panel *"the single biggest battery lever available (roughly halves GPU work)"*, and the toggle that is supposed to opt **in** to high refresh is, on this build, the state a phone boots into. **Three consequences, and the third is why this is High rather than Medium.** (1) Battery: the phone is doing up to 1.8× the GPU work the active preset asks for, on the row of doc 13 §7 (`D-07`) that has never been run. (2) Heat: the session's only thermal step (`0 → 1`, `AP 47.4 °C`) happened under that load. (3) **It corrupts the instrument.** Free-running against a 120 Hz panel puts a vsync cliff at 8.33 ms in the middle of this project's measured range — the 2026-09-01 day/night delta of **+2.60 ms of `gpu_est` (6.03 → 8.63)** reads as **−38.5 fps** for exactly that reason, and every `fps` column doc 11 §2.13 has ever taken from this phone is a step function of a millisecond number rather than a measurement of it. Closes with: hoisting the assignment out of the `if`, or writing it once beside the preset seed in `_ready`; the ruling is doc 13 §2.8's and it already exists. **The re-measurement it unblocks is the real prize:** with a cap in force, `fps` becomes a flat 60 and `gpu_est` becomes the only moving column, which is the shape every table in doc 11 §2.13 wants. |
| **A91-D-84** | **High** | **`--zoom` is delivered, parsed, and does not move the camera — so the three-pose matrix has now produced one pose, twice, for two different reasons.** The 2026-08-20 session got `near = 4` at every zoom because no argument reached GDScript at all; D-20 fixed that and 2026-08-21 confirmed the transport on hardware. On 2026-09-01 the argument demonstrably arrives — the plugin logs `SlacumNative: launch args: [--resume, --zoom=1.0, --perf]`, `game/main.gd:218` parses it into `camera_state.set_zoom_t()`, and its neighbours in the same list plainly work (`--advance-hours` moves `gpu_est` 6.0 → 8.6, `--flood-detail` moves `near` 4 → 7) — **and the render columns do not move at all**. `md5sum` over `dc … lights` for all 25 samples is **`cc761e3e782311b8db9b578f109b2417` for ELEVEN of the fourteen captures at once** — `z0.0_h13`, `z0.5_h13`, `z1.0_h13`, `z0.0_h21`, `z1.0_h21`, `rd0_h13`, `rd2_h13`, `rd2_h21`, `flood0`, `pads0`, `pads1` — and only three differ: `z0.5_h21` in exactly one sample of 25, `rd0_h21` at the mid-hold pose change (RR-129) and `flood2` from its first sample. Reproduce with `cd tools/device_results` and `for f in log_z0.0_h13 log_z0.5_h13 log_z1.0_h13 log_z0.0_h21 log_z1.0_h21 log_rd0_h13 log_rd2_h13 log_rd2_h21 log_flood0 log_pads0 log_pads1; do printf '%s ' "$f"; grep -ao 'dc=.* lights=[0-9]*' "$f.txt" \| md5sum; done`. Identical `prim` to the unit across three zoom values is only possible if the camera is in the same place, and `near` — the tier census against `near_max_m = 150 m` from `camera_rig.camera.global_position`, and `tools/run_matrix.sh:152`'s own stated discriminator for whether the poses separated — reads **4 in every capture**. **What is ruled out already, so the next session does not re-walk it:** the clamp in `CameraState.set_zoom_t` (`ui/camera_state.gd:245`) cannot collapse three values onto one, because `_d_max_eff` is left at the world-sized default — `game/main.gd` never calls `set_owned_land_aabb`, the only caller outside `ui/` and `tests/` is `tools/profile_frame.gd:367` — so `max_zoom_t()` is 1.0; nothing after `main.gd:218` writes `camera_state.zoom_t`; and `CameraState.advance` has no zoom spring. Closes with: **a headless probe first, not another device window** — `tools/profile_frame.gd` already sets the AABB and drives the camera, so a run at `--zoom=0.0` and `--zoom=1.0` printing `tier_census()` either reproduces the collapse on the workstation (in which case the device is innocent) or does not (in which case the next device window carries a per-capture screenshot, which is the only artefact that would have caught this in either of the last two sessions). **Blocks doc 91 §20.4 device item 2's pose half and doc 11 §2.13's `Z0`/`Z2` rows; does not block its day/night half**, which is measured across the same single pose and is valid for it. |

#### New rows, Wave 17 (2026-09-01) — `A91-D-41`, `A91-D-42` (render/art fork)

*One is **found and left OPEN with the frame that shows it** (`A91-D-41`), and
one is **found and closed in the same wave** (`A91-D-42`) and recorded rather
than quietly fixed because it is an arithmetic fault that produced a number
somebody would otherwise have believed. The instrument fault the same fork
found — a free-running layer contaminating a picture A/B — is recorded in
report 98 RR-95 and doc 11 §2.17c rather than as a row: measured, it is a
0.28 % floor, and a row for a floor is a row nobody re-opens.*

*The same fork's second half — the preset table (report 98 RR-98) — found
**eleven authored keys with no consumer** and left **two items open**: the
MEDIUM/FAR boundary's remaining LEVEL gap, which is an LOD2 mesh question, and
`performance` missing its own `chunk_budget` at Z2 by 36 against 24. Neither
takes an `A91-D-nn`: this fork's block is `D-41`/`D-42` and both are spent, and
§14.5's fifth collision is the standing reason a wave does not number past its
block. **They are filed as dated OPEN boxes in doc 11 §2.6b and §2.13b, each
with a re-open trigger written as a command** (§X4's rule), and are listed here
so a reader walking this section is not left thinking the fork closed
everything it found.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-41** | **Medium — OPEN, filed 2026-09-01 (Wave 17, render fork)** | **The data centre is the one black block on a lit street at 21:00 — the tech family has no night emissive at NEAR or MEDIUM, and the FAR tier lights it anyway.** `data_center` is `windowless` at every level and both LODs (`game/meshes/generated/manifest.json`, `window_cols = 0`, all twelve cells), and doc 11 §2.6's emissive is the window-cell hash, so `building.gdshader` emits nothing on any of its walls after dark; `building_far.gdshader` knows nothing about windowless and lights the tech family's bands from `window_colors[3]` like every other family. **Seen, not inferred**: `profile_frame --preset=balanced --hour=21 --poses=z0,z1 --focus=13,49` (DC-0001, authored `(-19,17)`), bench city — at Z1 the data centre sits dark between two lit apartment blocks and a lit office tower. It is the standing directive's exact shape ("textures on everything" includes *light* on everything the player is meant to find at night) and a ladder inconsistency at the one hour the ladder is hardest to read. **Not fixed here because the fix is a design call before it is a few hours' work**: what a data centre looks like at night — an LED status band, a lit plant deck, a glowing entrance — is doc 11 §2.6's to say; the sketch is an emissive mask on the `techpanel` page's alpha (`tools/gen_textures.py`, one regenerated page) fed to the near shader's emissive on windowless walls, plus the FAR tier taking the same cue so the two agree. Report 98 §38 (the visual-bar sweep). **Re-open/close condition:** a night Z1 frame at `--focus=13,49` in which the data centre carries any emissive, and the FAR arm at Z2 agreeing with it. **Note added 2026-09-01 (same fork, report 98 RR-98):** the DAY half of this row moved, and it moved AGAINST this row. Doc 11 §2.6b gives the far tier the page each family actually wears; `techpanel` is the darkest page in the atlas at linear luma **0.054**, and the neutral the far tier used to paint was **0.092** — so tech is the one family of five the far tier was too BRIGHT for, and the data centre is now correctly dark at every tier by day instead of artificially lifted beyond 420 m. By night it is a dark building that also emits nothing, which is exactly the frame this row was filed about. The design call is unchanged and still owed; what changed is that the night frame is no longer partly masked by the far tier disagreeing with the near one. |
| **A91-D-42** | ~~**Medium**~~ **✅ CLOSED 2026-09-01 (this wave)** | **Doc 11 §2.11's draw-call price for the contact-shadow layer was computed from a MODELLED chunk count and is wrong by 6×.** §2.11 prices `MM_blob` at *"one extra draw call per NEAR/MEDIUM chunk"* and §2.13's worked example spends **6** of them, from `3 NEAR·(13+1 blob) + 3 MEDIUM·(10+1)`. The benchmark city at Z0 on Performance reports **12 NEAR + 24 MEDIUM**, so a per-chunk layer costs **36 / 36 / 16 draw calls at Z0 / Z1 / Z2** — against a 180-call budget the bench city already exceeds at 229. **It was found by building the per-chunk shape the doc asked for and measuring it**, which is the only way it could have been found: the arithmetic is internally consistent and `near_chunk_max = 3` appears in the preset table, so nothing about the sentence looks wrong from inside it. **This is A91-D-19's shape on a performance claim rather than on a data path**, and RR-76's sibling: RR-76 governs a derived total re-derived from a source that has since moved; this is a derived total computed from a factor that was **never measured at all**. Closed by report 98 RR-96 / doc 93 §X2: the layer ships as ONE MultiMesh city-wide — **+1 draw call at every pose, measured `--blob=0\|1`** — and §2.11, §2.13's per-chunk line and the worked example are all corrected in the same change, with the census printed beside the model. **The generalised rule is X2: a per-chunk cost is priced against the CENSUS, never against the model.** |
...
| Shaders | ~~15~~ ~~18~~ **19** (Wave 17: `blob_shadow.gdshader`, doc 11 §2.11's contact decal, owned by `city_view.gd` — report 98 RR-96) | every one loads as a `Shader` **and** is named by its owning `game/render/*.gd` | `test_17` |
...
8 façade pages, 4 roof, 2 ground, 3 prop, 1 vehicle atlas, ~~15~~ **19** shaders
(16 at A91-D-26, 18 at doc 11 §2.17, 19 at §2.11's decal — Wave 17), 5 vehicle
...
**Wave 17 adds three, and every one of them has a door in the commit that
created it** (doc 93 §AD, doc 12 D-70/D-71):

| Verb | UI door | Playtest | Goal kind | Verdict |
|---|---|---|---|---|
| `cmd_upgrade_grid_component` | `ui/power_actions.gd` → `ui/building_panel.gd` (the POWER section's `UPGRADE` button, one per hop) | probed | — | ✅ |
| `cmd_demolish_grid_component` | `ui/power_actions.gd::demolish_quote/demolish` → `ui/building_panel.gd` | probed | — | ✅ |
| `cmd_fix_power_capacity` | `ui/power_actions.gd::fix_quote/fix` → the §2.7 `Fix this →` row's confirm strip | probed | — | ✅ |

**The count is 26 of 26.** `cmd_route_feeder`'s ⚠️ above closed at Wave 11;
Wave 17's three arrive doored, which is the standard A91-D-28 set and §17.3
restated: a verb and its door ship in one commit or the verb is not shipped.
...
### 17.3a In-flight at the Wave-17 fork — `cmd_rush_construction`

The UI half of the construction queue (doc 12 §2.22) ships **a door for a verb
that does not exist at this fork**: `ui/construction_queue_sheet.gd` and
`ui/building_panel.gd` → `UIRoot.bind_construction(provider, rush, treasury)`
→ *four lines in `game/main.gd` the lead lands at the merge*, calling
`CitySim.cmd_rush_construction(job_id)`, which a sibling branch
(construction-rush-sim) is building against the same contract. Until it
lands the door refuses with `E_NO_COMMAND`, which the shell reads as silence
(§2.21's rule), and the chip never appears because `construction_overview()`
is unbound. **This table is not re-graded here**: a row for a verb with no
`func cmd_*` under `sim/` would be a row for a promise. When both halves are
merged, `cmd_rush_construction` joins §17.1 as ✅ with the door named above,
and the count becomes **26 of 26**; if only this half lands, it is a doorless
verb in reverse — a door with no verb — and A91-D-49's "what stays open"
...
> **At the Wave-17 queue-surface fork it is 16 of 17 screens, 61 states, 18
> sweeps** — S16 (doc 12 §2.22) added `queue`, `queue_uncrewed`,
> `queue_empty` and `building_upgrading`, and the matrix is now six boxes ×
> **three** accessibility settings (100 %, 130 % + large, 150 % + large), all
> eighteen cells exit 0 with zero findings (doc 92 §46.3). The queue's chip is
> bound on **every** state, not only its own four, because it changes the
> corner rail's solve behind each of them. *(Annotated 2026-09-01, A91-D-49.)*
...
| 2 | **The three-pose day/night matrix** — ◐ **HALF-CLOSED 2026-09-01** | doc 11 §2.13's device column; doc 13 §2.8's heat half | **The DAY/NIGHT half is CLOSED.** `tools/run_matrix.sh` ran end to end on 2026-09-01 (`MATRIX EXIT: 0`), six captures, hour 13 and hour 21, `--preset=balanced` pinned, on the player's live city on the Wave-15 build: **`gpu_est` 6.03 → 8.63 ms, +2.60 ms (× 1.43), at identical `dc` (100 / 320), identical `prim` (47,783) and identical `vram` (134 MB)** — the whole night cost is per-pixel and post, and the table is re-taken in doc 11 §2.13, "The 2026-09-01 session". **The three-POSE half is NOT closed and the reason is new**: `--zoom` now arrives (the plugin logs it, `game/main.gd:218` parses it) and **still does not move the camera** — eleven of the fourteen captures share one `md5` over their entire render column set and `near` reads 4 in every one. `A91-D-83` and `A91-D-84`; report 98 §47. **The next step is a WORKSTATION probe, not a device window:** `~/.local/bin/godot --headless --path . -s res://tools/profile_frame.gd -- --zoom=0.0` against `--zoom=1.0`, printing `tier_census()` — it either reproduces the collapse off-device or hands the next session a reason to carry a screenshot per pose |
| 3 | **The zebra / flood / pad-shadow A/Bs** — ◐ **TWO CLOSED, ONE OPEN, 2026-09-01** | doc 11 §2.1.2's fragment ladder, §2.9b, §2.10b — all three published as workstation numbers | **zebra: CLOSED (report 98 RR-129).** `road_detail` 2 vs 0 is below the noise floor at both hours and the *cheaper* arm is the slower one in both matched pairs (day 8.5 vs 8.9 ms gpu; night, matched `t = 4–12 s` window, 8.5 vs 8.9) — RR-42's *"if rung 2 and rung 0 are within each other's spread on device, the ladder is pointless"* is answered YES for this part. **flood: CLOSED (RR-130).** `flood_detail` 2 vs 0 is 8.8 ms against 8.8 ms; rung 2 stays, and `tools/device_results/flood_night.png` is the first device look at standing water. **pad-shadows: STILL OPEN (RR-131)** — `pads0` is stamped `CONTAMINATED` (`fg_before=0 fg_after=1`), the pair straddles a `thermal 0 → 1` step, and the `pads1` arm sets the value the build already boots with yet costs 2.5 ms, so neither arm is a pad-shadow measurement. RR-33 stands unrevisited: its own gate (*a daylight pose within 5 % of the draw-call budget*) is unmet by **69 % headroom**, wider than the 41–56 % that failed it in 2026-08-20. **The exact re-run, and it must be the FIRST section of a window rather than captures 13 and 14 of 14:** `adb shell run-as com.slacumcity.game rm -f files/perf_capture.flag; bash tools/run_matrix.sh build_check` then, for `r` in 1..4, `bash tools/cap_pose.sh "pads1_$r" "--resume --zoom=0.0 --preset=balanced --pad-shadows=1 --perf" 40 13` and the same with `pads0_$r` / `--pad-shadows=0`, then `python3 tools/perf_rows.py tools/device_results/log_pads[01]_*.txt`. Three things that adds: the preset is **pinned** (2026-09-01's pads captures did not pin it), the arms are **round-paired** so a thermal step lands on both, and **four rounds** give each arm a spread |
...
| 5 | **The presentation-corruption ("tearing") A/B** — ◐ **the player has answered the WHERE; the WHY is in another lane** | doc 11 §2.13's open band; the reason `PERF` is not always-on | **The user's verdict, 2026-09-01: "tearing only in sub-menus."** That is a different defect from the one this row was written about and it moves the row twice over. First, it **de-links tearing from `PERF`**: the telemetry hypothesis (*the per-frame `viewport_set_measure_render_time` GPU timestamp query provokes the swapchain bands*) predicts bands wherever the 3D frame is expensive, and a 2D sub-menu is where the 3D frame is **cheapest** — so the arm/disarm A/B this row was built around is no longer the discriminating experiment and should not be spent a window on. Second, it points at presentation rather than at rendering: a panel that is free-running toward 120 Hz over a scene whose cost collapses when a menu opens is a **refresh-rate** problem, and `A91-D-83` is the measured half of it — `game/main.gd:1935` writes `Engine.max_fps` only inside `if perf_governor.update(delta):`, so the cap doc 13 §2.8 specifies is never applied on a device that never trips a rung, and the Fold measured **106.5–109.2 fps** at three daylight captures under a preset asking for 60. **This row now waits on the refresh-pin lane (IN FLIGHT) rather than on a device window**, and closes when a build that actually pins the cap is put in front of the same sub-menus. *(Housekeeping, unchanged and now confirmed by a completed run: `tools/run_matrix.sh` arms and disarms `files/perf_capture.flag` itself — `trap disarm_flag EXIT INT TERM` — and the 2026-09-01 session exited 0 through that trap, so the flag a **hand** session left armed in 2026-08-21 is cleared. The standing "first command of the next window" advice is discharged.)* |
...
> ### Wave-17 delta, 2026-09-01 — the completion statement, re-stated after the first complete device matrix
>
> **`tools/run_matrix.sh` ran end to end for the first time, on an unlocked and
> connected Galaxy Z Fold 6, against the player's live city on the Wave-15 build
> (`versionName` 0.4.0), `--preset=balanced` pinned, `MATRIX EXIT: 0`.** Fourteen
> captures, fifteen `PERFIO` rows, one screenshot, one `meminfo`, one thermal
> dump — `tools/device_results/run_matrix_2026-09-01.log` and the files beside
> it. Report 98 §47 rules it; doc 11 §2.13 re-takes the Fold table from it; doc
> 13 §2.9.1 re-derives the ANR budget lines from its `PERFIO` medians.
>
> **The row arithmetic does not move.** This lane wrote no `### 2.N` row and
> touched no `sim/`, `game/`, `ui/` or `data/` file, so **172 of 190 / 90 %** and
> **172 / 188 = 91.5 %** stand exactly as the merge block above prints them.
> What moves is the last clause of the sentence — *"half of everything that
> remains is one document's, and that document needs a phone, not a feature"* —
> and it moves in **both** directions, which is why this box exists instead of a
> tick.
>
> **The honest form of that clause, at this session:**
>
> > **A phone was found, the matrix ran, and it closed three of the eight
> > device-gated items and opened two defects worth more than the three it
> > closed.** Closed: device item 2's **day/night half** (+2.60 ms of `gpu_est`,
> > 6.03 → 8.63, at identical `dc`, `prim` and `vram`); device item 3's **zebra**
> > and **flood** A/Bs (report 98 RR-129, RR-130). Still open: item 3's
> > **pad-shadow** A/B (RR-131, re-run command filed in the row above, RR-33
> > unrevisited and standing), item 2's **three-pose half** (`A91-D-84` — the
> > argument arrives and the camera does not move, for the second session
> > running), item 5 (**the player has answered *where* the tearing is — "only in
> > sub-menus" — and the *why* has moved to the refresh-pin lane**, in flight),
> > and **item 1, the notification flow, which this session did not attempt at
> > all**. Items 4, 6, 7 and 8 are untouched. **The blocker is no longer "a phone"
> > and no longer "a build that receives arguments" — it is that two of the
> > arguments do not do what their names say**, and that is a workstation
> > question now (`A91-D-84`'s probe runs headless), not a device one.
>
> **Two device-gated rows got measurably closer without being closed, and the
> reason is worth carrying forward.** Item 6 (an on-device ANR run) no longer
> needs a 12-hour absence for its *arithmetic* half — doc 13 §2.9.1 now derives
> it from fifteen measured load rows and a 33× margin — but the ANR **event** has
> still never been watched for. And item 7 (battery) acquired its first real
> hypothesis: `A91-D-83` says the frame cap doc 13 §2.8 specifies is written only
> inside `if perf_governor.update(delta):`, so the phone free-runs at **106.5–109.2
> fps** under a preset asking for 60. **A battery session taken before that is
> fixed would measure the bug, not the app.**
>
> **What this session proves about the instrument, which is the durable part.**
> Three of the fourteen captures are unusable for what they were taken for, and
> in all three cases the *capture* is fine and the *harness* could not tell:
> `pads0` (foreground lost, correctly stamped), `rd0_h21` (the camera moved
> mid-hold — nothing stamps that) and every zoom row (`--zoom` inert — nothing
> stamps that either). **Two one-line instrument fixes would have caught all
> three**: an `hour=` column and a `near`-delta assertion in `cap_pose.sh`'s
> post-hold check. Both are named in doc 11 §2.13 and neither is this lane's to
> write.
...
> **Wave-17 delta (2026-09-01, the refresh pin) — a BRANCH box, quote the merge
> instead.** Measured on this branch, Godot 4.7.2, headless, one process:
> **125 files / 2,257 tests / 536,293 asserts / 0 failed / 0 silent**, exit 0
> (`~/.local/bin/godot --headless --script tests/run_tests.gd > suite.log 2>&1`).
> One new test file, `tests/test_refresh_pin.gd` (18 tests, 86 asserts).
> **All four determinism baselines are byte-identical to the fork**, which is the
> evidence that this branch shipped a platform declaration and touched no
> simulation: founding `a27da24aaf6e9663…` / `7745cb25e55ff65c…`, bench
> `7c99720f5ff14553…` / `d8e8889681b23297…`, re-measured rather than quoted
> (`tools/profile_sim.gd --hash-only`, and again with
> `--city=res://tests/fixtures/bench_city.json`). The UI audit is clean:
> `tools/ui_preview.gd --screen=settings --audit --strict` across all six `BOXES`
> × three text scales — **18 sweeps, 0 findings** — plus two whole-deck passes
> (`--screen=all`, 640 × 340 @ 150 % with large targets, and 360 × 800 @ 100 %),
> both exit 0.
>
> **What this branch did NOT close.** A91-D-81 is half-closed on purpose. Doc 11
> §2.13's presentation-corruption band is a scanout artifact and no instrument on
> this workstation can see it, so the branch ships the declaration, the levers,
> the row and the A/B protocol (report 98 §46) and **claims no fix**. The row
> above says so, the two documents say so, and the next completion statement
> should carry the band as open until the phone reports.
...
#### New rows, Wave 18 (2026-09-02) — `A91-D-89`, `A91-D-90` (the fix router and the shell extraction)

From the production audit's Lane D brief (doc 99 §3.2). Both are the same shape
as `A91-D-19`: a computed value with no consequence, in the one file the suite
cannot load. Report 98 §50 carries the rulings; doc 93 §AI carries the
extraction plan the rest of `game/main.gd` is filed against.

| Id | Defect | Sev | Status | Evidence |
|---|---|---|---|---|
| **A91-D-89** | **`Fix this →` was dead on two of the building panel's seven checklist rows, and the dispatcher lived where nothing could see it.** `POWER_CAPACITY` fills `fix_target_id` from `sim.grid.attachment_of(sim_id)`, which returns a **transformer** id (`T-02`), and `ui/requirement_formatter.gd` mapped the code to a BUILDING kind; `game/main.gd::_on_fix_requested` then did `sim.buildings.get(id)` and returned on the miss. `E_AVENUE`'s params carried no `fix_target_id` at all, so the formatter emitted `id == ""` and the same function's first line discarded it — while `ui/building_panel.gd:414-416` drew the button on `kind != FIX_NONE` alone. Button depressed, no camera move, no toast, no haptic, no test: `grep -rn "res://game/main" tests/` → 0. The same function anchored every building on its NW corner **tile** (`b.origin * tile_m`), so a jump to a 4×4 building landed sixteen metres off its own lot. | **P0** | ✅ **CLOSED 2026-09-02** (router half; the params half is filed on lane L) | `ui/fix_router.gd` (a `fix_target` → one of three ACTIONs, or a refusal carrying one of five named reasons) and `ui/world_locator.gd` (sim id → metres, per kind, with `locate_any` falling through the namespaces so a transformer id in the building slot resolves). `_on_fix_requested` is three lines; `_alert_world_pos` is one. `tests/test_fix_router.gd` (19) + `tests/test_world_locator.gd` (18), 468 asserts, including the audit's own criterion: every row of the real `_check_params` checklist for every building in the starter city resolves or refuses by name. Report 98 §50 RR-139 / RR-140, doc 93 §AI. |
| **A91-D-90** | **`NOTIFICATION_OS_MEMORY_WARNING` was emitted into a void for the life of the project.** `game/android_lifecycle.gd:195` emits `memory_warning`; `grep -rn "memory_warning\|NOTIFICATION_OS_MEMORY_WARNING" --include=*.gd .` found the declaration, its comment, the emit, and one assertion in `tests/test_save_service.gd` that it does NOT save. No listener. `main.gd::_notification()` handled `WM_CLOSE_REQUEST` only. Doc 11 specifies the response — tear chunks below MEDIUM, clear the LOD0 mesh cache, drop `far_cull_m` one governor step, *"~40 % of VRAM in one frame"* — and doc 13 D-12 is its device test. On a Fold running a large city beside other apps, the cost of no response is a silent task kill. | **P1** | ✅ **CLOSED 2026-09-02** | `PerfGovernor.on_memory_warning()` takes the `far_cull_m` rung by name and out of ladder order (the ladder is ordered by cost per millisecond; a memory warning must not spend its step on `render_scale`), and returns `false` at the floor rather than claiming a step. `CityView.shed_caches()` frees the FAR/MEDIUM nodes of chunks that are neither and evicts unreferenced merged atlas meshes, and states in its docstring which allocation it deliberately does not free and why. `tests/test_memory_warning.gd` (12, 41 asserts). Report 98 §50 RR-141. |
...
#### New rows, Wave 18 (2026-09-02) — `A91-D-97`, `A91-D-98` (the aim-height ramp)

| # | Severity | Defect |
|---|---|---|
| **A91-D-97** | **High** ✅ **CLOSED 2026-09-02 (this wave)** | **A camera axis built to look UP could not, at any angle it had, and every test it shipped with agreed that it could.** Wave 17's manual pitch band answered the directive *"we need to be able to look up at the buildings"* with a floor of 12° and thirty tests across `test_camera_state.gd`, `test_ui_tilt.gd` and `test_gestures.gd`, every one of which asserts the angle, the bias, the reach or the gesture. Not one of them asserted the FRAME. In §2.16's rig the camera looks at the focus and the focus is on `y = 0`, so the horizon is always `pitch` above the view axis and lands at `(1 − tan p/tan(fov/2))/2` down the frame — **20.8 % at the 12° floor, leaving the ground 79 % of the picture at the shallowest angle the band can reach**, with the tower tops cropped off the TOP edge. The band was necessary and not sufficient, and the floor could not be lowered to fix it: `18·sin 12° = 3.74 m` is the last angle that clears doc 11 §2.6's 3.5 m ground floor, so the feature had reached the end of the only axis it had. Closed by report 98 §54 RR-151: the LOOK-AT point rises with the lean (`aim_up_ground_frac = 1/3`, pavement in the bottom third), a pure aim lift that leaves `camera_position()` and therefore every clearance, tier and cull measurement untouched. **The shape to look for elsewhere: a feature whose acceptance criterion is a picture, tested only on the number the picture is derived from.** `tests/test_camera_aim.gd::test_the_full_lean_leaves_the_ground_in_the_authored_bottom_share` is the missing test — it asserts where the horizon is DRAWN, by projecting it. |
| **A91-D-98** | **Low** ✅ **CLOSED 2026-09-02 (this wave)** | **`CameraState.m_per_dp()` was documented as pitch-invariant, and the invariance was a property of the rig rather than of the arithmetic.** Doc 12 §2.23.4 and the function's own doc comment both state that §2.21's 48 dp tap radius is unaffected by tilt "because screen-right is parallel to the ground at every pitch". Half true: screen-right is indeed parallel to the ground, but the figure also carries a DEPTH — `2·D·tan(h½)/w` is the frame's width in metres *at the focus*, and that is only `D` deep while the camera is aimed at the focus. It always was, for four waves, which is why the claim read as arithmetic. The aim-height ramp tips the focus `Δ` below the view axis and its depth to `D·cos Δ`, and the un-corrected figure would have over-stated the tap radius, the drag ghost and the drawer offset by **5.4 % at the Z0 floor and 6.0 % at Z2** — small, silent, and in the un-conservative direction. Found and closed in the same commit that would have broken it (report 98 §54 RR-152): `m_per_dp()` measures at `focus_axis_distance()`, and `tests/test_camera_aim.gd::test_m_per_dp_is_the_scale_the_projection_actually_draws` proves it against `project_to_screen` itself — 27 poses — rather than against its own formula. **Same shape as A91-D-51:** a documented invariant that was true of the camera the game happened to have. |


## WAVE 18 — the two defects the building-panel branch closed (2026-09-02)

*(Doc 91 §14.5 id rule: an `A91-D` id names a defect in a thing that exists, never
a feature a doc authors — RR-125. Both rows below are surfaces that shipped, were
covered by tests, and did not do the thing they say they do.)*

| # | Severity | Defect |
|---|---|---|
| **A91-D-91** | **High** ✅ **CLOSED 2026-09-02 (this wave)** | **A refusal that a touch screen cannot read, on the first thing a new player does.** `ui/build_sheet.gd:870-879` put the requirement's TITLE on the placement bar and its BODY — the half carrying the remedy — into `tooltip_text`. Godot raises no tooltip for a touch event and this build has no long-press-to-tooltip path (`grep -rn -i tooltip game/touch_input.gd game/main.gd ui/ui_root.gd` → nothing), so every `ui_requirement_*_remedy` string was unreachable from placement on the device. The bar also carried no `Fix this →` while both panels have had one for waves, and the commit-refusal fallback wrote into `Sheet/Body/Notice`, a child of the sheet that `_on_card_pressed` closes at `:629` before the write. **The shape to look for elsewhere: copy whose only carrier is a pointer affordance.** Closed by report 98 §51 RR-143 — the bar stacks the body on line two with a readable floor, a 48 dp `FIX THIS →` sits on the bar whenever the row's `fix_target.kind != FIX_NONE`, and the dead notice path is gone. |
| **A91-D-92** | **High** ✅ **CLOSED 2026-09-02 (this wave)** | **Three of the building panel's four service tiles were hard-wired to `✕ —` while the sim published all three, and the test enshrined it.** `BuildController._coverage()` set every non-power slot to `STATE_OFFLINE` / `NO_DATA` with the comment *"docs 05 (water) and 06 (police/fire) have no per-building coverage query yet"* — a statement that stopped being true in Wave 5. `CityIncidentWorld.coverage_police/coverage_fire` (doc 02 §2.9, C-51) and `WaterSystem.pressure_at` (doc 05) had all been live since; doc 91 row 2.9 records the publisher closing on 2026-08-19 and the reader was never updated. `tests/test_build_controller.gd:368-371` asserted the three stay `NO_DATA`, so the hole was **defended**. The pump station's own water tile read "Water ✕ —", and the L4 curriculum ("stations and coverage") had no surface that could ever turn green. **The shape to look for elsewhere: a placeholder whose test asserts the placeholder.** Closed this wave — all four tiles read live values banded on their own doc's thresholds, with a one-line reason on tap naming the station or the pressure zone, and the test now asserts the founding city's real numbers (`POL-1` covers its own lot at 99 %, `H-001` reads 60 % water in `WTR-1-PMP`). |


---

## WAVE 18 — two rows the settings lane found on its way through (2026-09-02)

Filed in §14.5's `A91-D-nn` sequence by lane G, both of them shapes rather than
one-off bugs. Report 98 §52 carries the rulings; doc 93 §AK carries the
mechanics argument. `A91-D-69` and `A91-D-70` are closed in the same wave, in
place, above.

| Id | Defect | Sev | Status | Evidence |
|---|---|---|---|---|
| **A91-D-93** | **A HUD control positioned by a constant is positioned for one device box.** `ui/follow_chip.gd`'s first draft placed doc 12 §2.6 step 6's chip at a hard-coded 316 dp from the bottom of the safe area — read off the 360 × 800 layout, correct there, and **inside the top bar at 880 × 400**, where the whole safe area is 400 dp tall. `tools/ui_preview.gd --screen=all --size=880x400 --audit --strict` printed **six** `overlapping_targets` findings, every one of them against a top-bar stat chip. The generalisable half: doc 12's left column is SOLVED at runtime (`UIWidgets.solve_rail_stack`, whose own docstring records the 93-against-73 pitch bug), so any new member of that column that computes its own offset is re-introducing the bug that solver exists to prevent. | **Medium** | ✅ **CLOSED 2026-09-02 (this wave)** | `UIRoot.solve_follow_chip()` measures `LeftRail`'s laid-out `offset_top` and places the chip one gap above it, falling back to `UIWidgets.rail_slot(3, …)` where there is no tree; it also yields the column while the overlay strip is over it, on §2.23's ruling. `audit --strict` exits 0 at 360 × 800, 412 × 915, 640 × 340, 880 × 400 and at 360 × 800 with `--text-scale=1.3 --large-targets`. |
| **A91-D-94** | **A shipped default whose consumer is `data/ui.json` itself.** Auditing PA-58's three gameplay keys turned up two more `defaults` entries with no row, no reader and no plan: `colorblind` (read only by `SettingsModel.theme_opts()`, from `defaults`, so A6's palette can be *authored* but never *chosen* — doc 12 §3.2 lists it as device-scoped and it is in neither the rows nor `device_scoped_keys`) and `battery_saver` (no reader at all; doc 11 §2.13's governor and its `auto_quality` row already own the load ladder). Neither is a bug in itself; the shape is: **`defaults` has become a place where a key can live without ever being asked for**, and a reader auditing the file cannot tell an owed feature from a withdrawn one. | **Low** | 🟡 **OPEN — recorded, not fixed** | `grep -rn "colorblind" ui/ game/ data/ \| grep -v strings` → `settings_model.gd:theme_opts()` and `ui_root.gd`'s palette calls, both reading `defaults`, never a row. `battery_saver`: `grep -rn battery_saver sim/ game/ ui/` → zero. Wave 18 records the two deferrals in doc 12 §2.13 with the reason for each; closing this row means either shipping the rows or moving the keys out of `defaults` into a `deferred` block that says so in data as well as in prose. |

#### New rows, Wave 18 (2026-09-02) — `A91-D-95`, `A91-D-96` (Lane S, the money surfaces)

*Both found while wiring 99-PA PA-83, both on `data/ui.json.event_log`, and neither fixed here: that table is Lane K's this wave (99-PA §3.2), and §3.0 rule 1 says a lane that finds itself in another lane's file files a note instead. The append PA-83 makes is an append; these two are edits.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-95** | **High** — OPEN, **for Lane K** | **The event log's land-development row draws a BLANK title, and its `Jump to it` is dead.** `sim/world/development_controller.gd:166` emits `{"type": &"block_ready", "block": block_id}`. `data/ui.json.event_log`'s rule for it reads `"key": "block_id"` with `"args": {"block": "block_id"}` — a field the payload does not have — so `EventLogModel._args` supplies nothing for `{block}`, `n_land_developed_title` ("Block {block} is ready") renders as **the empty string**, and `_resolve_focus` gets a null key value so `has_focus` is `false`. Measured by feeding the real payload through the real model: `block_ready title= body=You can build on it now. has_focus=false`, against the row PA-83 adds in the same run, `development_charged title=Grading started on Block B_3_1 … has_focus=true`. **`data/notifications.json` has the SAME binding written correctly** (`"key": "block"`, `"args": {"block": "block"}`) with a `_comment` recording that exact fix — *"`block_ready` carries `block`, not `block_id`"* — so the alerts centre and the push are right and the log row was missed when the notification table was corrected. The player's own land finishing developing, the payoff of a six-phase purchase, is a blank line in the ledger. **Fix:** one edit, `"key": "block"` and `"args": {"block": "block"}`, matching the notification table. |
| **A91-D-96** | **Medium** — OPEN, **for Lane K's payload-SHAPE gate** | **The strings gate proves a placeholder has a DECLARATION, never a value; nothing anywhere joins a router's `args` to the payload the sim emits.** `tests/test_ui_strings.gd::test_every_notification_placeholder_has_a_supplier` walks `data/ui.json.event_log.events[].args` and asserts every `{name}` in `n_<id>_title/_body` is *named* there. A91-D-95 passes that gate: `args` names `block`, and the value it maps onto is a field that has never existed. `test_event_matrix.gd` assertion 4 has the same shape one level up — it proves every router row names an event `sim/` really *emits*, never that the row's `key` and `args` name fields that event really *carries*. This is the blind spot 99-PA §3.2 already assigns Lane K a gate for ("the payload-SHAPE gate beside `test_event_matrix.gd`"); A91-D-95 is a live first catch for it, and a census run when the gate lands will say whether it is the only one. **Fix:** in the new gate, for each router row, resolve a representative payload for its `type` (the emit-site scan `test_event_matrix.gd` already does, extended to the dictionary literal) and assert every `args` source and the `key` name a field of it. |

#### New rows, Wave 18 (2026-09-02) — `A91-D-99`, `A91-D-100` (the restore lane)

*`A91-D-99` is `A91-D-19`'s shape for the **sixth** time, and it is the first
instance a PLAYER found — not an audit, not a grep sweep, but a person looking at
their own dead city and asking for the button. That is the part of this row worth
keeping: five waves of audit walked past a transition with no caller, and the
sixth reading came from outside the project entirely.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-99** | ~~**High**~~ **✅ CLOSED 2026-09-02 (Wave 18)** | **A destroyed building was permanently dead: doc 02 §2.12's `destroyed → planned` transition has a model, a doc and a price, and has never had a caller.** `sim/buildings/building.gd::order_rebuild(now_minutes)` is typed, documented against §2.12's transition table, and returns a priced quote (`pending_level = level_at_destruction` inside a 72-game-hour window at `cost_fraction` 0.60, `1` at 1.00 after). **The grep is the finding:** `grep -rn "cmd_rebuild\|\.rebuild(" sim/ ui/ game/` returns **not one caller** — every hit is `RoadSurfaceView.rebuild` or `WaterTopology.rebuild`, two unrelated methods that share a name — so there is **no `cmd_rebuild`, no CitySim verb and no UI door**, and the only caller in the project was `tests/test_building.gd` asserting the price of a transition nothing could reach. And the verb a player naturally reaches for is closed against exactly this state: `cmd_repair_building` refuses anything that is not `active` or `damaged` with `E_STATE`, and on private stock the Wave-17 `E_OWNER_MAINTAINED` blocker fires first and `BuildController.repair_view` folds it into "nothing to buy", so a destroyed house's panel offered **no action at all**. **Why nothing caught it:** the model is correct, the doc is correct, the test is green, and the state machine is complete — what is missing is a *call*, and no suite fails an assertion nobody wrote. It is the same fault as A91-D-32 (doc 10's two injected siblings), A91-D-37/38 (the incident bounty), A91-D-40 (the street price table), A91-D-68 (`data/building_rules.json` as a mirror) and A91-D-80 (five unreachable Director events). **The player found it from the outside, 2026-09-02:** *"I have many buildings that are destroyed that I can't actually fix even if I upgrade power."* Measured (doc 92 §54): a `disaster_neglect` arc's three ruins at 45 game-days are the starter city's **power plant, substation and water plant** — the entire utility spine — and that city's net falls **$53,546 → $17,058 per game-day** as they die, with no verb in the project that could put any of them back. Closed by report 98 §56 RR-155/156/157: `CitySim.cmd_restore_building` + `cmd_restore_all_destroyed`, doc 03 §2.5's `RESTORE_COST_FRACTION` behind a `CostCurves` accessor, doc 12 §2.9 D-86's one-tap `RESTORE · $X`, and the `restore_started_sim` translator arm. `tests/test_restore_building.gd` (6 tests). Four determinism baselines unmoved. |

#### New rows, Wave 19 (2026-09-03) — `A91-D-101`, `A91-D-102` (the offline-credit lane)

*Neither row is a missing caller. `A91-D-101` is the opposite shape and it is the
first of its kind in this table: **a rule that was implemented exactly as
specified, and was specified against the wrong quantity.** The specification, the
derivation, the data file, the fifteen tests and the report were all internally
consistent, and the whole assembly paid a sleeping player for six hours of a
nine-hour night. Nothing in the tree could have caught it, because every test
asserted the rule and no test asserted the **player**.*

| id | severity | finding |
|---|---|---|
| **A91-D-101** | ~~**High**~~ **✅ CLOSED 2026-09-03 (Wave 19)** | **A performance budget was charged to the player's wallet, and the whole tree agreed with it.** Doc 08 §2.12's `max_coarse_hours = clamp(floor(ceil(2000 / measured_coarse_ms) / 24) × 24, 72, 720)` was NORMATIVE from report C-21 and was implemented in Wave 17 (RR-133) as a second clamp inside `CatchUpPlanner.plan`: `cap_ms = mini(OFFLINE_CAP_REAL_MS, cap_hours × 60_000)`. Measured at the founding city's 5.488 ms coarse hour it shipped at **360 game-hours, which is six real hours**, so the seventh, eighth and ninth hour of a night credited nothing. **The mechanical tell is a `mini` between two quantities that do not measure the same thing** — one is a promise to a person, the other is a property of a workstation (doc 93 §AO1) — and the game-hour conversion that made the comparison type-check is what hid it. **Why nothing caught it:** `tests/test_catchup_clamp.gd` had fifteen tests and every one of them asserted the *rule*; `test_the_same_absence_on_the_SHIPPED_clamp` even asserted `capped == true` on a 6 h 12 m absence and called it correct. No suite anywhere asked what a night was worth in dollars. **The player found it from the outside, 2026-09-03:** *"I went to bed hoping I'd wake up to a bunch of money. The money stops after a certain amount of hours of the game being closed."* Measured (doc 92 §55.3) on a settled L3 city, seed 1337: an eight-hour night paid **$281,319** where the uncapped absence pays **$354,830** — **$73,511, 26.1% of the night** — and a twelve-hour absence paid 57.5% of what those hours were worth. Closed by report 98 §58 RR-160/161/163: `plan()` takes three arguments and reads no data file, the budget becomes a measured wall-clock gate on the veil (`veil_ms_at_cap` 6,432 ≤ `veil_budget_ms` 9,000), and `tests/test_catchup_veil_budget.gd` (17 tests) asserts the credit at every length from 6 to 12 real hours **and** that no shipped code can name the budget. Four determinism baselines unmoved. Doc 93 §AO. |
| **A91-D-102** | **Low** | **A 1,500-building city needs 116,882 ms of catch-up veil for a full 12-real-hour absence, and the clamp that was supposed to bound it never did.** Measured (doc 92 §55.6): 162.6 ms per coarse hour on `tests/fixtures/bench_city.json` against 7.65 ms on a settled founding city — 7,013 frames at 60 fps, and doc 13 §2.13's 3–5× Fold multiplier on top. **It is not a regression and A91-D-101 did not cause it:** at the deleted `max_coarse_hours = 360` the same city already spent **64,552 ms**, because the clamp was derived from the founding city's measurement and had no idea the benchmark city existed — a number that produces 3 s on one city and 65 s on another was never bounding a veil. The coarse step is 57% `incidents` + `roads_congestion` (`profile_sim --coarse-hours=240`), which is where a fix has to start. **It is filed and not fixed here, on purpose**: it belongs to a coarse-step performance lane, its fix changes what a coarse hour costs and not what it does, and **the one repair that is forbidden is the one that was there before** — paying the player less. Tracked as doc 92 §55.7 `AC-19-1`. Doc 08 §2.12 records the measurement beside the budget so the gap stays visible rather than being forgotten. |
| **A91-D-109** | **Medium** | **Doc 03 §2.11's offline yield taper is authored, documented, exempt-listed — and read by nobody.** `EconomySystem.settle_hour` takes `yield_mult` from its inputs and defaults it to `1.0` (`economy_system.gd:352`); `CitySim.build_settlement_inputs` never sets the key; `TAPER_EXEMPT` has no reader outside `data/economy.json` (`grep -rn TAPER_EXEMPT --include=*.gd sim/ ui/ game/` → nothing). **Offline income has therefore never been tapered**, and doc 03's "leaving the game closed is not a strategy" has been carried by the OTHER offline rules (no street draws, no player verbs, a capped window) and not by the knob named for it. Found 2026-09-03 by the Wave-19 verify pass, while refuting a lane's claim that the taper was 'exonerated by measurement' — the measurement (offline earns 97-101 % of idle-online) was right and the mechanism was invented. **The seventh instance of this project's oldest shape** (A91-D-19's: authored, plausible, consumed by nothing). Filed rather than fixed: wiring it is a one-line producer and a balance question, and the balance question is the whole of it — a player who is now paid ~98 % for sleeping is a player whose expectations this wave just set. Whoever wires it re-measures the night. |
| **A91-D-118** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 22)** | **A curriculum rung above the population ladder's top would have been earned and never paid, silently.** `ProgressionSystem.grant_level` clamps its argument to `city_level_pop().size() - 1` — a guard written in Wave 9 for exactly the right reason (*"a curriculum with more rows than `data/progression.json` has rungs cannot invent a level that nothing in the game unlocks"*) — and nothing anywhere asserts that the two tables are the same height. So `data/goals.json` gaining a seventh row would have produced a city that earns curriculum level 7, shows the goals sheet complete, and never fires `city_level_changed` for rung 7: **doc 03 §2.5a's grant for that rung would never be paid, `Treasury.note_era` would never open, and the build sheet would never refresh.** Every symptom is an absence, which is why this is filed as a defect rather than as a wave's checklist item: nothing crashes, nothing logs, and the sheet says you finished. **Why nothing caught it:** the clamp's own docstring reads as a safety property and it IS one — it is the *silence* that is the defect, not the clamp. **Closed by report 98 §64 RR-189:** `data/progression.json` gains rung 7 (40,500) and `ProgressionSystem.CITY_LEVEL_POP_FALLBACK` with it, and `tests/test_city_services.gd::test_a_level_up_grant_is_paid_once_per_rung` now asserts `GoalSystem.top_level() == grant_table.size() - 1 == city_level_pop().size() - 1` — three tables, one height, checked. Doc 09 §2.11; doc 92 §61. |
| **A91-D-119** | ~~**Medium**~~ **✅ CLOSED 2026-09-04 (Wave 22)** | **Three preview states and one gate named a curriculum rung by NUMBER, and two of them had already gone stale.** `tools/ui_preview.gd`'s `goals_done` state called `_goals_at(5, 0)` to photograph "the curriculum finished" — true when it was written against a five-level curriculum, and false since Wave 10 authored a sixth: for two waves that state has been photographing an ACTIVE level 6 while its own comment claimed *"the one state with no objective rows at all"*. `goals_late`'s comment claimed level 5 was *"the widest the sheet ever gets"*, which Wave 22's twelve-row capstone makes false. `tests/test_city_services.gd` asserted the dispatcher's premium *"reaches 6.00x at the top of doc 09's ladder"*, a number that is `1.50 + 0.90 × 5` and therefore a statement about the ladder's HEIGHT written as a constant. **The shape to look for elsewhere: a test or a fixture that hard-codes a rung count is a claim about data that lives in another file.** **Closed by RR-190:** all three now ask `GoalSystem.top_level()`, and `goals_capstone` is the state that photographs the real top rung. Doc 12 §2.19 D-96. |
| **A91-D-120** | **Low** | **`data/economy.json`'s `_level_up_grant_derivation` disagreed with its own table for a whole wave, and nothing could have noticed.** The shipped array was `[0, 2500, 7000, 9000, 22500, 29000, 65000]` while the prose beside it derived `37,000` at rung 5 and `83,000` at rung 6 — the pre-Wave-17 figures, left behind when doc 93 §Y7 re-priced the upgrade ladder and the numbers moved. Doc 03 §2.5a's own §8 snapshot and report 98 §57's summary carried the stale pair too, so **three of the four places this number is written disagreed with the one the game reads**. Nothing was wrong in play: the array is the only thing `CostCurves.level_up_grant` reads. **Found while re-scaling the table** (Wave 22, RR-187), and the whole table is rewritten so the specific instance is gone. Filed anyway, because the CLASS is not: a derivation string is prose beside data, and no test in this project reads prose. **What would close it:** a test that re-derives a published curve from its stated rule and fails when the two part company — cheap here (the rule is one multiplication against doc 09's own costs) and not cheap in general, which is why this is Low and filed rather than fixed. Doc 92 §61.1. |

| **A91-D-100** | ~~**Low**~~ **✅ CLOSED 2026-09-03 (Wave 19)** | **The restore has a ledger CATEGORY and no lifetime ROW, and the reason is a hash rather than an oversight.** `Treasury.spend(cost, &"restore", …)` books the charge correctly and `_note_lifetime` has no `&"restore"` arm, so `ledger_totals` will never carry what the city has spent bringing itself back — the same gap `A91-D-37` names for `&"incident"`, from the same cause and with the same fix. It is **deliberate here**: `Treasury.lifetime` is captured into `canonical_capture().ledger_totals` and therefore into `state_hash()`, so adding a key moves **all four `profile_sim` baselines on every city**, and Wave 18 is a player-verb lane that must move none. Closes in a lane that holds the balance matrix, together with `A91-D-37`'s `&"incident"` arm, as one `ledger_totals` edit and one re-record — doing them separately would cost two baseline re-records for one schema change. Consequence until then: a player can see the charge in the treasury and in the budget panel's hourly view, and cannot ask the city what it has spent on restores for life. Doc 93 §AN5. **CLOSED 2026-09-03 by report 98 §59 RR-166, exactly as this row specified**: Wave 19 held the balance matrix, `lifetime_restores` and `lifetime_dispatch` were added in one `ledger_totals` edit with A91-D-37's arm, `lifetime_relief` joined them because RR-167 creates a grant that would otherwise be un-auditable, and all four baselines were re-recorded once. |

#### New rows, Wave 19 (2026-09-03) — `A91-D-103`, `A91-D-104`, `A91-D-105` (the one-way door)

*All three were found by ONE instrument — `tools/measure_catastrophe.gd`, doc 92
§56 — built because a player said their whole city was destroyed and nothing in
the project could say by what. Two are closed here. The third is the one the
measurement found by accident and it is the most interesting of the three: it
says the disaster layer does no damage at all, which is the opposite of the
complaint that started the wave.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-103** | ~~**High**~~ **✅ CLOSED 2026-09-03 (Wave 19)** | **`relief_grants_per_era` has never had an era, so the recovery ladder's bottom rung was a LIFETIME allowance of three.** Doc 03 §2.10 layer 5 is the last thing under a city that has fallen over, and `data/difficulty.json` publishes 4 / 3 / 2 / 0 grants "per era" across the presets. `Treasury.relief_grants_used` is incremented in `maybe_grant_relief`, persisted in `serialize()`, restored in `deserialize()`, and **reset by nothing anywhere in the project**: `grep -rn "era" sim/` returns the word in exactly one place, the key's own name. So a standard city gets three state grants for its entire life, in a game designed around weeks of play, and a player who has spent them has reached a dead end with no losing screen — which is the state the 2026-09-03 report describes (*"there's negative money … I've been trying to restore all the buildings"*). **Why nothing caught it:** the key was authored in doc 03 §2.9's table with a name that implies a period, no doc ever defined the period, and a counter that only goes up looks correct in every unit test that grants once. Same shape as A91-D-19 — a model whose *other half* was never written — with the twist that the missing half is one word in a key's name. **Closed by report 98 §59 RR-167:** `Treasury.note_era(city_level)` resets the allowance, called from `CitySim._pay_level_up_grant` on the same transition that pays `LEVEL_UP_GRANT_BY_CITY_LEVEL`. An era is a city level: already tracked, already persisted, composed from both routes by doc 93 §G1, and monotone — so it cannot be farmed by oscillating anything. Crisis stays 0 per era by ruling. `tests/test_relief_ladder.gd`. Doc 93 §AP4(a). |
| **A91-D-104** | ~~**High**~~ **✅ CLOSED 2026-09-03 (Wave 19)** | **The disaster-relief grant was proportional to the revenue the disaster had just destroyed, so the worse the catastrophe the smaller the relief.** `maybe_grant_relief` computed `clamp(RELIEF_DAYS_OF_REVENUE × daily_gross_revenue, RELIEF_MIN, RELIEF_MAX)` — and `daily_gross_revenue` is measured on the city *after* the loss, where a ruin has `output_mult` 0.0. **Measured at the limit the player reported** (doc 92 §56.5, `measure_catastrophe --relief --warm=21`): a standard city of **251 ruins** carrying a **$238 280** restore bill was offered **$8 000**, i.e. `RELIEF_MIN`, and the same $8 000 on all four presets, because at total loss every preset's revenue term is zero. The rung was thinnest exactly where it was the only rung. **Why nothing caught it:** the formula is correct doc 03 §2.10 prose and it is right for the case it was written for — a city bleeding money on operating costs — and nobody had ever measured it against the case it is *named* for, a disaster, because until 99-PA PA-04 the Director stalled after two events and no test drove a city to ruin. **Closed by report 98 §59 RR-167:** relief is `max(revenue term, RELIEF_DAMAGE_FRACTION × outstanding restore bill)` inside the same clamp — $83 398 on standard, 10.4× — where the bill is `CostCurves.restore_cost_building` summed over the actual ruins, the identical call `cmd_restore_building` charges (C-07 keeps one price). `RELIEF_DAMAGE_FRACTION = 0.35` is `DEFERRED_REPAY_FRACTION` adopted, not invented. **The anti-farm is an inequality, not a hope:** 0.35 < 1, so the grant never covers the bill and wrecking your own city always loses money; `tests/test_relief_ladder.gd` asserts the inequality rather than a dollar, so a later retune cannot quietly cross 1. Doc 93 §AP4(b). |
| **A91-D-105** | **High** — 🟡 **OPEN, for the lane that owns doc 06's escalation ladder** | **The entire incident and disaster layer does ZERO damage to buildings, and it took a wave about too much destruction to find out.** `tools/measure_catastrophe.gd` tallies `building_damaged` by cause, and across **all eight arms** — 45 game-days × 4 presets × 2 session kinds — the column reads the same shape every time: **`decay=61 incident=0 fire=0`** (61 on standard; 64 casual, 7 hard, 5 crisis, and `incident` and `fire` are zero in every one). Not one building in the project is damaged by an incident, a Director event or a storm over 45 game-days at any difficulty. The mechanism: doc 06's `building_condition` cascade ops live on the **tier-2 and tier-4** escalation rows of `structure_fire`, and auto-dispatch resolves a founding-scale city's fires before they escalate that far, so the ops never run; `storm_damage` carries none at any tier; and `FloodField` has no path to a building's condition at all — `grep -rn "flood" sim/ \| grep -i damage` finds nothing, so **the "water flooding" in the 2026-09-03 report could not have damaged a single building.** The consequence is that a storm is currently spectacle: the player said they enjoyed that a disaster happened, and mechanically nothing happened. **Why nothing caught it:** every doc 06 test drives an incident to a chosen tier and asserts the op fires, which it does; no test and no report had ever asked how often the *shipped* generators reach that tier in a *played* city, and doc 92's pass-2 matrix could not ask because it ran every hour as a catch-up hour with the Director structurally silent. **Not fixed here, deliberately.** The honest sequence is Wave 19's door first — doc 93 §AP1/§AP2, so that damage stops being terminal for private stock — and then a measured damage pass on the escalation ladder; doing both in one measurement would open a damage path and close a destruction door at the same time, with no way to attribute either. Doc 93 §AP3 states the four Director verdicts that come with it. **Fix:** measure the tier distribution the shipped generators actually reach per preset, then either move the `building_condition` ops down the ladder or author damage on the tiers a real city reaches — priced against doc 92 §56's ruin counts so the two rulings are balanced against each other rather than in sequence. |


#### New rows, Wave 19 (2026-09-03) — `A91-D-106`, `A91-D-107`, `A91-D-108` (money you actually collect)

*`A91-D-106` is `A91-D-19`'s shape for the **seventh** time and, like the sixth,
a player found it from the outside. What is new in this instance is that the
thing with no caller is not a transition or a price but a whole COLUMN: two of
the game's three collectible rewards had no city-level term at all, so they did
not break, they SHRANK — silently, for the life of every city, and only against a
denominator no test ever computed.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-106** | ~~**High**~~ **✅ CLOSED 2026-09-03 (Wave 19)** | **A flat reward is a shrinking reward, and two of the three the player collects were flat.** `data/economy.json city_services.dispatch_payout_base` has no city-level term in any form: `dispatch_payout(type, shape, manual, …)` multiplies it by doc 06's TIER and SPEED factors and by `MANUAL_DISPATCH_MULT`, and every one of those is a property of the *incident*, none of the *city*. So doc 06's reference crime — tier 3, answered on target — paid **$595 on game-day one and $595 on game-day three hundred**, while the same city's net per real-minute went **537.7 → 2,755.4** (`tools/measure_curriculum.gd --days=45 --seeds=1337,4242,9001`). In real terms the reward for answering an incident lost **80.5 %** of its value across doc 09's ladder. `STREET_REWARD_CITY_LEVEL_K` had the same disease more slowly (1.8× by level 5 against a city 3.3× richer) and its own note recorded the ratio as a virtue — *"that is the direction that keeps the street a second income"* — because the denominator it was checked against was a run AVERAGE. **The shape to look for elsewhere: a reward whose every factor is a property of the event and none of the payer.** **Why nothing caught it:** balance gate 32 measured the founding city (where the curve is 1.00 by construction) and gate 31 measured the moral-hazard ratio (which is scale-free), so both were green at every point on the decay. **The player found it from the outside, 2026-09-03:** *"The crimes we stop are only a few hundred dollars — add a zero to that."* Closed by report 98 §60 RR-169: `MANUAL_DISPATCH_LEVEL_K` 0.90 on the manual half only, `STREET_REWARD_CITY_LEVEL_K` 0.20 → 0.25 fitted against the new per-level series `pacing_guardrails.MODEL_NET_PER_HOUR_BY_CITY_LEVEL`, and balance gate 32 gains arm (d2), which asserts the ceiling share at **every rung** instead of only at the founding hour. |
| **A91-D-107** | **Medium** — OPEN, **for the lane that owns doc 02 §2.12's transition table** | **Restoring a ruin and then demolishing it nets +5 % of its capital, at every level and every archetype, on the FIRST cycle only.** `RESTORE_COST_FRACTION` is 0.20 and `DEMOLITION_REFUND_FRACTION` is 0.25, and both are read off the same `capital_value(level)` — so `cmd_restore_building` followed by `cmd_demolish_building` is `−0.20 + 0.25 = +0.05 × capital` per cycle. On the starter city's power plant that is **+$3,000**. It has been true since Wave 18 shipped the restore and nothing bounds it but the rebuild's construction time and the crew it occupies (and `cmd_demolish_building` accepts an `under_construction` building, refunding the job as well, so even that bound is soft). **Found while deriving `SALVAGE_FRACTION`'s FLOOR, not while hunting for it** — which is the useful part of the row: the two fractions had never been read against each other because they were derived against different things (0.20 against the repair a maintaining player buys, 0.25 against every other demolition in the project). **Not closed in Wave 19, deliberately:** moving either fraction to fix a third thing leaves a fraction that no longer means what its own note says, and the likely honest fix is neither — a short post-rebuild window in which the demolition refund IS the salvage fraction, which is one authored number and one state field on the transition table's owner. **What Wave 19 does instead is make it pointless:** `cmd_salvage_building` pays 0.15 of capital immediately, with no construction time and no crew, so the exploit is strictly dominated by the button beside it. Doc 93 §AQ4. **Two figures corrected at merge by the verify pass, 2026-09-03:** the gain is **$3,000** (0.05 x the plant's $60,000 capital), which is what doc 92 §57.2.2 and doc 93 §AQ4 already said — this row was the only place carrying $4,625 — and it is **not repeatable on the same lot**: the second attempt answers `E_UNKNOWN_BUILDING`. `check_doc_refs.py` validates ids and cannot see a wrong dollar, which is why a number that three documents disagreed about survived until someone re-ran the command. |
| **A91-D-108** | **Low** — OPEN, **for the lane that holds the balance matrix** | **A third ledger arm is now owed, and the reason is still a hash.** `cmd_salvage_building` credits `&"construction"` (which is where a demolition refund already goes, so the category is honest) rather than a `city_services` source of its own beside `street`. A `salvage` sub-row would let the budget panel say what the city has raised by clearing its own wreckage, which is exactly the kind of money the 2026-09-03 report is about. It is not taken because `Treasury.lifetime` is captured into `canonical_capture().ledger_totals` and therefore into `state_hash()`, so one key moves **all four `profile_sim` baselines on every city**. **This is now the THIRD arm waiting on the same one-line schema change** — `A91-D-37`'s `&"incident"`, `A91-D-100`'s `&"restore"`, and this — and the row exists mainly to say that: three separate lanes have each declined the same edit for the same correct reason, and the cost of doing them one at a time is three baseline re-records for one change. Fix: `hour_city_services` gains `"salvage"`, `_note_lifetime` gains the three arms, and the four baselines are re-recorded once with the delta published. Doc 93 §AQ2. |

#### New rows, Wave 24 (2026-09-04) — `A91-D-124`, `A91-D-125`, `A91-D-126` (a million at the first rung)

*The lane went looking for what a $1,000,000 first grant breaks and found two
things that were already broken and one that has been broken since Wave 6.
`A91-D-124` is `A91-D-120` recurring exactly one wave later, in the same file
family, and this time the stale prose was the DOC rather than the derivation
string. `A91-D-125` and `A91-D-126` are the same finding read from two ends: a
balance gate that has been over its own ceiling on an unasserted seed for five
waves, and the reason nobody could see it — a metric that sums three unrelated
failures into one number.*

| id | severity | finding |
|---|---|---|
| **A91-D-124** | ~~**Low**~~ **✅ CLOSED 2026-09-04 (Wave 24)** | **Doc 03 published a grant table the game never had — for a whole wave, in two places, and it was the REJECTED draft.** Doc 03 §2.5a's headline line and its §8 `data/economy.json` snapshot both read `[0, 215000, 235000, 255000, 275000, 300000, 325000, 5000000]` after Wave 22 merged. That array is not a stale earlier table: it is the **flat draft §61.12 measured and threw away** for failing eight assertions across seven balance gates. The shipped array was `[0, 45000, 65000, 95000, 145000, 215000, 325000, 5000000]`, and the paragraphs *underneath* the headline derive the shipped one correctly, cell by cell, in a table of its own — so the doc contradicted itself two lines apart and agreed with the game in the second half only. **This is `A91-D-120` one wave later** (that row is about the same file's derivation STRING disagreeing with its own array) and it is the same root cause: prose beside data, and no test in this project reads prose. Closed by rewriting both places for Wave 24's table; the CLASS stays open under A91-D-120, whose *"what would close it"* — a test that re-derives a published curve from its stated rule — would have caught this instance too, since the rule is one multiplication. |
| **A91-D-125** | **Medium** — OPEN, **for the lane that owns doc 06's dispatch capacity or doc 04's repair verb** | **A city's grid is never repaired, and the balance gate that would notice asserts one seed.** Gate 18b runs `balanced` for 50 game-days on seed 1337 only. Measured at this fork with `tools/measure_dark_share.gd`, on the SHIPPED Wave-22 build with no Wave-24 change of any kind: **1337 = 5.99 %, 4242 = 20.98 %, 9001 = 9.52 %.** The ruled ceiling is 20 %, so **seed 4242 has been over gate 18b's own bound since at least Wave 22 and nothing could see it.** `tools/probe_dark.gd` says what it is, and it is not capacity: at game-day 50 that city has **20 grid components sitting FAILED and unrepaired for the last fifteen game-days**, 82 buildings orphaned behind them, and a pool with headroom to spare. A FAILED component is restored only by doc 06 resolving its incident (`CityIncidentWorld.power_restore_component`), the failure rate scales with the transformer fleet, and no agent in `tools/playtest.gd` buys a grid repair — so on the richer Wave-24 city the same shape is **105 failed and 234 orphaned**. **Why this lane did not close it:** the fix is either a grid-repair rule in the agent (a balance-matrix change) or a dispatch-capacity question (doc 06's), both of which are somebody's whole lane, and a bound fitted around it here would bury it. What this lane shipped instead is the instrument (`tools/probe_dark.gd`, plus `supply_kw_end` / `demand_kw_end` in the playtest summary) and the measurement above. **What would close it:** either an agent rule that repairs a FAILED component the way `_relieve_feeders` buys a substation, or a gate 18b that runs `MATRIX_SEEDS` — and the second is cheap and would have caught this five waves ago. Doc 92 §63.3; ruling 93 §AW4(a). |
| **A91-D-126** | **Medium** — OPEN, **for the lane that holds gate 18b** | **`unserved_share` is three unrelated failures wearing one number, and the gate asserts the sum.** Doc 04 gives a building exactly three ways to be dark — **unattached** (no transformer's radius covers its tile), **orphaned** (attached to a transformer with no live path to a source) and **starved** (the pool is short and §2.7 shed the circuit) — and `Playtest.Runner._blackout_minutes` adds all three into one share. A wave that reads the share as *"the city has too little capacity"* fixes whichever of the three it happened to guess. **That is not hypothetical: it is what Wave 22 did.** §61.12 diagnosed a 26.44 % reading as a city outrunning its grid and set a whole grant curve against it; the actual cause was **generation** — `supply_kw` pinned at one L1 plant's 8,000 kW — which no amount of copper addresses and which the same metric was ALSO hiding at the fork, inside gate 18b's own run. **What would close it:** gate 18b asserting the three components separately, which `tools/probe_dark.gd` already computes; the census is the cheap half and re-cutting the gate's assertion is the expensive one, which is why this is filed with the instrument attached rather than fixed. Doc 92 §63.3; ruling 93 §AW4(b). |

#### New rows, Wave 20 (2026-09-03) — `A91-D-110`, `A91-D-111`, `A91-D-112` (the spiral has a floor)

*All three were found the same way, and it is a way this project had never tried:
`tools/measure_player_city.gd` loads the **player's own save file** through the
real `SaveService` and advances it. Every catastrophe instrument before it
measured a city this repository generated, and doc 92 §56's headline conclusion —
"42 of 42 destructions come through wear, 0 through fire" — is **false on the
file**, where it is 0 through wear and 10 of 11 through fire. The shape to take
away: a fixture that reproduces a player's numbers is not the same thing as the
player's state, and a wave that rules on the first has not necessarily touched the
second.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-110** | ~~**High**~~ **✅ CLOSED 2026-09-03 (Wave 20)** | **Every building doc 06 takes down disappears SILENTLY, and it took a wave about losing buildings to notice.** `CityIncidentWorld.apply_building_damage` and `destroy_building` call `Building.apply_damage` / `burn_down` / `demolish` and **discard the event array all three return**. `CitySim.apply_hourly_decay` publishes those same arrays two files away, so wear deaths are announced and cascade-op deaths are not: no `building_destroyed`, no `building_damaged`, no notification, nothing for `GoalSystem` (which lists `building_destroyed` among the events it watches), nothing for a surface to count. Measured on the player's slot 0: over 45 game-days **the census showed twelve buildings gone and the bus reported one**. The player's city was being erased through a door that never announced itself — which is why the 2026-09-03 report reads *"ALL of my buildings are destroyed right now"* and not *"I watched them go"*. **Why nothing caught it:** doc 06's tests drive an incident to a tier and assert the op ran, which it did, and they use a test double that records the call rather than a bus; nothing anywhere asserted that the op's *consequence* was published. **A second fault underneath it:** `Building._destroy` stamps `building` with the **int** `Building.id` while every roster key is the authored **string** sim_id (`P-077`, `H-001`), so even the one publisher that existed could only be joined through the `sim_id` key it adds separately. Closed by report 98 §61 RR-175: `CityIncidentWorld._publish` mirrors `apply_hourly_decay`'s publish exactly, `sim_id` and post-transition `condition` included. Doc 93 §AR2a. |
| **A91-D-111** | ~~**Critical**~~ **✅ CLOSED 2026-09-03 (Wave 20)** for the two settlement lines; **🟡 one remainder OPEN** (the fleet, see the tail of this row) | **The city was billed for its own rubble, at the WORST rate either line can charge, and that was the death spiral's engine.** `CitySim.build_settlement_inputs` walks `roster_ids()`, which keeps ruins (that is what makes RESTORE possible), and appends every one of them to the `buildings` array `E_building_maint` bills and the `stations` array `E_departments` bills. Neither line ever asked what STATE the building was in — and both scale on `1 − condition`, which for a ruin is exactly 1, so `MAINT_CONDITION_PENALTY` 1.5 charged a destroyed building **2.5×** what the same building costs in perfect repair and `ASSET_CONDITION_PENALTY_COEFF` 2.0 charged a destroyed station **3.0×**. **Every building that died raised the city's bill.** On the player's slot 0 at load: **$1,044.39/gh of $1,055.12 (99.0 %) of `E_building_maint` and 100 % of `E_departments`' $306.00/gh were charged against buildings that are rubble — $32,409 a game-day against a gross of $2,998.** `CityIncidentWorld.station_rows()` had the same defect one layer over: it is `FleetSystem.populate_from_stations`'s only source, so four ruined shells also housed a fleet doc 03 billed. **THAT HALF IS CLOSED FOR THE BOOT PATH ONLY, and the remainder is this row's open tail**: `populate_from_stations` runs once, before any restore; `FleetSystem.deserialize` then clears `_units` and rebuilds them from the save; and `sync_station` fires on a building's COMPLETION and never on its destruction. So a station destroyed while the city runs keeps its engines for the life of that city, and slot 0 still pays `E_fleet` $91.58/gh against four ruined shells after 45 game-days (doc 92 §58.2 — the number that caught the overstatement, on a re-read of the settled hour rather than of the guard). Not taken here because retiring a unit on destruction means retiring one that may be `DISPATCHED`, `EN_ROUTE` or `ON_SCENE`, with an assignment on a live incident and an id in the save — doc 06's ladder, not a settlement guard. **Fix:** a `FleetSystem.retire_station(station_id)` that releases assignments the way a vehicle breakdown already does, called from the same publish that announces `building_destroyed` for a station archetype. **Why nothing caught it:** every economy test measures a healthy city, and the only instrument that ever built a heavily-ruined one (`measure_catastrophe.gd --relief`) demolished the roster to price a *grant* and never settled an hour afterwards. **This is why the relief ladder looked broken and was not**: doc 03 §2.10 layer 5 paid this city **$296,181 in three grants, 105 % of its entire $282,078 restore bill**, and it vanished into this line. Closed by report 98 §61 RR-176: one `state == destroyed` guard in `build_settlement_inputs`, one in `station_rows`. All four determinism baselines unchanged — neither reference city holds a ruin inside 24 game-hours. Doc 93 §AR3. |
| **A91-D-112** | **Medium** — OPEN, **for the lane that holds the balance matrix** | **A collapsed city can never re-open its relief allowance, because an era is a level and levels only go up.** Doc 93 §AP4 defined `relief_grants_per_era`'s era as a city level and `ProgressionSystem.city_level` is monotone by construction (`level_up` returns early on `reached <= city_level`, and `deserialize` reads `city_level_max`). So a city that has fallen — the one city the ladder's bottom rung exists for — spends its three grants and gets no more, ever. On the player's slot 0 the allowance is exhausted by game-day 15 and the treasury goes back under by game-day 20 with `relief used 3/3 in era (level 5)` for the remaining thirty game-days. **Not closed in Wave 20, deliberately, and the reason is a hash.** The honest fix is a latch — one persisted "this collapse has already been counted" bit, so the re-open is one-shot per collapse and proportional to damage rather than to time — and `Treasury.serialize()` is inside `canonical_capture()` and therefore inside `state_hash()`, so a single bool moves **all four `profile_sim` baselines on both cities**. Wave 20's four baselines are UNCHANGED, which is the most useful single fact it hands the next reader about what its rulings are, and spending that on a bit is a bad trade while §AR3 has just made the existing three grants sufficient: the same relief now leaves the city at **+$68,464 on game-day 14** with every building it still had. **⚠ WITHDRAWN, Wave 21 (doc 92 §60.9):** that number was taken before its own branch's last checkpoint and re-measures at **+$57,032 with SIX of twelve buildings** on `f27c402`, and at **−$26,832 with all twelve** on the Wave-21 tree. The row and its severity stand; the sufficiency argument does not, and the deferral now rests on the schema cost alone. See the Wave-21 rows below. **This is the FOURTH item queued behind the same one-line schema change** — `A91-D-37`'s `&"incident"` arm, `A91-D-100`'s `&"restore"`, `A91-D-108`'s `salvage` sub-row, and this — and four lanes declining the same edit for the same correct reason is now itself the finding. Fix: take all four in one commit, re-record the four baselines once, and publish the delta with each cause named. Doc 93 §AR4. |

#### New rows, Wave 25 (2026-09-04) — `A91-D-132`, `A91-D-133`, `A91-D-134` (land that looked like nothing was happening to it)

| # | Severity | Defect |
|---|---|---|
| **A91-D-132** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 25)** | **The render layer had never heard of land development, and `grep` says so in one line: `grep -rn "CLEARING\|GRADING\|development_state" game/` came back EMPTY at the fork.** Doc 09 §2.3's six-phase pipeline has run since Wave 4 and doc 03 §2.8 has charged the treasury for it fourteen or fifteen times per 21 game-days at $1.2K–$21K a phase — and a block being dug out was drawn exactly like a block nobody had touched. The whole feedback for six charges was a progress list on a panel the player had to go and open; the world said nothing. **This is the A91-D-19 shape in its second direction.** The usual form of this project's signature defect is behaviour with no consumer; this is a MONEY EVENT with no surface, which is the same fault read from the other end — a player who cannot see what they bought cannot tell a working purchase from a broken one, and 99-PA PA-83 had already found the same silence on the LEDGER side (`development_phase_charged` had zero shell consumers until Wave 18 gave it a log row). **Why nothing caught it:** no test in the project asserts that a sim state has a renderer, and the event matrix explicitly rules that a renderer IS a consumer — which is correct, and is exactly why an event whose only honest consumer would be a renderer that does not exist reads as "covered" in `tests/test_event_matrix.gd`'s register. `development_phase_started` and `development_phase_completed` are both registered there as *"covered: same land-panel poll"*, and that was true and insufficient. Closed by report 98 §69 RR-211: `game/render/land_works_view.gd`, six phases, six buffers, zero draw calls on a city with nothing in flight (doc 11 §2.18). |
| **A91-D-133** | ~~**Medium**~~ **✅ CLOSED 2026-09-04 (Wave 25)** | **An instance tint authored as an sRGB hex and written raw into a MultiMesh renders as if it were linear, and this project has now made that mistake three times.** `ConstructionRigMesh`'s own header records the first two (RR-91's mesh-side fix, and the `SAND` / `GRAVEL` / `REBAR` dual-use trap it created); `LandWorksView` made it a third time on its first draft, and a dark olive scrub rendered as pale sand across a whole block. **The rule exists and was not findable from the call site**: *"convert at the WRITE, so each consumer decodes once, from one authored source of truth"* is written inside `ConstructionRigMesh`, which is the MESH builder, while the mistake is made by whoever writes an INSTANCE colour — a different file, a different call, and no compiler or test between them. **Why nothing caught it:** a colour-space error is invisible to every headless assertion in the project. The instance count was right, the transform was right, the buffer was right; only a photograph shows it, and the first photograph of this layer is what found it. `tools/land_works_preview.gd` exists partly for this. **Fix, and the general form:** the decode happens once at the READ in `LandWorksView._col`, so the palette member IS linear and no per-instance conversion is possible to forget. Filed as a row rather than closed silently because the next new prop layer will be the fourth. |
| **A91-D-134** | **Low** — OPEN, **for the lane that next touches `data/render.json`'s prop pages** | **`ConstructionRigMesh` bakes UVs in MESH space, so a `prop_*` page is only physical while an instance is near unit size — and nothing says so at the call site.** `LandWorksView` scales one unit quad to 126 m for its graded plane and one unit slab to 20 m runs for its base course; the pages stretched by two orders of magnitude and read as smeared rectangles, which was the SECOND thing the first photograph of the layer showed. Worked around correctly for now — those three layers take flat vertex-colour materials, which is exactly what `PropSurface` degrades to on a clone with no generated pages, and the split is documented in doc 11 §2.18 — but the general fix is a triplanar variant of `PropSurface.material` that maps in WORLD space at `prop_tile_m`, which would let a 126 m plane carry the same grain a 0.22 m crane leg carries. **Not taken here** because triplanar is three texture samples and doc 11 §2.13's mobile budget is the wrong thing for this lane to spend on a ground plane it can tint flat. **Severity Low and honest about it:** the shipped look is correct, this is a quality ceiling rather than a defect, and the row exists so the next prop layer does not rediscover the constraint by photographing it. |

#### New rows, Wave 21 (2026-09-04) — `A91-D-115`, `A91-D-116`, `A91-D-117` (a fire nobody could answer)

| # | Severity | Defect |
|---|---|---|
| **A91-D-115** | ~~**Critical**~~ **✅ CLOSED 2026-09-04 (Wave 21)** for the ruling; **🟡 `A91-D-111`'s fleet remainder still OPEN and now re-measured** | **A protection that reads the treasury is strongest when the player is worst-run, and doc 93 §AR2 shipped one.** `CityIncidentWorld._could_have_answered` opened with `if sim.treasury != null and not sim.treasury.austerity_active: return true` — "could the player have BOUGHT a fire station?" — so a fire's terminal outcome demolished or condemned depending on the BALANCE. Two independent refutations, both reproduced on the player's slot 0 in doc 92 §60.2. **(a) It switches itself off:** relief lifts austerity on game-day 11 at +$59,753, the protection ends, and the roster goes 12 → 6 in that same window — `fire` 5 and `damage` 1, the city's LAST power plant among them. **(b) Held under the line it is total immunity:** with the treasury kept below the austerity threshold and no fire station, the same save returns **76 buildings alive at game-day 45 and the identical 76 at game-day 90 — zero destructions in 45 consecutive game-days** — while the bus emitted `building_destroyed_by_fire` 8,938 times about buildings that never moved. The correct play on that build is to stay broke. **Why nothing caught it:** the branch's own gate (29) asserts that a `do_nothing` city eventually dies, and it does — through wear, on a preset whose fires the clause was silently exempting; no test and no instrument ever asked what the clause did to a city that RECOVERED, which is the only case it changes. Closed by report 98 §63 RR-182: the predicate is `has_fire_capability() and answerable`, a `fire_station` standing AND an engine in the fleet, and it reads no money at any point. **The re-measured remainder of `A91-D-111`:** on Wave 21's final tree the same save pays `E_fleet` **$91.58/gh — $2,198 a game-day — at game-day 45 and again at game-day 90 with NO station of any kind standing**. Still not closed, for `A91-D-111`'s reason (retiring a unit means retiring one that may be dispatched, en route or on scene) and for a new one: it is 14 % of that city's $16,012 game-day expense, so it changes no verdict in this lane and should be taken with doc 06's dispatch ladder rather than beside it. Doc 93 §AS1, §AS5. |
| **A91-D-116** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 21)** | **Two bus lines were lying about the city, in opposite directions, and one of them at six figures.** **(a) 279,071 `dispatch_blocked_unreachable` in 45 game-days** (the lane brief's reading on the merged tree; re-measured on the fork `f27c402` at **255,050** over the same 45 and **466,321** over 90) — 236 a game-hour on a city with twelve buildings, every one of them republished by `CitySim` onto the shared bus. `DispatchSystem._emit_blocked` kept ONE de-dup slot holding `"<event>:<role>"` and suppressed only an exact repeat, so an incident blocked on two roles for two reasons overwrote the slot on every need, on every integrator sub-step, and announced both forever. The ratio is the finding: **154 announcements per terminal incident**, where the honest floor is one. **(b) `building_destroyed_by_fire` for buildings that were still standing** — 3,891 of them in 45 game-days on the fork's restore arm, against a census showing the same 76 alive at 45 and at 90 — because `CascadeOps._destroy_building` emitted the event whatever `world.destroy_building` actually did, and under the condemn ruling it often condemns. `GoalSystem` lists this event among the ones it watches. **Why nothing caught it:** `test_incidents_dispatch.gd` asserts `blocked > 0`, which is true at 1 and at 279,071, and no test anywhere asserts an UPPER bound on an event count; the destruction event was asserted by the op's own test through a double that records the call rather than the roster. **And a third fault under (b), found while closing it:** `tests/test_event_matrix.gd` — the instrument whose entire job is "every event `sim/` emits is consumed or classified, and every router row names an event `sim/` emits" — scans for `bus.emit(` and `_emit(` only, and doc 06's `CascadeOps` publishes through `IncidentSystem.emit_event`, whose body calls `_emit(type, …)` with a VARIABLE. **Five call sites and four event types were invisible to it**, `building_destroyed_by_fire` among them, which is why (b) could go a whole wave unnoticed: no router had ever named that event either, so nothing on either side of the matrix was looking. Closed by report 98 §63 RR-184: the de-dup slot is a set (**29,685 in 90 game-days, 1.01 per incident**), `IncidentWorld.destroy_building` returns whether it destroyed so the op can emit `building_condemned_by_fire` when it did not, the scan pattern gains `emit_event(`, the three bookkeeping types get written classifications, and **both** of a fire's endings become `event_log` rows. The matrix reads 168 / 98 / 70 against 163 / 96 / 67. Doc 93 §AS3. |
| **A91-D-117** | **Medium** — OPEN, **for a doc 06 / doc 09 lane with its own derivation** | **A city that cannot answer anything triples its own ignition rate and keeps it there.** Every abandoned incident applies its type's `district_stability` penalty, and doc 06's fire generator multiplies by `f_arson = 1 + arson_k × max(0, arson_stability_knee − stability) / arson_stability_knee` = **3.0** at stability 0, on top of `f_power` 1.8 for a dark building. A city with no police station, no fire station and no construction yard therefore drives its own arson term to the ceiling and holds it. Measured on the player's slot 0 on Wave 21's final tree: **28,573 incidents born in 90 game-days**, essentially all of them abandoned unanswered, against doc 06 §2.10.1's own worst LEGITIMATE arrival rate of 26 a game-day. **This is bounded, and that is why it is Medium and not High**: doc 06 §2.13's `saturation_ceiling` 40 caps the open roster and the taper throttles births, so it is steady-state throughput and not a ratchet — `incident_roster_saturated` fires 2,301 times and the roster never exceeds the ceiling. It is filed because it is the reason a saved city is still a LOUD city: the roster survives Wave 21's rulings and then spends ninety game-days at doc 06's ceiling. **Fix:** a floor under the stability term, or a coverage-aware generation damper, either of which is a balance number needing its own derivation and its own gate — not a guard. Doc 93 §AS5. |

**`A91-D-112` amended, Wave 21.** Its closing argument — *"§AR3 has just made the
existing three grants sufficient: the same relief now leaves the city at
+$68,464 on game-day 14 with every building it still had"* — was measured before
its own branch's last checkpoint and does not reproduce. On `f27c402` it is
**+$57,032 with SIX of twelve buildings**; on Wave 21's tree, with §AS4's per-era
cap in, it is **−$26,832 with all twelve** (doc 92 §60.9). The ROW stands and its
severity is unchanged — a collapsed city still cannot re-open its allowance, and
on this save it takes nothing for the seventy-five game-days after game-day 15 —
but "the three grants are sufficient" is withdrawn, and the row is now the FIFTH
item queued behind the same one-line `Treasury` schema change rather than a
deferral justified by sufficiency.

#### New rows, Wave 23 (2026-09-04) — `A91-D-121`, `A91-D-122`, `A91-D-123` (the predicate measured the shell)

| # | Severity | Defect |
|---|---|---|
| **A91-D-121** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 23)** | **A predicate that decides whether a city is deleted was reading a BUILDING to answer a question about a SERVICE — and it was answering it wrong on the player's own save.** `CityIncidentWorld.has_fire_capability()` required a `fire_station` in `active`, `damaged` or `repairing`, on the stated ground that a station that has not opened cannot roll an engine. The ability to answer a fire lives in `FleetSystem`, and doc 93 §AR3's own finding is the refutation: `sync_station` fires on a building's COMPLETION and never on its destruction, so **every destruction path in the game leaves the engines in the roster, on duty**. Measured on one tree, both readings at the same instant (doc 92 §62.1): `fire_station shells standing 0 | fleet 14 units (fire 1, …) | §AS1 shell reading false -> §AV1 service reading true`, while the city paid $91.58/gh of `E_fleet` for the fleet the predicate could not see. **Three consequences, all measured by Wave 21's own adversarial verifier and all merged deliberately as strictly-better-than-dying:** the protection switched ON for a city that still had a working fire service (28 of 28 incidents answered, per that verifier); `start_upgrade()` on the only station switched it ON for the whole of the works, because `under_construction` was excluded from "standing" — true of a level-0 new build and false of a level-1 upgrade whose engines never left the bay; and the FIRE question was asked on `apply_building_damage`, the door EVERY hazard's `building_condition` op goes through, so a city with no fire department could not have a building finished off by a flood or a storm (doc 92 §62.2's deterministic probe). **Why nothing caught it:** §AS1's own tests asserted the predicate on a fixture that called `Building.demolish()` and nothing else — a SHELL verb — so the fixture reached the ruling only because the ruling was reading the wrong thing. The fixture is now the pair a real bulldoze performs. Closed by report 98 §65 RR-192: `_could_have_answered(answerable, role)` = `has_service_capability(role) and answerable`, one fleet fact and the hazard's own `primary_role`. Doc 93 §AV1. |
| **A91-D-122** | ~~**Medium**~~ **✅ CLOSED 2026-09-04 (Wave 23)** | **Two remainders that survived their own rulings, both because a guard was written where the writer was not.** **(a) A FLOOR IS NOT A TERM.** Doc 93 §AS4 charged relief's damage term against `relief_era_paid` and published the sentence *"an era may never out-pay the bill it is measured against"* — while `clampi(…, RELIEF_MIN, RELIEF_MAX)` sat outside both terms and lifted every grant to $8,000. `RELIEF_MIN × relief_grants_per_era` = $24,000, so **every bill under ~$24,615 was out-paid and a $2,000 bill drew 12.0× itself**; the body disclosed the floor as an exception and the heading did not. Closed by charging the FLOOR to the era — it is guaranteed once, not once per grant, so $24,000 becomes $8,000 — and the residual is published rather than claimed away: a bill under `RELIEF_MIN` is still out-paid once (4.0×), because doc 03 §2.10 layer 5's bottom rung is not negotiable. **(b) "THE SOLE AUTHOR OF BOTH ARRAYS" WAS NOT.** `CitySim.build_settlement_inputs` says so of itself in the comment that implements §AR3's rubble guard, and the `water_works` staffing row is appended by a SECOND loop, over `water.nodes`, keyed on a pump existing in the GRAPH. A demolished `water_facility` takes its nodes with it; one that BURNS DOWN does not — the same asymmetry §AR3's own hole had. On the player's slot 0, three water plants in rubble and 2 pump nodes still in the graph billed **$20.00/gh, $480 a game-day, for plants that do not exist**. **The shape to look for elsewhere: a comment that claims exclusivity is a claim about every writer, and no test in this project reads comments.** Closed by RR-193 and RR-194. Doc 93 §AV2, §AV3; doc 92 §62.4, §62.5. |
| **A91-D-123** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 26)** — *and it names its successor* | **CLOSED, and the cause was not the one the row predicted.** The row said the agent had no water-headroom PLANNER and ranked one first among doc 92 §61's open questions. It does now (`Balanced._lead_water`, report 98 §70 RR-216) — but the thing that had actually pinned the capstone was smaller and worse: `Api.upgrade_grid_component` had been returning `E_NO_VERB` on its first line since Wave 22 because `cmd_upgrade_grid_component` was never listed in `KNOWN_VERBS`, and both of its early returns bypassed the action log, so three waves of runs recorded an idle agent standing in front of a **$6,900** purchase that clears a **$29,900** upgrade with **$19,541,802** in the bank (A91-D-137). **Measured, `tools/measure_curriculum.gd --days=45`:** curriculum level 7 goes from **— / — / —** at the fork to **h593 / h301 / —** on seeds 1337 / 4242 / 9001, and the pressure zone the level-7 high-rise drinks from goes from **supply 77.8 / demand 127.3 / pressure 0.21** to **214.0–254.1 / 183.0–215.2 / 1.00** on every seed. The whole capacity programme costs $1.47M–$1.77M against ending treasuries of $24.6M–$34.0M. **Gate 21's count is NOT re-fitted up** (doc 92 §67.7): the count is 2 of 3 but the SET is seed-chaotic — two versions of the same planner, differing only in whether the water arm opens on a pump or on the term that binds, finish `{1337, 9001}` and `{1337, 4242}` — so the floor stays at 1 and the strength moves into a per-seed assertion a re-roll cannot flip: **no live pressure zone may end a run in doc 05 §5.8's CRITICAL band** (0.21 at the fork, 1.00 on every seed here). **Its successor, and it is the row the next lane should open:** the wall is now DISTANCE and PIPES, not supply. Seed 9001 stops with 57.1 m³/h of unused headroom in a zone at pressure 1.00 and its high-rise refused at `pressure_at(tile)` **0.50** against doc 05's 0.55 gate — a tile factor, i.e. how far the tower stands from a main; seeds 1337 and 4242 end supply-capped at exactly `feed_capacity` 214.0 with every node term above it. **`cmd_place_water_main` is probed, listed and driven by nothing**, and it is the only verb that moves either number. Doc 92 §67.8, §67.9 item 2; doc 93 §BA. |
| **A91-D-135** | **Low** — OPEN, **for the lane that builds doc 03's austerity layer** | **§AV1's fire-service predicate excludes `OFFLINE` units, and the only thing meant to park a unit `OFFLINE` is a money-driven layer that does not exist yet.** `sim/incidents/city_incident_world.gd` (lines ~282 and ~677) counts a fire engine toward `has_fire_capability()` only while `u.status != Vehicle.OFFLINE`, and its own comment attributes `OFFLINE` to "doc 03's austerity layer". Nothing in `sim/`, `game/` or `ui/` assigns `OFFLINE` today (Wave 23's merge verifier grepped all three), so wealth is genuinely outside the predicate now — `test_wealth_moves_no_roster_at_either_extreme` confirms it at ±$5,000,000. But the verifier parked slot 0's single engine `OFFLINE` by hand and the predicate flipped to false: the day austerity is built, a broke city loses fire capability through this exclusion and doc 93 §AV1's wealth ruling silently reverses — and the shipped wealth test cannot catch it because it never advances the clock. Fix: either the austerity layer must never park the LAST engine of a role, or the predicate must count parked engines as capability-with-a-delay; and the wealth test must advance at least one game-hour with the layer active. Filed by the Wave 23 merge, 2026-09-04. |
#### New rows, Wave 24 (2026-09-04) — `A91-D-127`, `A91-D-128`, `A91-D-136` (why the population does not visibly count up)

| # | Severity | Defect |
|---|---|---|
| **A91-D-127** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 24)** | **A derived aggregate that only a periodic tick writes reads as ZERO between the moment a city opens and that tick, and every consumer reads the zero with it.** `PopulationSystem.advance` is `HourlyPhaseSystem`'s, cadence `EVERY_HOUR`, and it is the only writer of `city_population`; doc 08 does not persist the aggregates because they are derived. So both doors into a city opened on `0`. **A fresh boot held it for one SimTick** — survivable — **and a RESTORE held it until the next hour boundary, which on a save taken 21 ticks past the hour is 220 ticks = 55 REAL SECONDS** (doc 92 §64.3), during which `main.gd`'s HUD, `build_director_inputs`, `goal_state_view()` *inside `_restore_finish` itself*, and doc 07's `storm_ready_earned` all read it. The last of those prices its reward against `outage_cm_per_1k_pop × pop / 1000`, so a zero population makes the budget zero and the Storm Ready relief unearnable for a storm report published in that window — **a display defect that costs money**. **Closed by RR-202:** `PopulationSystem.settle_aggregates` — pass 1 of `advance` with no `dt_h` and no write to the persisted occupancy map — called at the end of `boot()` and at the **end** of `_restore_finish`, with a `deserialize` that stops the previous city's totals surviving into the new one. **The ordering cost one whole-suite run to find**: settling BEFORE `_restore_goals` hands doc 09 §2.14's restore-time reconcile a population the save does not record, and `GoalSystem.serialize` writes `done`/`progress`/`earned_level` into `state_hash` — on the 1,500-building benchmark city that broke doc 08's boot → save → load identity (`tests/test_save_migration.gd::test_37_…`, the only test in 2,795 that could see it). **The shape to look for elsewhere: any `var` in `sim/` that is written only inside a phase system, not written by `deserialize`, and read by a UI snapshot or a command.** `HappinessModel.happiness` is NOT one (it is persisted); `districts.city_stability` and `economy`'s hourly rollups are the next places to check, and neither was checked in this lane. Doc 93 §AX1, doc 92 §64.3. |
| **A91-D-128** | **Medium** — OPEN, **for a doc 09 lane with the lead's ruling in hand** | **A published curve that no call site can reach: doc 09 §2.10's occupancy ramp does not run in the shipped game.** `PopulationSystem.ramp(age_hours)` implements *"a new building opens at 35 % and fills over 36 game-hours"* exactly as doc 09 §2.10 writes it, and `CitySim._population_inputs` — the only caller that has ever existed — hands it a hard-coded `48.0` for **every building in the city**, which is above `OCCUPANCY_RAMP_HOURS`, so `ramp()` returns 1.0 always and the curve is inert. Measured side by side at the moment a house completes: **0.386 (doc) against 1.000 (code)** (doc 92 §64.2). The literal is now `CitySim.POPULATION_INPUT_AGE_HOURS` and carries the argument, so it can no longer be read as an accident. **NOT fixed in this wave, deliberately:** wiring the real age is a balance change whose sign is known — it would add 36 game-hours of near-invisible fill on top of the 176 real seconds already measured between a tap and the population chip moving, which is the 2026-09-04 player report, made worse. **What would close it:** doc 93 §AX2's ruling (a) — delete the ramp from doc 09 and from `PopulationSystem`, since the aggregate has no per-arrival channel to show a fill through — or (b), wire it and pay for it with a doc 12 surface that shows the fill. **The shape to look for elsewhere: a `static func` in `sim/` whose every call site passes a literal.** Doc 93 §AX2, doc 92 §64.2. |

| **A91-D-136** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 24)** | **A method that documents its own callers, and has none.** `ui/building_panel.gd::refresh()` — *"re-reads the sim for the currently selected building (after an upgrade, a tick, or a construction completion)"* — was written in Wave 5 and **outside `tests/` nothing in the project has ever called it**. (`ui/ui_root.gd:1938` calls `land_panel.refresh()`, which is a different panel and is what made the absence read as present in a grep.) So S5 was a **still photograph**: an open building panel showed the city as it was at the moment of the tap and never moved again — its condition, its `Fix this →` checklist, S16's inline construction ETA and its `Occupants` vital all frozen for as long as the player kept it up, which on the shell's own numbers is the whole 176 real seconds a house takes to fill (doc 92 §64.1). **Closed by RR-203:** `UIRoot.refresh_building_panel()`, on `refresh_land_panel`'s exact shape and cost, called from `game/main.gd::_refresh_hud`'s 1 Hz cadence. **The shape to look for elsewhere: a public `refresh()`/`invalidate()` whose only callers are its own tests.** `grep -rn "\.refresh()" --include=*.gd ui/ game/` is the one-line check, and it is not run anywhere. Doc 12 D-100, doc 93 §AX3. |
### Wave 25 — the transformer is a thing you tap (report 98 §68)

| id | severity | finding |
|---|---|---|
| **A91-D-129** | ~~**High**~~ **✅ CLOSED 2026-09-04 (Wave 25)** | **The most common failure in the game had exactly one door, and the player did not hold the key to it.** `PowerGrid.repair_component` — the only function in the project that brings a FAILED grid component back into service — had **one caller**: `CityIncidentWorld.power_restore_component`, inside doc 06's incident resolution. There was no command, so there was no verb, so `ui/power_actions.gd` (the file whose whole job is doc 04 §4's operating verbs) shipped `fix`, `upgrade` and `demolish` and no repair. A player looking at a burned-out transformer could upgrade it (refused, `E_STATE` — "repair it first", with nothing that could), demolish it and pay to place another, or wait for a dispatch. **Why nothing caught it:** every test of the repair path drove it through `IncidentSystem._resolve`, which is a real path and a green one; no test asked whether a PLAYER could reach it, and doc 04 §4's own verb list was written before the pick seam existed to reach a component with. Closed by report 98 §68 RR-206 — `CitySim.cmd_repair_grid_component`, doc 03 §2.5-priced, dispatching a crew through doc 09's queue. Doc 93 §AY1. |
| **A91-D-130** | ~~**Medium**~~ **✅ CLOSED 2026-09-04 (Wave 25)** | **A number authored three times, published on every failure event, and read by nothing — the A91-D-19 shape, in the middle of doc 04.** `PowerGrid._fail(id, cause, damage_fraction)` put `damage_fraction` on every `PowerComponentFailed`: 0.35 for a transformer, 0.05 for a feeder, 0.30 for a substation, 0.05 for a line. The values were written as literals in `_pass_c_thermal`'s two failure arms and re-derived a **third** time by a ternary inside `_resolve_lightning_with`. Nothing consumed the key anywhere in `sim/`, `ui/`, `game/` or `data/`: `_fail` does not touch `condition`, `repair_component` lifted to a flat 0.85 whatever the damage had been, and `data/ui.json`'s and `data/notifications.json`'s `PowerComponentFailed` rows bind `component` and `cause` only. So *how badly a transformer burned out* was measured, announced and discarded, and a lightning strike that "destroyed" a unit (`damage_fraction` 1.0) left it in exactly the state a hazard roll at 0.35 did. **Why nothing caught it:** `tests/test_event_matrix.gd` checks that every event is consumed or classified — an event, not a FIELD, and `PowerComponentFailed` has a consumer. A per-field consumer check is the general form and is not cheap. Closed by RR-205 (`PowerGrid.FAILURE_DAMAGE`, one table, all three sites reading it) and RR-206 (`damage_fraction(id)` is the price basis, so the number that says how badly it burned is the number the player is charged to put it right). Values unchanged — all four `profile_sim` baselines byte-identical. Doc 93 §AY1. |
| **A91-D-131** | **Low** — OPEN, **for a doc 03 lane** | **Doc 03 §2.5 prices a repair for a grid component whose capital it cannot compute.** §2.5's grid bullet reads *"grid components — their §2.13(b) build cost at the current level"*, and `CostCurves.capital_value_grid(component, level)` implements it against `data/economy.json`'s `expenses.grid_components[<kind>].build_cost` array. **A feeder has no such array**: §2.13(b) prices a line **per tile of its run** (`cost_per_tile_overhead`, `× 2.6` underground), so `capital_value_grid("feeder", 1)` answers **0** and any repair priced through it is free. Latent until Wave 25, which is the first code to ask the question — and RR-206 answers it by refusing feeders (`REPAIRABLE_GRID_KINDS` is transformers only) rather than shipping a $0 purchase. **What would close it:** a route-length-aware capital accessor — `capital_value_grid_line(kind, conductor_class, tiles, underground)` — plus §2.5 text saying which of the two capitals a line takes, after which `REPAIRABLE_GRID_KINDS` gains `feeder` and the panel gains a repair on the feeder hop. It is Low because doc 06's dispatch already resolves a failed feeder and no player-facing path is blocked; it is filed rather than fixed because it is a doc 03 authorship decision (does a feeder repair price the whole run or the faulted span?) and this wave holds no dollar. Doc 93 §AY1, report 98 §68 RR-206. |

### Wave 26 — the utility planner (report 98 §70)

#### New rows, Wave 26 (2026-09-05) — `A91-D-137`, `A91-D-138`, `A91-D-139` (the door nobody could open)

| # | Severity | Defect |
|---|---|---|
| **A91-D-137** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 26)** | **A harness door that was dead on its first line for three waves, silently, and it is why gate 21 went red on main.** `Api.upgrade_grid_component` shipped in Wave 22 as the second answer to a power refusal — *"upgrade the transformer that is short rather than parallel it"* — and `cmd_upgrade_grid_component` was never added to `Playtest.KNOWN_VERBS`. `Api.verbs` is built from that array and from nothing else, so `has_verb` answered **false** every time, the door returned `E_NO_VERB` on its first line, and **both of its early returns bypassed `_log`** — so a run that called it and was refused produced an action log identical to a run that never called it. Measured at the fork (`tools/probe_utility_wall.gd`, seed 1337, game-day 25): `Curriculum._relieve` returns false, the action log grows by **0 entries**, `_relief_hour` sits at 430 with the cooldown 169 game-hours expired, and `sim.cmd_upgrade_grid_component("PT-053", true)` — the purchase being refused — previews **OK at $6,900** for doc 04's 400 → 1,000 kW rung. **Why nothing caught it:** `test_verb_probe_reports_the_live_command_layer` walks `KNOWN_VERBS` and asks whether each entry is live, which says nothing about a call site the list has never heard of; and every failure mode of the bug looks exactly like an idle agent. **Fixed** by listing the verb, by logging every exit (the harness no longer overwrites the command's own reason code with `E_BLOCKED`), and by `test_every_command_the_harness_drives_is_on_the_verb_roster`, which reads `tools/playtest.gd`'s own source for `sim.cmd_*` call sites and asserts each is on the roster — the check that walks the other way. Report 98 §70 RR-213, doc 92 §67.1. |
| **A91-D-138** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 26)** | **`cmd_place_building("water_facility")` built a water works that supplied nothing, and doc 04's identical hole was closed in Wave 6.** `cmd_place_water_component` says of the shell, the doc-05 node and the lateral that they *"always go together and have never been separable in this project"* — and this door separated them: it stamped a 3×3 `water_facility` that decays, is billed doc 03's `water_works` staffing, draws 60 kW through `_water_kw_by_building`'s fallback and hosts no node at all. Measured: `api.place("water_facility")` returns `ok`, `sim.water.nodes.size()` goes **22 → 22**. The second half was the UPGRADE: `_commission_grid_node` has re-rated a substation's grid component on the shell's own upgrade since doc 92 §17.3 fix 2, and `_commission_water_nodes` — written from the same paragraph, on the same seam, one document over — only ever brought a node ONLINE, so doc 09 §2.14.2's `l7_water_facility` asked the player for a purchase that bought a taller building and no water. **Why nothing caught it:** both shipped cities author `WTR-1` and `WTR-2` with their nodes already attached, and no strategy in `tools/playtest.gd` had ever called the building door on this archetype — the empty shell was reachable only from the build sheet, where the ghost ran doc 02's five checks and the confirm ran doc 05's twelve. **Fixed** by delegation (placement), `_rerate_water_nodes` (upgrade), `water_shell_top_level` (the shell may not out-climb the nodes it is) and `water_shell_node_delta_kw` (the power gate asks about the nodes the tick bills). Ruling 93 §BA, doc 12 D-120, report 98 §70 RR-215. **The shape to look for elsewhere:** a fix applied to one document's node-shell and not to the other's, where the two shells were written from the same paragraph. |
| **A91-D-139** | **Medium** — OPEN, **for the lane that next touches doc 02's upgrade gate** | **A zero-water-delta upgrade is refused when the building stands in no pressure zone — A91-D-53's exact shape, one document over, never written.** `CitySim.cmd_upgrade_building` reads `delta_water` off doc 02's own column, which is **0.0** for a `substation` and a `power_facility`, and then asks `WaterSystem.can_upgrade_water(sim_id, 0.0)`, whose first line is *"if `z == null or z.dead`: BLOCKED_WATER_CAPACITY"*. A grid shell outside every pressure zone therefore carries `E_WATER_HEADROOM` for the rest of its life. Measured on the Wave-26 arc at 45 game-days (`tools/measure_utility_plan.gd --days=45 --seeds=9001,1337`), on both seeds: `power_facility P-158 E_WATER_HEADROOM $69,000 | zone (none) press 0.00`, `substation P-205 E_WATER_HEADROOM $17,250 | zone (none) press 0.00`. **The twin was already fixed and its reasoning is still on the page**: `PowerGrid.can_upgrade_power` opens with *"A zero-delta upgrade needs no headroom (Wave 17, A91-D-53) … A building that adds no load cannot overload anything"*, written because doc 02's L1→L2 job on `SUB-A` — the job doc 04 §2.2 sells as 6,000 → 14,000 kW and the third feeder slot — could not be bought at all until it was. **Why nothing caught it:** doc 09 sites both authored plants inside the founding city's water zone, so the refusal needs a shell placed on ground the mains do not reach, which only a 45-game-day agent arc produces. **Why Wave 26 filed it rather than fixing it:** the lane already carried two sim changes (doc 93 §BA), the fix is hash-affecting and would land unmeasured at the end of a wave whose whole job was a measurement, and it is latent rather than blocking — `l7_power_facility` and `l7_substation` complete on both seeds that reach the capstone, through instances that do stand in a zone. **The fix, and it is four lines:** `can_upgrade_water` returns ok for `delta_water_m3h <= 0.0` before it looks for a zone, exactly as `can_upgrade_power` does for `delta_kw <= 0.0`. Doc 92 §67.9 item 1. |

### Wave 27 — the work moving (report 98 §71)

#### New rows, Wave 27 (2026-09-05) — `A91-D-140`, `A91-D-141`, `A91-D-142` (a still that said so in its own docstring)

| # | Severity | Defect |
|---|---|---|
| **A91-D-140** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 27)** | **The land-works layer named its own defect in its own docstring and shipped anyway.** `game/render/land_works_view.gd` went out in Wave 25 with the line *"No sim clock: this layer animates nothing — it re-reads its own active set at 4 Hz and re-uploads only when something moved"*, and `game/main.gd`'s call site carried the same sentence as a comment. The player found it on the first play of the merged build: *"The animation you have — I see it's not bad — but we need to actually show MOVEMENT over there."* A block being dug out was a still life whose props were swapped between polls: forty scrub clumps became twenty with nothing on the block to have removed them, and the heavy plant the phase called for stood at the FRONTAGE and never crossed the lot. **Why nothing caught it:** every assertion in `tests/test_land_works_view.gd` is a COUNT at a fixed progress, and a count is exactly what a still gets right — the file could not have failed on this and did not. **Fixed** by `game/render/land_motion.gd` + `land_motion_view.gd` (doc 11 §2.19, ruling doc 93 §BB): two dozers sweeping the block in strips with the brush going where they have been, an excavator and a haul lorry on GRADING, a paver and a roller laying the base behind them, a trencher cutting the corridor, a kerb machine, and hi-vis crews. **The gate that would have caught it, and now does:** `tests/test_land_motion.gd::test_every_working_phase_moves_between_three_game_minutes` reads the same phase at `t`, `t + 15` and `t + 45` game-minutes and requires something to be in a different place; measured largest travel per 15–30 game-minutes is **43.976 m (CLEARING) / 50.961 (GRADING) / 66.338 (ROAD_INSTALL) / 7.000 (UTILITY) / 66.338 (FINAL)** against 0.000 at the fork. **The shape to look for elsewhere:** a docstring that states a limitation in the present tense is a defect row that was written and never filed. |
| **A91-D-141** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 27)** | **`tools/land_works_preview.gd` published six per-phase screenshots that were, intermittently, six photographs of SURVEY.** The harness sets a block's phase and progress directly — it must, because a real pipeline never sits still at 0.55 — and it also bound `DevelopmentController`, so `LandWorksView._poll` (the ONLY writer of phase and progress in a running city, correctly) fired every fifteenth frame, found a block whose pipeline had not started a job yet, and put the site back to whatever the sim said. Measured: `survey.png` and `clearing.png` from a clean Wave-25-shaped run differ by **2 pixels out of 921,600** (`PIL.ImageChops.difference`, threshold 8). **Why nothing caught it:** the `--census` table is printed from `census()` immediately after `refresh()` and was always right — it is the IMAGE that lagged, and nothing compared two images. **Fixed** by binding `world` and passing `null` for the controller and the queue, so the poll returns on its first line and the harness is the only writer; the pipeline is still bought through the real command, so the block being photographed is one the game would really develop. **The shape to look for elsewhere:** any preview harness that forces state a live poll also owns. |
| **A91-D-142** | **Low** — OPEN, **for the lane that next touches doc 10's block template** | **The paver repositions across the block once per ROAD_INSTALL, because doc 10's six template runs are not one traversal.** `LandMotion.pave_state` walks a TRAVEL leg between each pair of runs — so the machine drives the join and the strip correctly stops growing while it does — but `RoadNetwork.TEMPLATE_*`'s two collector lines are at right angles to each other, so the leg from the end of run 4 (the east-west collector, ending at the block's east edge) to the start of run 5 (the north-south collector, starting at the block's south edge) is a **90 m diagonal across ground the machine has just paved**. It reads as a machine repositioning rather than as a bug, and it happens once per phase against a phase of game-hours. **Why it is filed rather than fixed:** the fix is an ORDER on doc 10's template runs — a traversal that ends each run where the next begins — and doc 10 owns that order for the stamp as well as for the render; changing it here would put the base down in a different sequence than the street it becomes. Doc 11 §2.19's open q1. |
### Wave 28 — every building fits the transformer envelope (report 98 §72)

#### New rows, Wave 28 (2026-09-05) — `A91-D-143`, `A91-D-144` (a load no transformer could carry)

| # | Severity | Defect |
|---|---|---|
| **A91-D-143** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **Nine of doc 02's sixty-six published `power_demand_kw` cells named a load that NO transformer in the game could carry, and the roster had shipped that way since doc 02 §2.14 added a sixth rung of demand to a five-rung ladder of copper.** A building attaches to exactly one transformer (`PowerGrid._attachments` is a `building_id → transformer id` map; doc 04 §2.1's rule picks one), so the largest load the distribution model can serve is one rung at doc 04 §5.3's own 0.90 ceiling — **2,250 kW** at the fork. Measured at each archetype's own doc 01 channel peak, which is the hour §5.3's gate is judged at (RR-120): `apartment` L6 **2,832 kW**, `office` L6 **4,666**, `high_rise` L5 **5,563** and L6 **14,162**, `data_center` L3 **2,600** / L4 **6,630** / L5 **16,900** / L6 **43,150**, `water_facility` L5 **2,484** — which doc 05's own per-variant table repeats as `pump` L5, the only one of its thirty rows that was over. Reported by the player from the device: *"a fully loaded data center still pulls too much, and I haven't even upgraded it past level two"* — and at the fork a `data_center` **L1** already needed doc 04's rung **4**, a $6,900 transformer before the first server was switched on. **Why nothing caught it:** every table was internally correct and no document, test or tool compared them. Doc 02 §7's `test_curve_consistency` regenerates the demand column from its own seeds; doc 04's gate 18c pins the capacity ladder against its own literals; `test_upgrade_blocked_power_headroom` proves a refusal is *raised*, never that it is *answerable*. TWO of the nine are invisible without the PEAK column, which no document had ever multiplied out against the ladder — `apartment` L6 at 1,940 kW and `water_facility` L5 at 2,160 kW both look comfortably inside a 2,250 kW ceiling and both are not (2,832 at 20:00; 2,484 at the civic peak). The other seven are over even at base, which is the worse half: they were checkable by reading two published tables side by side, and nobody did. And neither shipped city holds an offending building: the benchmark's tallest tower is L4 and its 15 data centres are all L1–L2, so no hash, no gate and no soak could see it. **Fixed** by doc 04 §2.2's sixth rung (6,750 kW = `0.90 × FEEDER_CAPACITY[2]`, the last rung any conductor class can carry) *and* doc 02 §2.3's `min(curve, ceiling)` clamp with the `data_center` seed re-derived 400 → 100 — neither alone reaches, and doc 93 §BC-2 shows why. **Gated** by `tests/test_balance_gates.gd::test_gate_34_every_building_fits_the_transformer_envelope` (all four clauses, plus doc 05's thirty per-variant rows, plus the `service_envelope` mirror re-derived from `PowerGrid` / `CitySim.DEMAND_CLASS_CHANNEL` / `DayCurveSet`), by `tools/gen_buildings.py` at write time and by `BuildingCatalog._check_service_envelope` at load time. Ruling doc 93 §BC, measured doc 92 §68, report 98 §72 RR-221/RR-222. **The shape to look for elsewhere:** two documents each publishing a ladder that indexes the same physical quantity, where one of them grew a rung and the other did not — the coverage columns of that very same sixth rung got this ruling (doc 93 §G5 rider 1) and the power column did not. **REOPENED AND RE-CLOSED in the fix pass:** the first cut of §BC-1 compared the STEADY-STATE reading (`cell × channel_peak`) to the envelope, and `CitySim.cmd_upgrade_building` compares `pad_peak + 1.15 × delta` — so `data_center` L5→L6 shipped at 6,346 kW against a 6,075 kW envelope with the gate green, and the last upgrade of a data centre staffed above 93.6 % was unbuyable at any price. The clamp is now floored on whichever of the two quotients is tighter, which moved one cell (L6 6,070 → 5,830), and gate 34 asserts BOTH inequalities for every cell and every step. **The shape to look for elsewhere, second reading:** a gate that states the rule the DESIGN intends while the command states the rule the PLAYER meets — the two differ by whatever safety margin the command applies, and a margin is exactly the kind of thing a gate forgets. |
| **A91-D-144** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **The fix router told the player the purchase would not work and never what would.** `CitySim._power_fix_plan` quotes the NEXT transformer rung and only the next, because `cmd_upgrade_grid_component` moves one rung per call — correct, and it returns `clears: false` when that rung is not enough. Measured on a `data_center` L2 on an L1 pad (`tools/measure_envelope.gd --panel`, founding city, seed 1337): `action=upgrade_transformer to_level=2 cost=1100 clears=false`, and nothing anywhere on any surface said that the load actually wanted **rung 4**. A player reading that row can only tap it, be refused, and tap it again. The building panel's POWER row named the transformer and its ratio; S18 listed the customers and their draw; neither could answer *"how big does this pad have to be?"*, which is the only question a `POWER_CAPACITY` blocker is really asking. **Why nothing caught it:** the row was honest and complete about the purchase it was quoting, so every test of that quote passed; the missing sentence was about a purchase nobody had asked the code to name. This is the A91-D-19 shape with the arrow reversed — not an event with no consumer, but a consumer with no event. **Fixed** by `PowerActions.rung_needed` (static, so `ui/fix_router.gd` reads the arithmetic rather than a second copy): the smallest rung whose nameplate carries the host's post-upgrade PEAK load, with `needs_rung == 0` kept as its own answer rather than clamped to the top rung. Three surfaces read it — the building panel's POWER row, S18's customer block (drawn in every state, because silence reads as "the panel does not know"), and the router's `FIX_POWER` answer beside its quote. Doc 12 D-123, doc 93 §BC-3, report 98 §72 RR-223, measured doc 92 §68.3. **REOPENED AND RE-CLOSED in the fix pass, for two faults of exactly the two shapes this row is about.** (i) *A consumer with no event, again:* `armed["needs"]` was written inside `if with_quote:` and both production callers pass `false`, so the third of the "three surfaces" reached none — the sentence was authored, tested, documented and unreachable. It is unconditional now and `UIRoot._serve_transformer_fix` hands it to S18. (ii) *A sentence that lied in a legal state:* `needs_rung == 0` was given the words *"nothing on the ladder feeds it"*, but it only ever meant "no rung under THIS pad" — and doc 04 §2.9's parallel transformer, which `cmd_fix_power_capacity` already sells, is the answer. Reproduced on a top-rung pad at 6,150 kW (`r = 0.911`, un-shed, legal): the panel said give up while the fix verb quoted a $42,160 purchase that cleared. `rung_needed` now separates `needs_second` from `no_rung_carries` using `alone_rung`. **The shape to look for elsewhere:** a helper that answers about the COMPONENT while the sentence it feeds is about the BUILDING — the two agree until the component is shared. |
### Wave 28 — the whole water system, audited (report 98 §73)

#### New rows, Wave 28 (2026-09-05) — `A91-D-145`..`A91-D-149` (the break nobody could clear)

*Lane B. Rulings doc 93 §BD. Resolutions report 98 §73 RR-224..RR-228. Measured
doc 92 §69, on `tools/measure_water_chain.gd` and on the player's own save.
Surfaces doc 12 D-124..D-126. Doc 05 amendments in its §10.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-145** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **A water main break that nobody answered left a permanent, un-clearable pressure penalty on a whole city — and doc 05 §2.8 already said it should not.** §2.8 rules that doc 06's tiered `zone_pressure_delta` *"is held until the incident resolves"*, and in the same paragraph that doc 05's own `break_pressure_penalty_fallback` fires *"only for a broken segment with **no owning doc-06 incident**"*. Between those two sentences is a lifecycle with THREE exits — RESOLVED, FAILED, ABANDONED — and only the first ever reached `set_segment_repaired`. `IncidentSystem._run_fail` and `_abandon_unanswered` set the terminal status, run `on_fail`'s actions (which for `water_main_break` include one last `zone_pressure_delta: −0.8`), release the units and erase the incident from `_active` — and never tell doc 05. `WaterEdge.owning_incident` then names an incident that is in nobody's roster, `_solve_zone` reads its held 0.80 instead of the fallback, and nothing in the game can ever clear either. **Measured on the player's own slot** (`tools/measure_water_chain.gd --saves=… --slot=0`): **three** mains — `LAT_B_3_3`, `M_NORTH`, `M_SOUTH` — broken at severity 1.00, naming `incident:1811` / `incident:2171` / `incident:2045`, against **zero** `water_main_break` rows in `incidents.snapshot()`. Three held 0.80s sum to 2.40 and clamp to §2.8's `break_penalty_cap`, so the zone sat at **P = 1.00 − 0.50 = 0.50** for as long as the city ran, **89 of 89 buildings under doc 02's 0.55 upgrade gate**, with **31.3 m³/h of supply spare**. They also leaked **40.1 m³/h** — **95 % of that city's entire water demand** — and doc 06's own adapter carries the comment *"so a break that FAILS cannot leave a zone permanently depressurised with nothing left to clear it"*, which was true of nothing. **Why nothing caught it:** every water test drives a break to RESOLUTION, because resolution is the path with a consequence to assert; the two terminal paths that skip the seam are the paths where nothing happens, and "nothing happened" is what a city with a permanently broken main looks like. **Fixed** in `IncidentSystem._release_finished_units` — the one loop all three exits pass through, so the next terminal path cannot forget it — plus `WaterSystem.release_segment_incident` (doc 05's own verb for "the claim is dropped, the break is not") and `tests/test_water_chain.gd`. Report 98 §73 RR-225, doc 92 §69.1. **The shape to look for elsewhere:** a cross-document hold released on ONE of a lifecycle's exits, in a doc that names only that exit. |
| **A91-D-146** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **Every save already written carries A91-D-145's orphan, and the two halves of the claim are in different save sections.** `WaterEdge.owning_incident` is persisted in the `water` section; the incidents are persisted two sections later, and `_restore_water` runs before `_restore_incidents`. So a city saved while a break was open, whose incident then went terminal, restores with a segment naming an id no roster can resolve — and RR-225's fix only stops NEW ones being made. **Measured on the player's slot: three of three.** **Fixed** by `CitySim._reconcile_water_incident_holds`, called at the end of `_restore_incidents` because that is the only line in the load where both halves are in memory at once. It invents nothing: it drops a reference the roster cannot resolve, and leaves the break, the severity and the leak exactly as the save wrote them. **On the player's save the reconciliation alone** moves zone pressure **0.50 → 0.64**, `water_health_pct` **83 % → 100 %**, and buildings under the upgrade gate **89 of 89 → 46 of 89**. No new event: the player-visible change is the zone's pressure, which the water overlay already draws, and this project counts a renderer as a consumer — an event nothing read would be the defect this wave was auditing for. Report 98 §73 RR-225. |
| **A91-D-147** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **One refusal code, two opposite remedies, and the row gave the wrong one to both.** Doc 05 §2.11 refuses an upgrade on EITHER arm behind `E_WATER_HEADROOM`: the zone has no spare water, or the BUILDING's own tile is under `upgrade_min_pressure` because §2.3's factor is distance to a main. `can_upgrade_water` returned `{ok, reason, deficit_m3h}` and `deficit_m3h` is **0.0** on the pressure arm, so no caller could tell them apart — and the shipped remedy string said *"Add a pumping station or a storage tank in this district, or raise an existing one a level."* That sentence is wrong on the pressure arm (no amount of supply moves a tile factor — doc 92 §67.8 measured a zone at pressure **1.00** with **57.1 m³/h** spare refusing a high-rise at its tile's **0.50**) and wrong on the capacity arm too whenever the binding stage is not the pump (doc 92 §67.4 measured 118 pumps and **$5.5M** bought against a treatment-bound zone whose supply did not move). **Fixed** by `can_upgrade_water` publishing `limit` ∈ {`none`, `no_zone`, `capacity`, `pressure`} with the zone key, the tile pressure and the tile's distance to the nearest main; by `WaterSystem.supply_chain` naming the binding stage; by four arm-specific remedy strings; and by `BuildController._water_headroom_params` routing `FIX_COMPONENT` at the binding node (which opens S19) on the capacity arm and `FIX_BUILDING` on the pressure arm, instead of `FIX_DISTRICT` at the zone key for both. Report 98 §73 RR-226, doc 12 D-126. |
| **A91-D-148** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **Doc 03 published the price of a water repair, doc 05 published its `damage_fraction` and its work content, and no verb in the game ever spent it.** `CostCurves.capital_value_water_main`, `capital_value_water_component` and `water_demolition_refund` are authored in `sim/economy/cost_curves.gd`, documented in doc 03 §8, derived cell by cell in that block's own `_main_derivation` — and `grep -rn capital_value_water` over the whole tree returned the definitions and **nothing else**. **CORRECTION (Wave 28 fix pass): this row closes on TWO of those three, not three.** `capital_value_water_main` and `capital_value_water_component` have a reader in `cmd_repair_water_asset` and an assertion in `tests/test_water_chain.gd`; **`water_demolition_refund` still has none** — `WaterActions.remove_quote` rides `cmd_demolish_building`'s own `refund`, which is `demolition_refund_building`, and quoting a second, different number beside the one actually paid would be worse than quoting none. It is a doc 03 §8 tidy-up (a third spelling of a fraction `demolition_refund_building` already applies to the same $45,000 `water_plant` anchor), not a doc 05 gap, and it is carried forward as such rather than counted closed here. Doc 05 §2.12's whole ownership split (*"this doc supplies only `damage_fraction`"*) had a producer on both sides of the seam and no consumer in the middle. The consequence is A91-D-145's: a broken main had no door at all — doc 06's crews are its only repair path, and a break whose incident has gone terminal is on no drawer, in no queue and in nobody's plan. **Fixed** by `CitySim.cmd_repair_water_asset(asset_id, preview)`, which is `cmd_repair_grid_component`'s body one document over: doc 05's `damage_fraction` and work content, doc 03's `capital_value × damage × 0.85 × M_repair`, doc 02's construction queue, the same `&"repair"` ledger category and the same `E_NOT_DAMAGED` floor of wear. **Measured on the player's save: $28,200 buys all three mains** — `$1,945` + `$12,641` + `$13,614`, against a treasury of **$14,899,376** — and the zone goes to pressure **1.00**, demand **42.1 → 2.2 m³/h** (95 % of it was the leak), headroom **31.3 → 71.0**, buildings under the upgrade gate **46 → 6**. Report 98 §73 RR-224, doc 92 §69.2. |
| **A91-D-149** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 28)** | **A pump that is not running kept its slice of the treated water, and the pumps that could have moved it never saw it — a third of a grown city's chain.** §2.5 splits `upstream_cap = min(Σ source, Σ treatment)` among a zone's pumps *"in proportion to `rated_flow_m3h`"*, and `WaterTopology._resolve_supply_chain` computes that split on TOPOLOGY change while `_solve_zone` decides whether a pump runs on every TICK, off doc 04's power fraction and §2.6's restart lockout. A pump that is dark, tripped or in its five-minute lockout is `is_live()`, so it kept `share_m3h` and that share was simply not delivered. **Measured** (`tools/measure_water_chain.gd --strategy=curriculum --seeds=1337 --days=25`, the only sim delta between the two runs): at game-day 25 the level-7 zone read `source 257.3 / treatment 385.9 / pumps rated 480.0 / feed_capacity 214.0` and supplied **128.6 against a demand of 135.3** — **a zone in deficit with 105.5 m³/h of its own treated water stranded**, and the old chain read named `mains 214.0` as the binder while the mains were carrying 60 % of what they could. (**CORRECTION, Wave 28 fix pass:** this row first published *85.4 m³/h stranded*. 85.4 is the SUPPLY GAIN, `214.0 − 128.6`; the stranded figure is **105.5**, and the two differ because part of the stranded water was also capped by the running pumps' own `rated·power·condition` term. Re-measured with BOTH re-split sites reverted — reverting only `_solve_zone` leaves the READ on the running-set split and prints `stranded 0.0`, which is what makes this easy to get wrong. Doc 92 §69.4 carries the full re-measurement and the exact commands.) **Why it was nearly invisible:** the loss is exactly zero whenever every live pump is running, which is true of both shipped cities at every tick of both `profile_sim` baselines, true of the founding city, and true of the player's own save to within **0.1 m³/h** (his two pumps are the same size, so the split is even). It needs a city with pumps of different sizes and one of them dark — which is what a player builds and what the curriculum agent builds by game-day 15. **Fixed** in `WaterSystem._solve_zone`: the same upstream capacity, re-split over the pumps that are actually RUNNING. Supply at day 25 goes **128.6 → 214.0 (+66.4 %)**, headroom **0.0 → 68.4**, the deficit is gone, and the chain's binder reads `mains 214.0` honestly because supply now IS 214.0. **All four `profile_sim` baselines are bit-identical**, by the argument above and by measurement. `WaterSystem.supply_chain` keeps publishing `stranded_m3h` — now normally zero — so the next regression of this shape is a number on a table rather than a difference nobody computes. Doc 92 §69.4, doc 05 §10's Wave-28 row. **The shape to look for elsewhere:** a quantity split on one clock and consumed on a faster one. |

#### New rows, Wave 28 fix pass (2026-09-05) — `A91-D-150`..`A91-D-153` (what the verifier found, and what step one found under it)

*Lane B's adversarial verifier sent the wave back `fix_first` on five blockers.
Four of them are rows here; the fifth is a number, corrected in place in
A91-D-149 below and in doc 92 §69.4. The last two rows are not the verifier's —
they are what driving the player's own first two sentences on his own save turned
up underneath them, and they are the most expensive things in this block.*

*Rulings doc 93 §BD7–§BD9. Resolutions report 98 §73 RR-229..RR-234. Measured
doc 92 §69.9–§69.12, on `tests/fixtures/player_save_0903/` — the player's own
device slot, committed, so every number below can be re-run.*

| # | Severity | Defect |
|---|---|---|
| **A91-D-150** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 28 fix)** | **Three of Wave 28's own shipped surfaces named a reader that did not exist — the A91-D-19 shape, committed by the wave that was auditing for it.** (a) `WaterPanelModel.building_row()`, 45 lines, string-backed and tested, had NO caller outside its own test: `ui/building_panel.gd` was not in the branch diff at all, so doc 12 D-125's *"the building panel gains a one-row WATER summary beside D-115's POWER row"* described a row that was never drawn. (b) `data/ui.json`'s `water_asset_repaired` row carried a `_comment` promising *"a second reader in data/notifications.json … which is what puts a banner in front of a player who is looking somewhere else"*, and `data/notifications.json` had neither a binding nor an event row, so `NotificationRouter.rule_for()` never matched and no push ever fired. (c) Doc 12 §2.26 asserted *"the served list caps at `data/ui.json.water.max_customer_rows` (8)"* and `data/ui.json` had no `water` section at all, so `WaterPanelModel.section()` returned `{}` on every call and both caps lived only as code constants — the exact asymmetry with S18, which does have `data/ui.json.transformer.max_customer_rows`. (d) The same §2.26 promised the advice line names *"what would bind after it"* and `advice_of` never read `next_binding`, which `WaterSystem.supply_chain` had been publishing since the wave opened. **Why nothing caught it:** `tests/test_event_matrix.gd` accepts a `data/ui.json.event_log` row as a consumer, which (b) had; a `static` method with a passing unit test looks consumed to every grep that counts call sites including tests; and a `UIConfig.get_num` with a default silently succeeds when the section is missing. **Fixed**: `ui/building_panel.gd` draws the row (`_render_water_row`, `water_row_opened` → `UIRoot._on_water_row_opened` → S19); `data/notifications.json` gains the `water_asset_repaired` event row and binding; `data/ui.json` gains its `water` section; `WaterActions.node_block` publishes `next_binding` and `advice_of` reads it into `ui_water_advice_binds_next`. Report 98 §73 RR-229..RR-232. **The shape to look for elsewhere:** a `_comment` in a data file that names its own second reader — it is a claim, and nothing verifies it. |
| **A91-D-151** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 28 fix)** | **The water placement ghost said `valid` and the command answered `E_AUSTERITY`.** Doc 12 §2.7 is explicit that the preview and the commit are one code path, and doc 05's two placement verbs quoted money with `treasury.balance < cost` — which is not the whole of doc 03 §2.10. Layer 2 refuses every `construction` spend outright while austerity is active, whatever the balance is, and `Treasury.spend` checks that first. **Measured on the player's own save**, which restores with `austerity_active = true` at a balance of **$14,899,376**: `PathTool.enter("water_main_service")` → an 8-tile run → verdict `valid`, cost `$2,288`, `can_confirm` true → `commit()` → `E_AUSTERITY`. The same disagreement sat on `cmd_place_water_component`. **Fixed** in both verbs, as an arm BESIDE the balance test and not in place of it — `Treasury.can_spend` would have been tidier and is a different number, because it admits a spend down to the credit floor, which is a layer-4 decision neither verb has ever made. `cmd_repair_water_asset` deliberately gets no arm: its category is `repair`, which layer 2 does not block, because a city in austerity must still be allowed to stop a leak. **STILL OPEN, and it is not this lane's**: the same `balance < cost` quote appears at **eighteen** other command sites (`cmd_place_building`, `cmd_place_grid_component`, `cmd_upgrade_building`, `cmd_build_road`, …). Every one of them has this disagreement and fixing them is a wave of its own. **The second half, also open**: the save restores `austerity_active` verbatim and nothing re-evaluates it on load, so a city that was in austerity when it was saved is frozen until the first hourly settlement — at $14.9M, one game-hour. That is doc 03's ladder and doc 08's restore, and the lead should rule on which owns it. Report 98 §73 RR-233, doc 92 §69.10. |
| **A91-D-152** | ~~**High**~~ **✅ CLOSED 2026-09-05 (Wave 28 fix)** | **A water site that caught fire before its crew finished became a building that can never be finished, holding a doc-05 node that can never be switched on — and the player had paid for both.** `Building.is_new_build()` is `state == under_construction and level == 0`, and `complete_construction()` promotes `pending_level`. `cmd_place_building` leaves `level` at 0 for exactly that reason. `cmd_place_water_component` wrote `b.level = level` **before** `start_construction()`, so from the moment it was paid for a water site looked like an UPGRADE IN PROGRESS — and `Building.condemn_unanswered`, whose own comment says a new build must be DESTROYED rather than stranded because *"it is no longer `under_construction`, so `complete_construction` can never run"*, took the other branch and put it in `damaged` at level 1. `on_construction_completed` then refuses (`E_STATE`), `_commission_water_nodes` never runs, and the node stays `offline_manual` for the life of the city. **Measured on the player's own save**, driving his own second sentence *"put pumps on it to increase our volume"*: a pump placed at (36, 44) for **$45,858** was condemned by fire on game-day 1 (`building_condemned_by_fire`, incident 2235), landed in `damaged` at level 1 condition 0.10, and `P-094-PMP` was still `offline_manual` **thirty game-days later** — with no verb anywhere in the game to finish it. **Why nothing caught it:** it needs a fire on a water site inside its build window, which no test stages and no shipped city produces; and the failure is silent — the shell is on the map, the money is gone, and the only symptom is a chain term that never moves. **Fixed** in one line: `b.pending_level = level`. All four `profile_sim` baselines are unchanged, by measurement — neither shipped city places a water component. Report 98 §73 RR-234, doc 92 §69.11. **The shape to look for elsewhere:** two constructors for one state machine, where one of them writes a field the other deliberately leaves alone. |
| **A91-D-153** | ~~**Medium**~~ **✅ CLOSED 2026-09-05 (Wave 28 fix)** | **Doc 93 §AD3's P0, one document over and never written: a water site was never asked whether the transformer that reaches it can CARRY it.** `cmd_place_building` has asked `PowerGrid.can_serve_tile` since Wave 12 and answers `E_TRANSFORMER_FULL` as a **warning** — doc 04 §2.1 gates placement on coverage and authorises no capacity refusal — and `cmd_place_water_component`, which places the heaviest single draw in doc 05's roster (a pump house is 60 kW, a treatment plant 40), asked only `would_serve`, which is coverage and nothing else. **Measured on the player's own save**: a pump sited at (36, 44) for **$45,858** passed every check, was built, was commissioned, and then ran at `power_fraction 0.00` for a full game-week — `flow_m3h 0.0`, the zone's `pump rated` up 40 m³/h and its `pump DELIVERING` up **zero**. **Fixed**: the quote publishes `power_ok` / `deficit_kw` / `transformer` / `ratio`, `BuildController._component_verdict` turns `power_ok false` into `VERDICT_WARN` + `E_TRANSFORMER_FULL` (the formatter and the `Fix this →` are the building path's, unchanged), and `BuildController.placement_sites` sorts clean sites ahead of warned ones so the site the bar recommends is one that will actually run. **Measured after**: the same drive sites the pump at (36, 56) instead, and the zone goes `pump DELIVERING` **73.5 → 113.5 m³/h**. Report 98 §73 RR-234, doc 92 §69.11. |
| **A91-D-157** | ~~**Low**~~ **✅ CLOSED 2026-09-06 (lead, on the tablet)** | **The goals sheet's reward card named the wrong level.** `GoalsSheet._apply_reward(reward, level)` titled the card `ui_goals_reward` with `level + 1` while the card's lines were `GoalsModel.reward(level)` — so the first tutorial card read "Reaching level 2 unlocks · $1,000,000 council grant" over objectives whose completion reaches level 1 (the $1,000,000 IS level 1's grant, doc 03 §2.5a). Seen on the Tab S10+ the first time the Wave-28 build opened the sheet. Fix: the title reads `reward["level"]`, the same rung the lines were read off. `tests/test_ui_goals.gd` 25/487 green. |

#### New rows, Wave 29 (2026-09-06) — `A91-D-154`..`A91-D-156` (what reserving the lot turned up under it)

*The player asked for one thing: a building should take the ground its final form
needs, from the day it is placed. Building it turned up three separate cases of
this audit's signature shape — a document, a docstring and a data column each
describing behaviour that nothing consumed.*

| **A91-D-154** | ~~**High**~~ **✅ CLOSED 2026-09-06 (Wave 29)** | **Doc 02 §2.11's check 12 has never existed, and the footprint growth it was supposed to gate was never stamped — so a grown store's mesh stood on three tiles that were still for sale.** §2.11 authors, as check 12, *"`E_FOOTPRINT`: if `footprint(L+1) > footprint(L)`, the added tiles (extending +X/+Z from origin) are owned, developed, empty, and not road"*, and doc 02 §5 names a `test_footprint_growth_gate` that asserts it. **Neither is real**: `grep -n 'E_FOOTPRINT' sim/city_sim.gd` finds the code in placement, in the road door and in the water door and **never in `cmd_upgrade_building`**, and `grep -rn 'test_footprint_growth_gate' tests/` finds nothing at all — at this fork and at every fork before it. The other half is worse: `on_construction_completed` re-stats the building and never re-stamps the grid, and `_building_records[…]["footprint"]` is never updated either. **Measured at the fork** on the founding city's `STR-001` (49, 49), forced to L3 exactly as the completion path does it: `stats.footprint` becomes `[2, 2]`, `record.footprint` stays `(1, 1)`, and `can_place(origin + (1,0), 1×1)` returns **`true`** — so the game would happily found a second building **inside** the first one's mesh, and doc 99's PA-42 had recorded the seam without anyone measuring what it cost. **Fixed** by making the question unaskable rather than by adding the guard: a building now reserves its LOT — the footprint of its final form — at placement (doc 02 §2.3a), so an upgrade only ever fills ground it already holds. Check 12 is struck in doc 02 §2.11 and §5's test 16 is replaced by `tests/test_lot_reservation.gd`. Report 98 §74 RR-236. **The shape to look for elsewhere:** a numbered check in a doc's own ordered list, where the surrounding checks are all implemented and one is not — the list reads as a unit and nothing tests the gaps. |
| **A91-D-155** | **Low** — **OPEN, and it is not this lane's** | **`TileGrid.elev_m()` has no RENDER consumer — the sim has one, and this row said otherwise until it was checked.** Doc 09 §2.2 gives every land block an integer elevation, `TileGrid` stores 49 of them, and the class comment above the tile→world helpers reads *"Elevation is a per-block integer the render layer applies; a locator that guessed at it would put the camera underground on the two raised blocks."* **The render half is true**: `grep -rn 'elev_m(' game/ ui/` returns **nothing**, so every building, road, prop and camera in the game is placed at `y = 0` and the two raised blocks are raised only in the data. **The "anywhere in the project" half was FALSE, and this row's first draft said it** (caught by the Wave 29 verifier, 2026-09-06): doc 05's pressure model reads it. `sim/water/water_boot.gd:21` wires `system.terrain = loader.world.grid`, and `sim/water/water_topology.gd:395` tests `terrain.has_method("elev_m")` before `:407` calls `terrain.call("elev_m", …)` on **every tile of every pressure zone** to compute `elev_factor = clamp(1 - elev_penalty_per_m * (elevation - head_m), elev_factor_floor, 1)`. Elevation is therefore live arithmetic in the water system of every city, and `elev_m()` is not a dead accessor. **The struck advice**: the first draft asked the lead to rule *"whether doc 09's elevation is a live feature or should be struck"*. **Do not strike it** — deleting or stubbing `elev_m()` would silently move water pressure in every city and all four `profile_sim` baselines with it. The open question is much smaller than it looked: **should the RENDER layer honour the elevation the sim already does?** **Found by building on it**: the first cut of doc 11 §2.16a's lot dressing honoured the elevation, and its aprons rendered 12 m above the buildings they belonged to — which showed up in the preview as no apron at all, because they were out of frame. The layer now draws at grade like everything else, with one row (`rows_from_sim`'s `elev_m`) marked as where a future elevation pass arrives. Giving the render layer an elevation touches the ground mesh, the camera, the road surface and every prop layer, which is a doc 11 / doc 09 wave of its own. **The shape to look for elsewhere:** a grep scoped to the layers a lane happens to be working in, reported as a fact about the whole project. |
| **A91-D-156** | ~~**Medium**~~ **✅ CLOSED 2026-09-06 (Wave 29)** | **`data/buildings.json`'s `water_facility` footprint column is the PUMP row, and two of the four placeable doc-05 variants grow inside the shipped ceiling — invisible to every reader that consulted doc 02 alone.** The rows are correctly flagged `footprints_are_reference_variant_only` and doc 02 §2.3 names doc 05 §8 as the source, which is exactly right and exactly the trap: a caller asking the catalog *"how big is a water works"* gets `3×3` flat, and that is true only of a pump. Measured against doc 05's own component table, capped at each variant's `placeable_levels` and doc 05's `levels_4_5_enabled` (`false`): `treatment` goes **2×2 → 3×3 at L2** and `tank` goes **2×2 → 3×3 at L3**, while `source` is flat at 2×2 and `pump` flat at 3×3. So the growing set is **five archetypes, not the four doc 02 shows**, and the founding city's `WTR-2` — a **tank** — is the eighth building the lot migration expands, which doc 02's column alone would have said does not grow. Sizing a `source` from doc 02 would also have reserved the pump's 3×3 where doc 05 asks for 2×2, taking five tiles of shoreline the component never uses. **Fixed**: `CitySim.water_lot_for(variant)` reads doc 05's table per variant, `lot_of_building` routes every water shell to it, and `tests/test_lot_reservation.gd::test_water_lots_are_doc_05s_per_variant` pins all four. Report 98 §74 RR-237, doc 93 §BE4. **The shape to look for elsewhere:** a data column with a `*_reference_variant_only` flag on it — the flag is honest, and it is not machine-checkable, so every reader has to remember. |
| **A91-D-158** | ~~**Medium**~~ **✅ CLOSED 2026-09-06 (Wave 29 fix pass)** | *(Id note: this row skips the number below it. The lead had already spent that one on main `95a9c80` — the goals sheet's reward card — while this lane's fork was behind main, so this row takes the next free id rather than renumbering anything on main.)* **A preview quote whose `ok` nobody read, in front of a button — and under it, a blocker list that reported FREE ground as a blocker.** Doc 12 §2.7's rule is that a preview and a commit are one code path; `BuildController._lot_block` asked `cmd_demolish_building(neighbour, true)` for a number and then used the number without ever testing the result. **Measured on the player's own save** (`tests/fixtures/player_save_0903`, 77 of 89 buildings `destroyed`): all seven lot-locked buildings with a named neighbour named a RUIN, the row read *"clearing it refunds $0"*, and `cmd_demolish_building` answered `ok=false, E_STATE, {state: destroyed}` on every one. `cmd_salvage_building` succeeds on all seven and pays **$390** each — **$27,000** for the destroyed data centre. A second, quieter half: `CitySim._lot_blockers` classified every missing tile with `if road / elif water / elif occupied / else E_NOT_DEVELOPED` and had **no arm for buildable-and-empty**, so free ground was reported as a blocker to the panel and to `measure_lots --list`. **Fixed**: the verb is chosen by the neighbour's state, the quote's `ok` gates the door, and `_lot_blockers` spells `TileGrid.can_expand`'s own predicate so the two answers cannot disagree. Report 98 §74b RR-240. **The shape to look for elsewhere:** a `_preview` call whose return value is indexed but never tested — `grep -n 'true)\[' ` over the UI layer finds the class, and the tell is a quote used as a NUMBER rather than as an ANSWER. **The second shape:** a chain of `if/elif` classifying a state, whose `else` assumes the remaining case is a problem when the commonest remaining case is *nothing is wrong*. |

#### New rows, Wave 30 Lane A (2026-09-06) — `A91-D-159`..`A91-D-161` (what profiling the suite turned up under it)

*The lane was sent to make `tests/test_balance_gates.gd` cheaper. The first thing
it had to build was an instrument that could see the cost at all, and the
instrument named three things the lane could not fix without leaving its remit.
All three are recorded with the measurement that would justify them, so the next
lane starts from a number rather than from this row's opinion.*

| **A91-D-159** | **Low** — **OPEN, and it is deliberately not this lane's** | **`Playtest.Api.relief_spot` is the water sweep's defect, unfixed, because its cheap necessary condition lives behind a private rule.** `tools/profile_gates.gd --verbs` measures it at **55.20 s of self time over 3,685 calls (15.0 ms each)** at fork `9aecbf9` — the third most expensive agent verb in the file, behind `grid_shortfall_tile` and `place_water_component`, both of which this wave fixed. It is the identical shape: a ring of `(2r+1)²` tiles around the hottest transformer, each one paying a full `cmd_place_grid_component(preview)` even though `E_FOOTPRINT` — `world.grid.can_place(tile, size)`, ~1 µs — refuses most of them outright on a dense late-game city. **Why it is not fixed here**: the footprint the command reserves comes from `CitySim._placeable_rules(kind).footprint_tiles`, which is private, and a harness that reads `data/power.json` for it grows exactly the second, wrong copy of the rule that doc 93 §BF2 forbids and that doc 92 §67.4 says the water sweep previews to avoid. **The fix is one public accessor** — `CitySim.grid_lot_for(kind)`, the sibling of the existing `water_lot_for` — and then the same two lines the water sweep got. Doc 92 §71 has the table. **The shape to look for elsewhere:** a search that previews to avoid duplicating a rule, and therefore also pays for the rules it *could* have asked about cheaply. |
| **A91-D-160** | **Medium** — **OPEN, and it needs its own wave** | **`PowerGrid.would_serve` is O(all components) per tile, and it is the shared root under every site search in the project.** It walks `_order` — every transformer, feeder, substation and tie — testing `kind`, `state`, `tile` and `level`, and returns on the first transformer in range. Measured on the city seed 4242 reaches at 45 game-days: **66.4 µs per call**, against 0.8 µs for `TileGrid.can_place` and 15.5 µs for `WaterSystem.nearest_main_tile`. `Api.grid_shortfall_tile` makes ~800 of these calls per invocation, which is the whole of its **207.63 s / 7,333 calls** at the fork; `candidate_site`, `unserved_tiles`, `site_near`, `tools/site_search.gd`, `game/main.gd`'s build sheet and this wave's own water filter all sit on it too, so this is a **player-facing** number as well as a suite one. **The fix is a coverage memo inside `PowerGrid`** — one copy of the rule, made O(1) — landed the way doc 04 §167's warm-transformer memo was, with an oracle test that re-implements the authored scan and walks the envelope plus an uncovered margin. **Why this lane did not take it**: `would_serve` reads a component's `state`, which goes `FAILED` inside the tick, and `mutation_epoch`'s own docstring claims only *"a component added, removed, re-rated or re-conductored, a building attached or detached"* — so the memo needs an invalidation term nobody has audited, on the hottest correctness predicate in the project, in a lane whose brief is hash-neutral tests-and-tools. A stale mask is a silently wrong placement, not a slow one. **The shape to look for elsewhere:** a linear scan over a heterogeneous collection that filters to one kind — the filter is the measurement, and it is free to move. |
| **A91-D-161** | **Low** — **OPEN, and it is doc 92's question, not a performance one** | **Two thirds of `tests/test_balance_gates.gd` is the sim advancing, and nothing has ever asked whether the gates need the horizons they run.** `tools/profile_gates.gd --verbs` at the fork: instrumented agent verbs account for **491.77 s of 1,571.67 s (31.3 %)**; the other **~1,073 s is `advance_coarse_hours`**. The per-method table names where it goes — gate 29 drives `do_nothing` for **690 preset-days** (300 casual + 200 standard + 120 hard + 70 crisis) to read four insolvency days, gate 30 another **200** on `crisis`, gate 33 **60** on `balanced`, gate 21 **3,240 game-hours** of `curriculum`. **The consequence, stated so the next brief does not repeat this one's:** a harness with every agent verb reduced to zero still leaves this file at **~18 minutes**, so the 10-minute target cannot be met from `tests/` and `tools/` and no amount of memoising gets there. The question the numbers raise is whether a gate that reads *the game-hour a `do_nothing` city crosses zero* needs to keep advancing to day 300 after it has crossed — an early-out at the reading, which is a semantics question doc 92 owns and this lane may not answer. **What is NOT available**: the runs are not redundant. `_runs` already memoises `(strategy, days, seed)` within the file, gate 30's 200-day `crisis` run is a superset of gate 29's 70-day one rather than a duplicate, and `test_balance_gates.gd` is the only file in the suite that drives a 45-game-day arc — so "share the three curriculum runs across the gate files" has no second file to share with, and a static cross-file cache would be authored behaviour nothing consumes. **The shape to look for elsewhere:** a performance brief whose named suspect is real, and is a third of the number. |
| **A91-D-162** | ~~**High**~~ **✅ CLOSED 2026-09-06 (Wave 30)** | *(Id note: this row skips the three numbers below it — 159..161 belong to this wave's sibling lanes, whose pre-assigned blocks sit under this one.)* **`A91-D-19`'s shape again, and this time in the fix for it.** Wave 28 built `BuildController.placement_sites()` because the ghost is a point-wise answer to a question about a SET (doc 93 §BD8), and it works: 441 previews of the ghost's own preflight, the verified legal origins, the first-blocker histogram, the cheapest purchase that opens one. Its own doc row says `tiles` is *"the set the shell paints"*. **Nothing painted it.** `grep -rn 'site_hint\|placement_sites' game/` came back empty; `site_hint()["tiles"]` was an `Array[Vector2i]` of correct, tested, expensive answers that reached the player as a **count** — *"3 spots for this within 10 tiles"* — and left him hunting one tile at a time for three tiles the game had already found. **What that cost, measured on the founding city's shoreline:** the window finds `(35, 49)`, `(34, 49)` and `(33, 49)`, all clean, the nearest 3 tiles from the ghost at $38,086, and the only way a player could reach any of them was to drag the ghost over them by hand until it went green. **Closed** by doc 12 D-130: `ui/site_paint_model.gd` (headless, driven by `tests/test_site_paint.gd`) + `game/render/site_paint_view.gd` + `game/shaders/site_paint.gdshader` — one MultiMesh, **+1 draw call while a refused ghost is up and +0 the rest of the game** (doc 11 §2.20). **The half that makes it an answer rather than a picture** is the tile it is drawn on: `placement_sites` returns footprint ORIGINS and the shell's tap centres the footprint, so a paint on origins is wrong by `centre_offset()` for every lot bigger than 1×1 — which is all of doc 05's roster. It is drawn on `origin + centre_offset()`, and the tap that follows it is the tap `main.gd` already had. Doc 93 §BG1. **The shape to look for elsewhere:** a published field whose own documentation names a consumer in the future tense (*"the set the shell paints"*), and a `grep` for the field name that returns only the file that writes it. |
| **A91-D-163** | ~~**Medium**~~ **✅ CLOSED 2026-09-06 (Wave 30)** | **The sentence that exists to unblock a blocked player told him the thing he could not afford was free.** `BuildController._site_advice`'s `E_FUNDS` branch is reached exactly when every buyable tile in the window refuses for money — which means `legal` is empty, which means `found["cost"]`, *the cheapest LEGAL site's price*, is **0**. So the placement bar read, verbatim, on the founding city as it boots: *"A spot 3 tiles away would take this and it costs **$0**; the treasury holds $24.1K."* The true price is $38,086 and the `preview = true` verdict that raised `E_FUNDS` was carrying it the whole time, in `params.cost` — $286 above the card's own $37,800 headline, because that tile needs a one-tile lateral. **Fixed** (doc 12 D-131): the scan keeps the refused tile's own quoted cost on its buyable row and the sentence spends that. **Why no test saw it:** every Wave-28 test asserts which advice KEY was chosen and that its arguments are well-shaped, and both were right — the string table, the router and the `fix_target` were all correct. Nothing in the project read the rendered WORDS until `tools/site_paint_preview.gd` printed `placement_issue_text()` beside a screenshot. That is the same lesson as RR-242 and RR-243 and it is now three for three: **the deck of preview states is not decoration, it is the only instrument that reads the sentence a player reads.** |
| **A91-D-164** | **Low — OPEN (filed 2026-09-06, Wave 30)** | **`placement_sites` is 60.6 ms for a water card and there is no budget row anywhere that says so.** Measured, founding city, radius 10: **60.6 ms** for `source` (441 `cmd_place_water_component` previews, each of which solves a tap-distance and a transformer capacity), **66.8 ms** on `tests/fixtures/player_save_0903`, and **14.2 ms** for `house`. That is fine where it is spent — once per card, once per window, off the 10 Hz path (doc 93 §BD8) — and it is a **quarter of a second of dropped frames on a phone** the first time a player opens the water card, with no loading affordance and no measurement in doc 11 §2.13's table. Wave 30 did not make it worse (the paint adds no scan; it reads the memo) and did not fix it either. **Three cheap levers, none taken:** (a) the scan short-circuits on the first blocker per tile already, but `cmd_place_water_component` re-derives the nearest main per tile where the window could resolve mains once; (b) the window could be scanned in a ring outward and stopped at `site_rows` clean hits, which is 32 of 441 in the common case; (c) it could be spread over the frames of the card's entry animation, which is 300 ms of cover the design already pays for. Whichever is taken, the number belongs in doc 11 §2.13 beside the sim-step column, because it is the largest single-frame UI cost the project has. |
#### New rows, Wave 30 (2026-09-06) — `A91-D-165`..`A91-D-168` (what naming the water wall turned up under it)

*One brief — "the game says water headroom when the truth is too far from a main"
— and under it three more instances of this audit's signature shape: a rule
decided in three places on three predicates, a refusal that could not state its
own number, and a doc sentence nothing has ever implemented. The fourth row is
new: a price that is right and a MODEL that makes it wrong.*

| id | severity | finding |
|---|---|---|
| **A91-D-165** | ~~**Medium**~~ **✅ CLOSED 2026-09-06 (Wave 30)** | **One rule, three surfaces, three different predicates — and they could disagree with the gate and with each other.** Doc 05 §2.11's upgrade gate refuses on capacity or on §2.3's per-tile factor, and Wave 28 gave the answer a `limit` field so the UI could tell them apart. Three surfaces then each decided the REMEDY for themselves: `ui/build_controller.gd` on `limit == "pressure"` (so the fix button flew to the building), `ui/water_panel_model.gd` on its own `pressure < gate and zone.pressure >= gate` (so the row said "too far from a main"), and `ui/requirement_formatter.gd` on a third arrangement of the same two tests (so the advice string said "lay a main closer"). **None of the three was the gate's**, and they part company on a real case: a tile at **0.32 inside a zone at 0.50** is a building whose ZONE is short — `can_upgrade_water` says capacity, the panel row said *"too far from a main"*, and the formatter offered to sell the player a pipe that would have moved the pressure by nothing. **Why nothing caught it:** each predicate is two lines and each is correct on the case its own test exercises (a healthy zone), and no test in the project asked two surfaces the same question about the same tile. **Fixed:** `WaterSystem.pressure_remedy_at(tile)` decides it once, `can_upgrade_water` publishes the answer as `reason` ∈ {`BLOCKED_WATER_DISTANCE`, `BLOCKED_WATER_CAPACITY`}, and all three surfaces read it. `tests/test_water_system.gd::test_the_gate_says_a_main_when_distance_is_the_wall_and_supply_when_it_is_not` pins both directions on one city, one hour, one zone. Doc 05 §10 RR-250/RR-251, doc 93 §BH2, report 98 §77. **The shape to look for elsewhere:** a field published so that consumers can classify something, where the classification itself was never published — every consumer then writes the classifier, and the versions drift. |
| **A91-D-166** | ~~**Low**~~ **✅ CLOSED 2026-09-06 (Wave 30)** | **The one refusal whose entire content is a distance was the one that could not state a distance.** `WaterTopology.distance_at_tile` returns **−1** outside doc 05 §2.2's `max_service_distance_tiles` cutoff, because the BFS stops there and the sentinel means *not reached*. `can_upgrade_water`'s `no_zone` arm is exactly the tiles that are not reached, and `BuildController._water_headroom_params` published that −1 straight into `main_distance_tiles`, where `data/strings.en.json`'s `ui_requirement_e_water_headroom_no_zone` — *"No water main reaches this building at all"* — had no `{tiles}` slot to print it in and the `pressure` sentence would have rendered *"this building is -1 tiles from the nearest main"*. **Measured** on the Wave-29 curriculum arc at 45 game-days, seed 1337: `power_facility P-234 E_WATER_HEADROOM $69,000 | zone (none) press 0.00`, a plant standing outside every main's reach for the rest of the run with nothing on the row to say how far it had to go. **Fixed:** the arm falls back to doc 05's own whole-map `nearest_main_tile` **on the refusal path only** (O(main tiles), once per refused upgrade) — the identical read `CitySim.cmd_place_water_component` has done for its `E_NO_MAIN` sentence since Wave 10, which is why this is a re-use and not a new search. Doc 05 §10 RR-252. **The shape to look for elsewhere:** a sentinel return value (`−1`, `""`, `INF`) crossing from a cache into a *player-facing* number without an arm for the sentinel. |
| **A91-D-167** | ~~**Low**~~ **✅ CLOSED 2026-09-06 (Wave 30)** | **Doc 05 §2.11 has specified a 30-game-day rolling average for six waves and nothing has ever computed one.** The upgrade-gate bullet reads *"doc 02 refuses an upgrade if the building's **30-game-day average** `P < upgrade_min_pressure (0.55)`"*. `WaterSystem.can_upgrade_water` reads `pressure_at(tile)` live; `grep -rn "30" sim/water/` finds no window, no accumulator and no ring buffer; `WaterServiceLedger` keeps an **hourly** factor for doc 03 §5.4 and nothing longer. So the doc has been describing a gate the game does not have, and any lane that had "implemented" the sentence would have shipped a regression. **Why nothing caught it:** the discrepancy is invisible from either side — the code passes every test written against the code, and the doc reads plausibly because a 30-day average is what a utility engineer WOULD use. **Fixed by STRIKING the sentence, not by building it** (doc 93 §BH3): an averaged gate would leave the refusal standing for thirty game-days after the player laid the main that answers it, which is a door that does not open and the exact defect class doc 12 §2.7 exists to prevent. The bullet now reads *"the building's tile pressure `P_tile`"*. **No behaviour changes and no baseline moves** — this row is a claim withdrawn. **The shape to look for elsewhere:** an adjective in a design doc that implies state (*rolling*, *averaged*, *smoothed*, *since last*) with no field in `§3`'s save schema to hold it. |
| **A91-D-168** | **Medium** — **OPEN, and it is a doc 05 §2.5 re-derivation, not a price** | **§2.5's min-cut counts a feed main's WHOLE nameplate the moment the edge touches a supply node's tile, so the shortest legal trunk is a 24× arbitrage on the term that binds two of three measured cities.** `WaterTopology._resolve_supply_chain` adds `edge.capacity_m3h` to `feed_capacity_m3h` for every live edge that touches a live pump's or tank's tile, with no reference to where that edge GOES. `cmd_place_water_main` requires two tiles. So a **two-tile** trunk stub off the pump adds the full **213.0 m³/h** for doc 03 §2.13(g)'s **$1,608** — **$7.55 per m³/h**, against a pump's $45,000 for 240 m³/h, or **$187.50**. **Measured** on the founding city (`tests/test_playtest_harness.gd::test_a_trunk_laid_at_the_plant_raises_the_term_no_node_can_raise`): `feed_capacity` **160.5 → 373.5** for a three-tile run. Doc 92 §67.8 measured seeds 1337 and 4242 both ending supply-capped at exactly **214.0**, which is this term, so the arbitrage is on the binding constraint of the late game rather than on a curiosity. **Not fixed here, and deliberately not exploited:** `Balanced.MAINS_TRUNK_TILES` is **8** — `main_tap_radius_tiles`, half a land block, the run a player would draw — so the harness buys the honest purchase and this row records the artefact. **Why it is not a doc 03 fix:** the dollar is derived twice over and lands within 2.8 % of the price doc 05 itself used to carry (doc 03 §2.13(g)); moving it to hide a modelling artefact would be C-07's inversion. **The fix, for the lane that takes it:** §2.5's feed term should count a main against the length or the supply-tile count it actually serves, which moves every `feed_capacity` figure doc 92 has published and belongs in the same wave as doc 93 §BD6's deferred MVP-ladder question. Doc 93 §BH6. **The shape to look for elsewhere:** a capacity term summed per-ENTITY where the entity has a size the sum ignores. |
#### New rows, Wave 31 (2026-09-06) — `A91-D-169`..`A91-D-171` (the mesh is not this building's)
...
don't follow their grid pattern; they're just off-centre from it."** All three
rows are render-only; `sim/` is untouched and all four `profile_sim --hash-only`
baselines are byte-identical to the fork.*

| **A91-D-169** | ~~**High**~~ **✅ CLOSED 2026-09-06 (Wave 31)** | **The gray-box manifest is keyed by ARCHETYPE and carries one footprint per archetype-level, so every doc-05 water variant was drawn with the PUMP's shell — a 2×2 tank under a 3×3 building, 4 m over every edge of its own lot.** `data/buildings.json`'s `water_facility` footprint column is flagged `footprints_are_reference_variant_only` and it is the `pump` row: 3×3 flat to L4. Doc 05's own columns are not: `treatment` is 2×2 at L1, `tank` is 2×2 to L2, `source` (river) is 2×2 to L2, and `CitySim.built_of_building` builds every one of them on exactly that. The SIM was never wrong — `game/main.gd::_building_view` centres the mesh on the BUILT extent and the founding city's `WTR-2` lands at `(272, 0, 456)`, its 2×2's exact centre. The RENDER asked `manifest["water_facility:1:0"]`, got 3×3, and centred it on the same point. **Measured with `tools/probe_water_shell.gd`** on the founding city and on one of each variant placed through the real `cmd_place_water_component`: `WTR-2` (33, 56) built 2×2 lot 3×3 → mesh 3×3, overhang **+4.0 m/edge**; placed `treatment` (42, 33) **+4.0**; placed `source` (33, 49) **+4.0**; placed `pump` (56, 34) 0.0. And the same defect with its sign flipped: `treatment` L4 is built 4×4 and was drawn with a 3×3 shed, **−4.0 m**, so the fault was never "the mesh is too big". **How long:** `WTR-2` is a 2×2 tank under a 3×3 shell in `data/starter_city.json` as authored, i.e. since the founding city existed, and every placed plant since Wave 5. **Why nothing caught it:** every test that touched the manifest asked whether a mesh EXISTS for an (archetype, level); none asked whether its footprint is the one the catalogue publishes for the thing standing there. **Fixed**: the render's unit is now the SHAPE (doc 93 §BI1) — `water_facility_treatment` / `_tank` / `_source` are generated with doc 05's own footprints and picked by `ShapeCatalog.shape_of(archetype, variant)`; `pump` keeps `water_facility` because it IS doc 02's declared `reference_variant` and its meshes did not move by a millimetre. `tests/test_water_shell_shapes.gd` asserts every one of the 162 manifest rows' `footprint_tiles` against its catalogue footprint. Report 98 §78 RR-254, doc 93 §BI1/§BI2. **The shape to look for elsewhere:** a generated manifest keyed on FEWER dimensions than the thing it describes varies on — the key is a claim that nothing else matters, and A91-D-156 had already written down that this particular claim was false. |
| **A91-D-170** | ~~**Medium**~~ **✅ CLOSED 2026-09-06 (Wave 31)** | **A building that GROWS its footprint has never been re-centred, so five of doc 02's twelve archetypes have stood half a tile off their own lot from the moment they climbed.** A mesh is centred on the ground it holds (doc 93 §BE1), so a footprint growing by one tile moves the centre 4 m along both axes. `RenderStateModel.apply_events`' `building_completed` / `building_upgraded` arm called `_rebucket(rec, new_level)` — which frees the slot, sets the level and re-allocates — and **never touched `rec.transform`**. The growers are `store` (1×1 → 2×2 at L3), `construction_yard` (2×2 → 3×3 at L4), `power_facility` (3×3 → 4×4 at L4) and doc 05's `treatment` (L2) and `tank` (L3). **Found by sweeping every consumer of a footprint** rather than the one the player's report named — it is the same sentence, *"off-centre from it"*, about a much larger set of buildings. **Why nothing caught it:** the lot-dressing and construction preview decks walk a ladder by re-stating the whole view through `add_building`, which recomputes the centre, so the harness that photographs every rung of every grower could not see it; and no test drove a completion event with a level that changes footprint. **Fixed**: `game/main.gd` attaches `world_pos` and `built_tiles` to the translated completion event — only the sim can answer them and the render model must not learn doc 02's ladder — and `_rebucket` re-centres and re-scales from them. Both fields are optional, so every other producer of that event keeps the arm it had. The chunk is deliberately NOT re-derived: a 4 m move can cross a 128 m chunk line and a rec whose `chunk` disagrees with the `ChunkRec` holding it corrupts the slot allocator; re-homing is the streamer's job. Report 98 §78 RR-255, doc 93 §BI4. **The shape to look for elsewhere:** a state transition that updates the INDEX a thing is filed under and not the GEOMETRY that index implies. |
| **A91-D-171** | ~~**Medium**~~ **✅ CLOSED 2026-09-06 (Wave 31)** | **Wave 29 changed what `building_record()["footprint"]` MEANS and fixed one of its two readers in `game/main.gd`; the other is three lines below it.** `_building_view` moved to `built_of_building`; `_add_construction_site`, which fences the site and sizes its crane, kept reading the record — which is the LOT now. So the founding city's 2×2 tank was fenced with a **3×3** run of hoarding panels with one tile of it in the street, and doc 11 §2.16a's lot dressing was drawing its own compound fence on that same remainder: two layers owning one piece of ground, and neither of them owning it correctly. The crane's height came from `_height_of["water_facility:L"]`, the pump's roofline, for a tank that stands 14 m. The same key was feeding `PowerInfraFeed.building_view`'s service drop, so a tank's weatherhead landed 6 m below the wall it is supposed to reach. **Fixed**: the hoarding takes `built_of_building` and both height reads resolve the SHAPE. The ruling is doc 93 §BI5 — the hoarding fences the BUILT extent, §2.16a's apron dresses `held − built`, and between them they cover the lot exactly once. **A third reader, deliberately left alone**: `tools/lot_dressing_preview.gd::_aim` frames the LOT on purpose, because the lot is what that deck photographs. Report 98 §78 RR-255, doc 93 §BI5. **The shape to look for elsewhere:** a wave that redefines a field and greps for its readers — the grep finds them all, and a reader two lines from a fixed one reads as already handled. |
