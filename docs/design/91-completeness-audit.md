...
| # | Severity | Defect |
|---|---|---|
| **A91-D-97** | **High** ✅ **CLOSED 2026-09-02 (this wave)** | **A camera axis built to look UP could not, at any angle it had, and every test it shipped with agreed that it could.** Wave 17's manual pitch band answered the directive *"we need to be able to look up at the buildings"* with a floor of 12° and eleven tests that assert the ANGLE. None of them asserted the FRAME. In §2.16's rig the camera looks at the focus and the focus is on `y = 0`, so the horizon is always `pitch` above the view axis and lands at `(1 − tan p/tan(fov/2))/2` down the frame — **20.8 % at the 12° floor, leaving the ground 79 % of the picture at the shallowest angle the band can reach**, with the tower tops cropped off the TOP edge. The band was necessary and not sufficient, and the floor could not be lowered to fix it: `18·sin 12° = 3.74 m` is the last angle that clears doc 11 §2.6's 3.5 m ground floor, so the feature had reached the end of the only axis it had. Closed by report 98 §54 RR-151: the LOOK-AT point rises with the lean (`aim_up_ground_frac = 1/3`, pavement in the bottom third), a pure aim lift that leaves `camera_position()` and therefore every clearance, tier and cull measurement untouched. **The shape to look for elsewhere: a feature whose acceptance criterion is a picture, tested only on the number the picture is derived from.** `tests/test_camera_aim.gd::test_the_full_lean_leaves_the_ground_in_the_authored_bottom_share` is the missing test — it asserts where the horizon is DRAWN, by projecting it. |
