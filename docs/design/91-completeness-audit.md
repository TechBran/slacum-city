...
| **A91-D-43** | ~~**High**~~ **✅ CLOSED 2026-09-02 (Wave 17)** | **The city paid the upkeep of every building it does not own, and the code says so in one line.** `EconomySystem.settle_hour`'s `E_building_maint` loop skipped every row where `CostCurves.is_revenue_producing(type)` was false — C-08's no-double-billing guard, so a civic or utility shell is not charged beside its own department or O&M line. That predicate is `REVENUE_CLASSES.has(class_of(type))` and `REVENUE_CLASSES` is `["residential", "commercial", "industrial", "tech"]`, so **the set the line billed was exactly the set the city does not own** — the houses, shops, offices and towers whose owners pay it tax. Measured (`tools/measure_repair_burden.gd --days=21 --seeds=1337`): $63,915–$78,038 of upkeep on private stock over three game-weeks against $62,692–$70,053 of repair on everything the city *does* own, and on `do_nothing` the entire building-related bill ($16,585 against $0). **This is not A91-D-19's shape** — nothing here is dead or unread; every line ran, every test passed, and the defect is that a correct computation was charged to the wrong party. It survived four balance passes because the ledger row's own label, `Building upkeep`, is true of a *landlord* and the reader supplies the landlord. Closed by report 98 RR-99 / doc 93 §Y1: the line is retired, `BUILDING_MAINT_RATE` and `MAINT_CONDITION_PENALTY` are deleted rather than defaulted, private stock keeps itself up (doc 02 §2.6a), and what the city sees of that is `f_condition` on the tax line — a 4.5 % drag at the measured steady-state mean condition 0.9251. Founding gross is **bit-identical** across the change (1047.184374 both sides) and only the expense side moves, which is the evidence separating "a charge withdrawn" from "a subsidy added". |
