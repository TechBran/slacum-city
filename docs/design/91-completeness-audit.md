...
| **A91-D-123** | **Medium** — OPEN, **for the lane that owns `tools/playtest.gd`** | **The curriculum's capstone is decided by an agent that cannot plan water headroom, and the cell is a knife-edge.** Doc 92 §61.11 already recorded seed 9001 stopping at curriculum level 5 with 7,518 residents on ONE water works, `l6_tower` refused `E_WATER_HEADROOM` for the rest of the run, and ranked a `utility_planner` strategy first among its open questions. Wave 23 measured how thin that edge is: doc 93 §AV4's fire-coverage term perturbs doc 06's ignition stream, and **seed 1337 lands on the same cliff — level 7 at game-hour 430 becomes never** — while seed 4242 finishes 16 game-hours EARLIER and seed 9001 ends $423,875 richer. That is a chaotic re-roll around a knife-edge, not a systematic penalty, and the A/B is one line of data (`coverage_slope` 0.0 returns 1337 and 4242 to 430 and 377 exactly). **The wall is measured and it is neither the horizon nor money:** seed 1337 on a 75-game-day horizon still ends at level 6, holding **13,946 residents, $3,633,922 and one water works**. **Why this is filed rather than fixed:** two answers were already tried and both made the arc worse (doc 92 §61.11 — widening `power_blocked_top_rung` to answer a water refusal took the capstone from two seeds to one; letting a headroom refusal consume the agent's hour froze all three), and `_relieve` DOES answer `E_WATER_HEADROOM` by buying a pump — it is the PLANNING that is missing, not the reflex. **What it cost:** gate 21's capstone count is re-fitted 2 → 1 (doc 92 §62.9) and the cell gains the assertion the count was standing in for — a seed that does not reach the capstone must be walled by UTILITY and not by money, `treasury_end >= $500,000`, which a real balance regression fails and the old count would have passed. **What would close it:** a `utility_planner` that keeps headroom AHEAD of demand rather than answering refusals, and the floor goes back to 2 or to 3. Doc 93 §AV4, §AV5; doc 92 §62.6, §62.9. |
...
| **A91-D-139** | **Low** — OPEN, **for the lane that next re-measures doc 92's agent matrix** | **`balanced` still has no water planner, and seven published matrix rows are measured on an agent that answers water refusals instead of preventing them.** Wave 26's `_lead_water` is written in `Balanced._grow`, beside `_lead_generation`, where it belongs — and it is gated on `plans_water`, KNOB 3, which only `curriculum` sets (ruling 93 §BA4). The reason is stated and is not a technical one: doc 92 publishes seven matrix rows, a 50-game-day dark share and nine balance gates fitted to what that ladder buys, and doc 92 §62.9's merge addendum ruled that this wave ships the planner **rather than another re-fit**. Turning the purchase on for all eight agents would have moved every one of those numbers in the same commit as the planner and made the planner's own measurement unreadable underneath them. **What it costs today:** nothing the player can see — `curriculum` is the only agent gate 21 or doc 92 §22 measures the taught route on. **What would close it:** flipping `plans_water` to true by default and re-deriving §17's matrix, §18b's dark share and every gate fitted to them, as ONE lane whose whole job is that re-derivation. Doc 93 §BA4, doc 92 §67.5. |
