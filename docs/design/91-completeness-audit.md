...
| **A91-D-155** | **Low** — **OPEN, and it is not this lane's** | **`TileGrid.elev_m()` has no consumer anywhere in the project, and its own docstring says the render layer applies it.** Doc 09 §2.2 gives every land block an integer elevation, `TileGrid` stores 49 of them, and the class comment above the tile→world helpers reads *"Elevation is a per-block integer the render layer applies; a locator that guessed at it would put the camera underground on the two raised blocks."* At this fork `grep -rn 'elev_m(' game/ ui/ tools/` returns **nothing**: every building, road, prop and camera in the game is placed at `y = 0`, and the two raised blocks are raised only in the data. **Found by building on it**: the first cut of doc 11 §2.16a's lot dressing honoured the elevation, and its aprons rendered 12 m above the buildings they belonged to — which showed up in the preview as no apron at all, because they were out of frame. The layer now draws at grade like everything else, with one row (`rows_from_sim`'s `elev_m`) marked as where a future elevation pass arrives. **Not fixed here**: giving elevation a renderer is a doc 11 / doc 09 decision that touches the ground mesh, the camera, the road surface and every prop layer, and inventing it inside a lot-dressing lane would be the wrong wave making it. The lead should rule whether doc 09's elevation is a live feature or should be struck. |
