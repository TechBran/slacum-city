...
| **06 — Incidents, dispatch & emergency fleets** | the routing API of §4, exactly as doc 06 §5 requested | `route_minutes`, `congestion_index`, **`access_quality`** (doc 06's `< 0.6` degraded-response penalty), `set_edge_speed_mult`, `close_edge`, `nearest_node`, `signalised_intersections` (for `dark_frac`), plus **`condition_hazard_mult`** (which doc 06 now multiplies into `traffic_accident` per C-48), `estimate_eta_practical`, `request_path` |
