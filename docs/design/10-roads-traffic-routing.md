...
12. **NEW (Wave 12) — the auto-repair quote passes no `M_repair`.** `CitySim` wires `RoadNetwork.repair_quote` as `econ_curves.repair_cost_road(road_class, damage_fraction)` (`sim/city_sim.gd:330`), leaving C-16's difficulty multiplier at its 1.00 default. That quote is used for exactly one thing — deciding how many contiguous runs fit inside `auto_repair_daily_cap` — and §2.12 calls the cap "a **player budget setting**, not a price", so quoting it at nominal is defensible. The consequence, measured in doc 92 §31: a `crisis` city's $25,000/game-day cap admits **1.60× more tile-fractions than repairing them actually costs**, and a `casual` city's admits 0.70× fewer. Doc 03 §2.4's accrual line is now `M_repair`-only and correct on every preset (doc 93 §M1); this is the other side of the same seam and it is doc 10's call. **Either is hash-neutral on the default preset** (`M_repair = 1.00` on `standard`), so the question is what the cap is *for*, not what it costs to change.
