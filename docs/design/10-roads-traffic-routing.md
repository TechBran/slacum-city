...
**Two accessor notes, added 2026-08-20 (report RR-31), both pure optimisations with no behaviour behind them.** They are recorded here rather than left as folklore because both are on the per-tick path.
...
> **As built (Wave 14) — the feed's per-edge index is ORDER-CANONICAL, and this closes a fragility class rather than a bug.** `TrafficFeed._by_edge` maps `edge_id → [vehicle ids]`, and it used to be insertion-ordered in both halves. A shipped city produces two different insertion histories for the same feed: a **live** feed appends a car to the edge it spawns on and again to every edge it hops onto, so a per-edge list is in visit order and the key order is first-touch order; a **restored** feed rebuilds from the saved roster in ascending vehicle id, so a per-edge list is in id order and the key order is first-appearance-in-that-walk. The two disagreed on every city with a hop in it.
>
> Nothing diverged, because the one consumer — `rebalance()` — copied each list, sorted it, and walked `_sorted_keys()`. That is a defence that has to be remembered by every future reader of the container, and the first one to forget it breaks save→load→advance identity in a way that only reproduces after a hop. So the order moved **into** the container: `_attach` / `_detach` place by binary search, so every per-edge list is ascending and a maintained `_edge_keys` vector carries the keys ascending; `edges_with_vehicles()` and `vehicles_on_edge()` are the seam, and `rebalance()` no longer sorts anything.
>
> `tests/test_roads_traffic_order.gd` asserts over the **raw** containers — no sorting on the way in — which is exactly the assertion an unsorted future consumer would need. Run against the pre-Wave-14 append behaviour it produces **356 failures**; against the shipped one, none. Hash-neutral on both cities coarse and fine, and the whole 30-gate contract is unmoved: the feed has zero simulation authority, and this changes only the order it is *stored* in. Report 98 §28 RR-71.
...
The **graph's SHAPE is derived and is never serialised**. On load it is rebuilt in full from the tile grid (measured target: < 30 ms for 12,000 road tiles) and closures are re-applied. Its **labelling** is not derived and does travel with the save — see the `1 → 2` rung below. The route cache is not saved.

*Congestion was specified here as "not saved — a pure function of time-of-day, density, weather and closures, recomputed cold at load with smoothing bypassed". That was wrong and is superseded (doc 93 §E2): the smoother CARRIES HISTORY, so a cold reload made a loaded city diverge from the live run it was saved from. `edge_dynamics` persists the smoothed value and the daily density index, and the cold recompute still runs — it is what gives every edge a value before the saved ones are laid over the top of it.*
...
- Migration: the loader tolerates unknown keys and missing optional keys (`auto_repair`, `event_spikes`) by falling back to defaults. The ladder as built (`RoadNetwork.SECTION_VERSION`, and the authority for it is that constant's own doc comment):
  - **1 → 2** — `edge_dynamics` and `c_day_sum` move from edge-id keys to the edge's canonical tile key, and the graph's LABELLING (`edge_allocator`, `node_labels`, `node_allocator`, `edge_heads`) starts travelling with the save. A rebuild cannot re-derive it once a road tile has ever been edited; a v1 section is read for everything else and its two id-keyed maps are dropped.
  - **2 → 3** (Wave 13, defect A91-D-30) — `last_hour_sampled`, the day-scoped cursor of the hourly `c_day` sampler. Its two readers were both wrong across a load. `_last_sample_hour()` turns the `-1` default into a hard-coded **12.0**, which is the hour `_after_closure_change()` prices its immediate two-hop spillback recompute at — with smoothing BYPASSED, so it does not nudge those edges toward noon, it sets them to noon's `c_raw`. And `full_pass`'s `hour_of_day != _last_hour_sampled` guard, restored as `-1`, samples the save's own hour a second time, settling that game-day's `e_roads_repair` from 25 samples instead of 24. Additive: a v2 section restores `-1` and behaves exactly as it did.

**Derived state must be DERIVED at the load seam, not left for the next tick (Wave 13, report 98 §26 RR-60b).** `load_section()` now samples signal power itself, in `SIGNAL_REFRESH_NODE_BUDGET`-sized `roads_signals` steps. `rebuild_all()` builds every node `powered = true` and nothing else wrote it — so a city loaded with a substation down came back with every signal lit, and `step()`'s first `refresh_signal_power` then reported eleven nodes CHANGING, dirtied every edge in the city and took a full smoothed congestion pass the live run does not take. One extra smoothing step on 644 edges is a different city one game-hour later. Doc 04's grid is restored three cursor steps before roads, so the answer is available and is re-derived rather than persisted. **It is also the most expensive thing in a load on a cold sim** — `power_is_tile_powered` fills doc 04's per-tile transformer memo, 106 ms for the bench city's 2,024 nodes — and that cost is not new: the first live frame after a load was paying it, measured at **186 ms → 79 ms** for the first tick once the sample moved under the veil.

**The restore is RESUMABLE, and the graph rebuild is most of it (Wave 13, doc 08 §2.14, doc 13 §2.9).** `RoadNetwork.load_section_steps()` returns `[[label, Callable], …]` and `CitySim.begin_restore()` splices the whole list into its cursor. `RoadGraph.rebuild_all()` was **73.7 ms** of a 202 ms restore on the 1,500-building benchmark city — the largest indivisible step in the game, and therefore the entire per-frame budget of a loading veil — and is now four phases cut at the seams the function already had: `graph_scan` (clear + the 512 × 512 tile sweep), `graph_nodes` (§2.4's node predicate), `graph_trace` (the polyline walk, one step per `REBUILD_TRACE_NODE_BUDGET` = 700 nodes) and `graph_finish` (orphan loops, node meta, the version bump). Slicing the trace is exact rather than approximate: `_trace_from_nodes` is a loop over node ids in ascending order and `seen` / `_edge_key` carry across the batches, so a batch boundary changes nothing about which edge is created, in what order, or with what id — `tests/test_roads_graph.gd::test_a_stepped_rebuild_lands_where_the_one_call_lands` asserts that at batch sizes down to one node. Measured after: **no restore step over 32 ms on the bench city**, and the longest is `decode`, which is not roads.
...
**Wave 13 conformance (new cases 49–51, report 98 §26)**
...
**C-3's OTHER half — the congestion sweep — is measured, and one of its two proposed forms is refused (report 98 RR-43, 2026-08-20).** Doc 11 §2.13 named "a dirty-set congestion pass" as the fine tick's next honest step. **A dirty set that SKIPS AN EDGE cannot exist here, and the reason is a one-line census rather than an argument: `3,092 of 3,092` edges move on an ordinary pass on the benchmark city.** `hour` is an input to every edge on every pass through §2.10's `D_tod(district, hour)`, and the smoother `c_e ← c_e + (c_raw − c_e)·α` never lands on its target — so the skippable set is empty and always will be, and an implementation that skipped an edge whose closures and condition had not changed would be a different simulation rather than a faster one. The census is `CongestionModel.last_moved`; `tools/profile_congestion.gd` prints it, so the refusal stays checkable.

**The dirty set this pass CAN have is over its INPUTS, and it is taken.** Each edge's road class and district hold still between passes, and those two are the whole of `c_raw`'s shared factor `K_base(class) · D_tod(district, hour)`; the (class, district) pairs are resolved once per graph and priced once per pass, and the per-edge loop reads an index. The key is exact — `district_id` is written only by `RoadNetwork._assign_districts`, `road_class` only where an edge record is built, and both paths run `_refresh_all_edge_state`, which invalidates the table. The association is preserved to the term (`K · demand · dens · evt` binds left to right, so `kd = K · demand` then `kd · dens · evt` is the same float), which is what keeps it hash-neutral. Three whole-graph sweeps go with it: `mean_congestion()` folded into the pass that has just written every value it would sum; the district roster memoised on `graph_version`; and `dark_signal_counts_by_edge()` early-outing on a count `refresh_signal_power` maintains for nothing — **its own doc comment claimed O(dark nodes) and it was O(all nodes)**. Measured: `RoadNetwork.full_pass` **7.7568 → 3.5902 ms per game-minute (−53.7 %)** on the benchmark city, `roads_congestion` **5.205 → 4.127 ms/tick**, fine tick **17.20 → 16.06**; starter city `roads_congestion` **0.873 → 0.668**, fine tick **1.801 → 1.592**, coarse step **9.39 → 8.18**. Hash-neutral on both cities, coarse and fine.
...
5. ~~**Auto-repair default: on or off?**~~ **CLOSED 2026-08-21 (shipped Wave 12; the question was answered in §2.13 and never struck here).** The dial exists — `CitySim.cmd_set_auto_repair_policy(threshold, daily_cap)` plus the read-side `auto_repair_policy()`, behind two `policy: "roads"` rows in `data/ui.json.settings.rows` — so this stops being a permanent ruling, which is exactly the escape §2.13's Wave-12 note said it was waiting for. **The default stays `threshold 0.40` / `cap $25,000/day`**, and it is a measurement rather than a wire that was never connected: doc 92 §30.2's control run is byte-identical with the dial in place across every game-state column of all 21 runs, while turning the threshold off and turning it up to `0.55 / $200,000` both move the matrix. The original recommendation — *off during onboarding, offered as a prompt after the first `road_condition_critical` event* — was **not** taken and is not outstanding: `road_condition_critical` gained a log line in Wave 11 (doc 91 A91-D-26's remainder), so the event a player would be prompted from is now narrated, and the prompt is a doc 12 onboarding question rather than a doc 10 default. Original text: ~~*(Wave 11 — `cmd_road_repair` is settled and is NOT a player verb; see §2.13's ruling and doc 93 §J3. What is still open is the DIAL.)* Default is `threshold 0.40` (RR-3 scale), `cap $25,000/day`. On by default keeps the offline city healthy (spec §21.3, Risk 5) but hides road decay from new players.~~
...
12. ~~**NEW (Wave 12) — the auto-repair quote passes no `M_repair`.**~~ **RULED AND FIXED, Wave 15** (report 98 RR-83, doc 92 §39.8 — doc 92 §34's top-ranked open number, held since Wave 12). `CitySim` wired `RoadNetwork.repair_quote` as `econ_curves.repair_cost_road(road_class, damage_fraction)`, leaving C-16's difficulty multiplier at its 1.00 default; it now passes `M_repair` from the live preset (`sim/city_sim.gd`, the `roads.repair_quote` lambda in `_boot_roads`). **The ruling is that a budget is a budget only if it is compared against the price.** §2.12 calls `auto_repair_daily_cap` "a *player budget setting*, not a price" — and that is exactly why the quote must carry the multiplier: the cap is denominated in the player's dollars, so the settings row that says *$25,000/day* has to buy $25,000/day of repairs on every preset. Quoting at nominal made it buy **1.60× more tile-fractions than repairs cost on `crisis`** and 0.70× fewer on `casual` (doc 92 §31), i.e. the one dial doc 10 gives the player meant a different thing on each difficulty and said so nowhere. Doc 03 §2.4's accrual line has been `M_repair`-only and correct on every preset since doc 93 §M1; this closes the other side of the same seam. **Hash-neutral on `standard`** (`M_repair = 1.00`, the multiplication is the identity) — all four `profile_sim` baselines are bit-identical across the change — and the preset arms move by construction: the cap admits `1/M_repair` of the tile-fractions it used to, so `crisis` queues fewer and larger-costed runs and `casual` more.
13. ~~**NEW (Wave 12) — the signal-power sample cold-fills doc 04's transformer memo one intersection at a time.**~~ **CLOSED, Wave 14.** 96 ms on the benchmark city, paid under the veil as five ~21 ms `roads_signals` restore steps (report 98 §26 RR-60b). `CitySim._boot_power` now warm-fills the memo in one transformer-major pass: `roads_signals` **109.5 → 1.54 ms**, restore total **316 → 207 ms**, against **+9.4 ms** on the cold boot that pays for it. Doc 04 §2.2 carries the table and the hash-neutrality argument; report 98 §28 RR-70.
14. ~~**NEW (Wave 12) — `TrafficFeed._by_edge`'s insertion order differs live vs restored, and is inert only because every consumer sorts.**~~ **CLOSED, Wave 14.** The order moved into the container (sorted insertion plus a maintained ascending key vector, with `edges_with_vehicles()` / `vehicles_on_edge()` as the seam), so the index is identical live and restored without anyone sorting it. §2.15's "as built" note carries the argument; `tests/test_roads_traffic_order.gd` asserts it over the raw containers and produces 356 failures against the old behaviour. Report 98 §28 RR-71.
