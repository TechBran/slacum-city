...
**Derived state must be DERIVED at the load seam, not left for the next tick (Wave 13, report 98 §26 RR-60b).** `load_section()` now ends its graph work with `graph.refresh_signal_power(power_is_tile_powered)`. `rebuild_all()` builds every node `powered = true`, and nothing else wrote it — so a city loaded with a substation down came back with every signal lit, and `step()`'s first `refresh_signal_power` then reported eleven nodes CHANGING, dirtied every edge in the city and took a full smoothed congestion pass the live run does not take. One extra smoothing step on 644 edges is a different city one game-hour later. Doc 04's grid is restored three cursor steps before roads, so the answer is available and is re-derived rather than persisted.
...
**Wave 13 conformance (new cases 49–50, report 98 §26)**
...
50. `test_the_daily_sampler_cursor_rides_the_save` (RR-60) — a `section_version` 3 section carries `last_hour_sampled` and restores it, so `_last_sample_hour()` answers the hour the city is actually in rather than the hard-coded 12.0 the `-1` default produces; a v2 section (the key erased, the version stamped back) still loads and lands on that documented default, with the rest of the section unchanged. Doc 08 §2.8's additive-first rule, read from both ends.
