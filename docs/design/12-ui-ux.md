...
**Three states, and only the third has a button.**
...
| **LOT-LOCKED** | `Lot · LOT-LOCKED` (warning tint) · *"Boxed in: it holds 1×1 of a 2×2 lot, so it can only reach level 2 of 6. HSE-014 stands on the rest — clearing it refunds $300."* | `Fix this →` → `FIX_BUILDING` at the neighbour |
...
Five strings: `ui_building_lot_title`, `ui_building_lot_reserved`,
...
on every machine. A lot blocked by a road or by undeveloped land routes
`FIX_NONE` and draws **no button** — a `Fix this →` that opens nothing is the
defect this project is named after — and says so in words instead
(`ui_building_lot_locked_ground`).
