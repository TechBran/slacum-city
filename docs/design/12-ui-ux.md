...
**Still open against this doc:** the building shader does not yet read `sc_overlay_mode` (doc 11's to implement — the rail writes it, and D-4 fixes the value); the overlay legend ships inside the strip rather than as the separate top-left `OverlayLegend` card of §2.5; and S6, S7, S8, S11, S12 and S13 remain unbuilt.
