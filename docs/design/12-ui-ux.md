...
✓  Top off water storage                               $1,240
   Fill every tank, so hydrant pressure survives     [ DO IT ]
...
* **The button is on the row's SECOND line, and that is a measurement rather
  than a taste.** `mark · title · cost · [ DO IT ]` on one line has a combined
  minimum width of **395 dp** at A2's 1.3 text scale with A3's larger touch
  targets — against the **300 dp** a 360 dp phone leaves this panel's body
  (360 − 2×4 safe area − 2×16 panel offset − 2×10 `SheetPanel` margin). A
  `ScrollContainer` with horizontal scrolling off hands its content's minimum
  width straight up its parents, so the sheet became **423 dp** wide, and
  `grow_horizontal = BOTH` centred the overflow and pushed ✕ — the only way out
  of a modal — off the right edge (`tools/ui_preview.gd --screen=all
  --size=360x800 --text-scale=1.3 --large-targets --audit --strict` → exit 1,
  one finding in the whole deck). Beside the detail sentence, which autowraps
  and therefore has a minimum width of one character, the row needs 157 dp and
  the panel 307. It is the shape §2.9's requirement rows already use.
...
32. **`test_ui_transformer`** (§2.25, D-114/D-115/D-116, Wave 25) — the whole of S18 and the diet, on a real `CitySim` and the real `ui_root.tscn`. **The pick:** every pad in the founding city answers a tap on its own centre, and each of those tiles is asserted to carry no building — which is the reconstruction of what the fork answered (`PICK_NONE`, i.e. *deselect*); the catch is capped at half a tile, proved at the zoom where 48 dp is wider than a tile, so a tap a whole tile away is refused and the pad still answers its own centre; and a street collectable standing on the pad still outranks it. **The reading:** the customer list is `PowerGrid.buildings_served_by`'s own roster in its own order, the kW total is the sum of the rows rather than a second reading, the capacity is `cap_eff` at today's ambient with the nameplate published beside it, FED BY walks the radial tree to the pool, a pad with no customers says so, and the panel's distress band equals `PowerInfraModel.distress_for`'s at four load ratios — the delegation is DRIVEN, not asserted. **The crew:** the panel opens on UPGRADE when the unit works and on REPAIR when it is dead; CALL A CREW arms on the first tap and spends on the second; the transformer is still FAILED after the money is spent, comes back only when the crew finishes, and lands at doc 02 §2.12's post-damage target; a second crew is refused `E_ALREADY_REPAIRING` and the panel shows the queue's own ETA in its place; a transformer in good repair is offered no strip at all; and the job survives a save — two restores of one capture agree, and both complete on the other side. **The diet:** the POWER section is one 48 dp row with a tooltip that names the transformer and opens S18 on it while S5 stands down; an UNSERVED building still gets the sentence on its own panel; and `Fix this →` on a `POWER_CAPACITY` row opens S18 on the component the headroom binds at. **The world and the wires:** the selected pad is ringed, the ring leaves with the selection and an unknown id clears it rather than stranding it; and every signal this wave added or kept is proved CONSUMED — `UIRoot.grid_action` for S18's three verbs, and S5's two retired grid signals still raised on the panel's behalf so an unpatched shell keeps working. 23 tests. The pixel pass is `tools/ui_preview.gd --screen=transformer|transformer_stressed|transformer_troubled|transformer_severe|transformer_dark|transformer_failed|transformer_repairing|transformer_crowded`, plus `building_power|building_power_fix` for the two S5 variants.
...
| D-78 | **S17, the Storm Prep window, ships** — `ui/storm_prep_model.gd` (headless: the countdown, the three readiness meters, six rows with a price and a reason, the Storm Ready line) + `ui/storm_prep_sheet.gd` (code-built rows on a `ModalLayer` modal, the S14 shape) + `UIRoot.bind_storm_prep` / `open_storm_prep` / `refresh_storm_prep` + the node in `game/ui/ui_root.tscn` + 36 `ui_storm_*` strings + `data/ui.json storm_prep` + a `SURFACES` row + **two preview states**. §2.24 has the screen; the sim door is `CitySim.cmd_storm_prep_action` (`99-PA` PA-26), the mechanics ruling is doc 93 §AH2, and the defects are A91-D-87 / A91-D-88. **The action row is two lines because the strict sweep said so**, not because the sketch wanted it: one line needed 395 dp of a 300 dp body at 130 % text with larger targets, which made the sheet 423 dp wide and put ✕ off a 360 dp screen (§2.24's fourth bullet carries the derivation). `tests/test_ui_audit.gd` could not see it — its `SURFACES` sweep reads a CLOSED modal, and a hidden subtree reports a minimum width of 0 (`PauseMenu/Panel` 260 and `GoalsSheet/Panel` 0 in one mount) — so this lane binds S17 to a six-row fixture there and measures `Panel/Body` directly. Widening the sweep itself is left as an open question for the lane that owns the file. | §2.2, §2.24, doc 07 §2.7.6/§2.7.7 | Doc 07 has authored six preparation actions since it was written and the game had **no verb, no door and no minute** in which to take one: `grep -c storm_prep sim/city_sim.gd` → **0**, and `DisasterDirector.storm_prep_action`'s own window test could never be true. The player was handed the most urgent notification the game can send — F7's CRITICAL, exempt from every rate limit — and it pointed at nothing. **Deferral table, stated rather than faked** (doc 93 §AH3): two of the six actions ship PRICED, RECORDED and counted toward Storm Ready with their effect not yet wired, because neither file is lane B's to edit — `pre_stage_crews`' −35 % travel time needs a knob on `sim/incidents/fleet_system.gd`, and `load_shed`'s −5 % commercial tax needs doc 03's revenue half in `sim/economy/economy_system.gd`. Also deferred: the `weather_warning` alert's tap target and the `storm_report_ready` sheet, both `game/main.gd` lines the lead merges (the S16 precedent, D-66). |

| D-79 | **`Fix this →` is dispatched by a headless router, and a row that cannot be fixed says so.** `ui/fix_router.gd` maps a `fix_target` to one of three ACTIONs — focus a world point, open a sheet pre-armed (`FIX_POWER` → the panel's power strip, carrying the component the headroom **binds at** — which is what that row's id actually is), run a verb with a quote (`FIX_REPAIR` → `cmd_repair_building`, `preview: true`) — or refuses with one of five named reasons. `ui/world_locator.gd` turns any id the sim publishes into metres: tile, flood cell, block, building, **grid component**, district, road segment. `game/main.gd::_on_fix_requested` is three lines and `_alert_world_pos` is one. No UI file changes shape; the panels emit the same `fix_requested` they always did. | §2.7, §4.5, doc 99 PA-05 / PA-38 | **The single most important teaching device in the game was dead on two of its seven rows** (§2.7's own words for `Fix this →`). `POWER_CAPACITY` fills `fix_target_id` from `sim.grid.attachment_of()` — a **transformer** id — and the code table mapped it to a BUILDING kind, so the shell looked `T-02` up in `sim.buildings` and returned; `E_AVENUE` supplied no id at all and the shell discarded it on line one, while `ui/building_panel.gd:414-416` drew the button on `kind != FIX_NONE` alone (the land panel also requires an id, which is why it was safe). Both produced a button that depresses and does nothing: no camera move, no toast, no haptic. Every building answer was also anchored on the NW corner **tile**, so a jump to a 4×4 building landed sixteen metres off its lot. 468 asserts now stand behind it, including the audit's criterion — every row of the real `_check_params` checklist, for every building in the starter city, resolves or refuses by name. Two consumers filed on the panel side: `E_AVENUE`'s `fix_target_id`, and gating the button on `FixRouter.can_route` (report 98 §50.2). |
...
| id | change | doc ref | why |
|---|---|---|---|
| D-84 | **The Economy tab gains an `Upkeep` band above the ledger, the assistance row gains a note, and the event log gains four rows.** (a) **Upkeep** — `Tax lost to wear −$169` with its per-game-day figure, `27 of 27 taxed buildings below Good`, a `Repair all worn (N) — $X` button, and the standing `Automatic repair:` policy line. `DashboardModel.feed_upkeep` / `upkeep_view` format it; the loss is `BudgetModel.condition_loss()` off doc 03's own per-building settle rows, the repair half is `CitySim.cmd_repair_all_worn(true)`'s real payload and the policy line is `building_repair_policy()`. `CityDashboard.bind_upkeep(repair_all, policy, balance)` is the wire; a shell that does not bind it draws the loss half and no button. (b) **The assistance row's note** now reads `+$4,128/d today · ends day 7, 7 days left` — `ui_budget_assistance_days_left` takes `{per_day}` and `{end_day}` beside the count RR-102 added, and `_build_ledger` learned to draw a line's note at all. (c) **Four `data/ui.json.event_log` rows** (append): `assistance_stepped` ×2 (routine / final), `building_condition_band` ×2 (worn / poor), plus `building_repair_policy_ran` and `development_phase_charged`. (d) **The policy line becomes a CONTROL** (report 98 RR-150a): two cycling faces under the sentence that reports them, `PolicyBand` (`off` → `below 60%` → `below 85%`) and `PolicyCap` (`no budget` → `$2,500/day` → … → `$50,000/day`). Both ladders arrive verbatim on `CitySim.building_repair_policy()` (`thresholds`, `daily_caps`) and every press writes the PAIR through `cmd_set_building_repair_policy`. `bind_upkeep` grows a **fourth, optional** parameter: three wires draw the sentence, four draw the dials. The row carries no label of its own — the sentence above is the label and the faces name themselves to a screen reader (`ui_dashboard_upkeep_policy_band_a11y` / `_cap_a11y`). | §2.10, §2.13, doc 02 §2.6, doc 03 §2.5/§2.5a, 99-PA PA-31/PA-32/PA-33/PA-83, report 98 §53 RR-148–RR-150a, doc 93 §AL3a | **Four audit rows, one finding: the simulation knew and no screen said.** PA-31 measured an untouched city losing 53 % of its non-subsidy income to decay with nothing on any surface until `damaged` at 0.35 — a line doc 93 §Y1's ownership floor now makes unreachable for served private stock, so on a 60-game-day starter run there are **48 band crossings against 0 `building_damaged` events**. PA-32 measured doc 03 §2.5a's grant retiring **$589.71 a game-day** with no toast, no log row and no end date. PA-83 found `development_phase_charged` — 14–15 debits of $1.2K–$21K per 21 game-days — with **zero** shell consumers. The band answers the first two in the unit the player is losing them in: a count of worn buildings is a fact, and *"you are losing $169/gh; ending it costs $24,281"* is a decision. **The two counts on the band are deliberately different sets** and it never conflates them — the loss counts taxed stock (private included, which the city can never repair), the button counts what the city owns. **Two preview states ship in the same commit** as the screen (A91-D-28's lesson): `economy_assistance` and `economy_upkeep`, the latter driving the real `cmd_repair_all_worn` on a booted `CitySim`, both `--audit --strict` clean at 412×915 and at 360×800 with `--text-scale=1.3 --large-targets`. **The verb needed a door and the first draft of this row declined to give it one.** RR-150 shipped `cmd_set_building_repair_policy` with its ladders, its refusal code, its save rung and its daily pass, and no way for a player to call it — which is 99-PA's own PA-55 shape filed against the lane that wrote the verb. It lives on the band rather than on §2.13's Auto-response rows for a reason that is not only ownership: the settings sheet is where a player arrives having already decided, and the band is where the game tells them there is a decision. **Switching on supplies a budget** — doc 03's own `AUTO_REPAIR_DEFAULT_DAILY_CAP`, now published on `building_repair_policy()` so `ui/` reads it instead of restating it (C-07) — because `enabled` needs both dials and a rung with no money behind it is a control that does nothing when pressed; switching off keeps the budget (doc 93 §AL3a). **Pressing it found a live use-after-free in the button (a) had already shipped:** every control on this band lives inside the subtree a refresh rebuilds, so `refresh()` from a `pressed` handler frees the button while its own signal is on the stack — `Repair all worn` had it from the moment it was written and nothing had ever mounted the dashboard to press it. `_reread_upkeep_after_press` refreshes the reading now and the nodes once the emission unwinds. **A third preview state, `economy_upkeep_auto`**, ships with it — the policy-ON band, which a screenshot of the shipped default can never show, and the state that caught the control's first layout (`Automatic repair  below 85%  $10,000/day` is 300 dp on a 360 dp screen at `--text-scale=1.3`; both faces clip and the audits are back to exit 0). |

**The settings row is still owed, and is now a SECOND door.** 99-PA §3.2 assigns
...
So the S9 row is a one-line note for the lead: it lands the moment a
`POLICY_BUILDINGS` arm exists beside `POLICY_ROADS`, reading its ladder from
...
`data/roads.json`. **The verb is not waiting on it** — D-84 (d) puts the two
dials on the Upkeep band, which is the door 99-PA PA-33 actually asked for
...
| D-87 | **WHILE YOU WERE AWAY stops over-reporting the night and starts naming the cap in the hours the player slept.** Three edits, one header. (a) `elapsed_game_minutes` is the **credited** time, not the wall clock: `game/main.gd` passed `elapsed_wall_s` straight through, so a 26-hour absence rendered *"Away 26h · 65 days of city time"* over a city that had lived 30 — the shell now passes `plan.credited_real_ms / 1000` (report 98 §58.2's snippet). (b) `AwayModel._cap_real_hours` reads the plan's new `cap_real_hours` instead of `cap_game_hours`, and `ui_away_capped` becomes **"Your city ran for {hours} hours while you were away — the maximum."** — 12, not 720. The legacy key is still accepted and divided by 60, so an un-updated caller says something true rather than something absurd. (c) The **other direction** is pinned for the first time: `test_an_uncapped_away_report_does_not_imply_a_cap` builds the report from a real eight-hour plan and asserts `capped_text == ""`. | §2.12, doc 08 §2.12, doc 01 §2.10, doc 93 §AO | D-77 fixed the surfaces for the wrong cap. It landed the same wave RR-133 shipped `max_coarse_hours` at 360 game-hours, so the honest thing to say was "six real hours" and both surfaces learned to say it. **RR-160 deletes that clamp** — an eight-hour night is not capped at all any more — which turns D-77's fix inside out: the line that had to fire now has to *stay quiet*, and the only absence that still trips it is one longer than half a day. What survives D-77 unchanged is its structural lesson, and it is the reason (c) exists: `capped_text` was dead code for three waves because nothing tested the branch, and a line that renders in exactly one untested case is a line that renders wrongly. |
...
### 2.20 Wave 21 — the boarded-up building is a thing the player can BUY BACK

*This lane adds no widget and no preview state. It adds one string family, one
event-log row and — the reason it is here at all — it makes a REPAIR button
appear on a class of building that has never had one. Ruling: doc 93 §AS1/§AS2.
Verb: report 98 §63 RR-182/RR-183.*

| id | change | doc ref | why |
|---|---|---|---|
| D-94 | **A private building an unanswered fire has gutted grows a REPAIR affordance, and that is the whole surface change.** `CitySim.cmd_repair_building`'s `E_OWNER_MAINTAINED` blocker gains one exception — `Building.burnt_out` — so §2.9 item 6's actions row, `RequirementFormatter.FIX_REPAIR` and the building panel's price line all start answering on private stock they have always refused. **No file in `ui/` changes**: the panel asks the command for its own quote and draws whatever comes back, which is D-80's rule and why this is one blocker and not a screen. Two copy changes ride with it. **(a)** `data/ui.json.event_log` gains **both** of a fire's terminal endings — `building_destroyed_by_fire` AND `building_condemned_by_fire` — and the first of those is the surprise: doc 06 has emitted it since Wave 8 and **no router had ever named it**, so a building a fire actually took was announced only by doc 02's own `building_destroyed`, which says nothing about a fire. Under §AS1 they are two different sentences about the same moment and the feed can now say either. Each gets its OWN notification id, and that is not tidiness: `building_lost_to_fire` is P1 and un-aggregated (a building the city actually lost is the sharpest thing that happens to a roster) and `building_condemned` is P2 and aggregated (a serviceless city collects several at a time, and fifty separate lines would be §AS3's event storm one screen over). Reusing `incident_failed`'s id for the destroyed row was tried and rejected in the same commit: doc 06 sends `{incident_id, target}` and that id's strings ask for a `{kind}` and a `{tier}` the payload does not carry — a row whose args do not resolve renders a brace. `building_condemned` is also distinct from Wave 19's `buildings_condemned` (plural), which is the condition BAND's running summary and answers a different question. **(b)** the `E_OWNER_MAINTAINED` requirement line keeps its wording for every other case; the gutted shell simply never raises it. | §2.9, §2.6, doc 02 §2.6a, doc 93 §AS2, report 98 §63 RR-183 | Under §AS1 a city with no fire service does not LOSE buildings, it collects boarded-up ones — and the whole ruling rests on the player having something to do about that. Doc 02 §2.6a has always said the city may not spend money on private stock because the owner keeps it up; §AS2 stops the owner rebuilding a burnt-out shell, so without this exception the player would be looking at a building earning a fifth of its tax, with a REPAIR button greyed out and a reason line telling them somebody else was handling it. **That is the failure mode this game already knows by name** — doc 12 D-81's four hard-wired `✕ —` tiles, a surface confidently reporting a thing that is not happening. The affordance and the ruling are one change: an unanswerable fire costs the player 80 % of a building **and a repair bill**, and the bill is only a cost if they can see it and pay it. |

### Wave 23 — the predicate measured the shell, not the service (2026-09-04)

*Verb: report 98 §65 RR-192/RR-195.*

| id | change | doc ref | why |
|---|---|---|---|
| D-97 | **No file in `ui/` changes, and that is the finding rather than an omission.** Doc 93 §AV1 rewrites what "this city cannot answer a fire" MEANS, and every surface that speaks about it already reads the sim rather than re-deriving the predicate: D-94's two terminal event rows (`building_destroyed_by_fire` and `building_condemned_by_fire`) are emitted by `CascadeOps` on the answer `world.destroy_building` actually gives, so a building that now BURNS where it used to be condemned changes which of the two existing rows fires and changes nothing about either row. The building panel asks the command for its own quote (D-80's rule), and §AV1 moves no blocker. **The one surface that DOES move is the money**, and it moves by disappearing: §AV3 takes `E_departments` from $11.00/gh to absent on a city whose water plants are rubble, so the budget sheet's expense list is one row shorter and the ruin-bill line the player is shown is $480/game-day smaller. **What is deliberately NOT surfaced yet, and is ranked in report 98 §65's open questions:** §AV4 makes a fire station cut its district's ignition rate by up to 42.9 %, and nothing on any screen says so. Doc 02 §2.9's coverage overlay draws the RADIUS and doc 09's happiness reads the scalar, but the sentence *"a covered district catches fire less often"* is now a real mechanic with no line of copy anywhere — the same shape as PA-44's unlabelled grant, one wave earlier in its life. | §2.9, §2.13, §2.21, doc 06 §2.6, doc 93 §AV1/§AV3/§AV4 | A wave that changes a RULE and not a screen still owes the UI ledger a row, because the absence has to be checkable: the reason nothing in `ui/` moved is that the surfaces were built to read the sim, and a future reader finding no D-row for Wave 23 could not tell that from an oversight. The §AV4 half is filed here rather than fixed because a coverage line is a copy decision with a strings-file owner and a preview state, and this lane owns neither. |
### 2.25 Wave 24 — where are my people?

*One vital changes its words, one chip learns to acknowledge itself, and one
preview state photographs the moment the whole thing is about. Measured in doc
92 §64. Ruling: doc 93 §AX. Verbs: report 98 §67 RR-202/RR-203.*

| id | change | doc ref | why |
|---|---|---|---|
| D-100 | **S5's `Occupants` vital says how many have MOVED IN, and the population chip pulses on the frame it moves.** (a) `BuildController._occupants_text` → `CitySim.settled_residents(sim_id)`: the vital reads `ui_building_occupants_of` — *"0 of 4"* — whenever the settled figure and the authored one disagree, and the bare authored figure (`4`) when they agree, so a full building spends no extra characters and a building that is not full stops claiming to be. `-1` from the sim means *houses nobody*, which is every shop, station and pump, and those keep reading `0`. (b) `CityHUD._note_population` flashes `HudModel.CHIP_POPULATION` for `data/ui.json.layout.population_flash_s` (1.2 s) when the number it is handed differs from the one it last PAINTED — the comparison is against the painted value because `refresh()` re-runs with the same snapshot on every re-layout. Same mechanism as Wave 14's deposit pulse, so A8's reduce-motion suppression already covers it; `HudModel.FLASHABLE_CHIPS` is the set `_process`'s expiry sweep may clear, and `grid`/`water` may never join it because they carry §2.4's STANDING pulse. (c) **S5 is re-read on the shell's own cadence.** `BuildingPanel.refresh` has documented itself since Wave 5 as the re-read *"after an upgrade, a tick, or a construction completion"* and **nothing in the shipped shell has ever called it** (doc 91 A91-D-136) — an open panel was a photograph of the city at the moment of the tap, so its condition, its ETA and its occupants never moved while the player watched. `UIRoot.refresh_building_panel()`, on `refresh_land_panel`'s exact shape and cost, called from `_refresh_hud`. Preview state **`building_moving_in`, same commit** — a house placed through the real `cmd_place_building`, panel open, `0 of 4` on its face. | §2.4, §2.9 item 4, doc 09 §2.10, doc 93 §AX, report 98 §67 | The player, on their own city, 2026-09-04: *"As I'm building up houses and have new residents that pop up, I don't get an increase in population like people. I don't see it actually counting up."* **They are describing a latency, and it was measured rather than guessed** (doc 92 §64): from the tap to the number moving is **176 real seconds at 1×** — 120 s while the shell goes up contributing nothing, then up to 60 s more waiting for doc 01's hourly settle — and when it finally moves it moves in ONE jump, 144 → 148. It never counts up and it never will: doc 01 settles population once per game-HOUR and doc 09 §2.10 sums an aggregate, not a queue of arrivals. **So the counter is honest and the game was failing to SHOW it** — and worse than failing, it was contradicting itself, because the panel of the house the player had just placed read `Occupants 4` beside a chip that had not moved. Two surfaces telling a player different things is not a slow counter. The chip's own pulse is the other half: three minutes is long enough that the player is looking somewhere else when the payoff lands, and a chip that lights up when it changes is the cheapest possible *"that was you"*. |

**The deck is 84 states**, `building_moving_in` included; `--screen=all --audit
--strict` exits 0 at 412×915.

### 2.26 Wave 25 — the land panel says what the ground gives back, and a find is a receipt

*Two rows. D-117 is the surface half of doc 03 §2.8b's `land_works` line — a band
quoted before the purchase, a running total after it, and a toast on every find.
D-118 is the world half: the block under development finally LOOKS like a block
under development. Rulings: doc 93 §AZ. Measured: doc 92 §66. Verbs and deltas:
report 98 §69.*

| id | change | doc ref | why |
|---|---|---|---|
| D-117 | **S4 gains a `What the crews find` band, and every find is a toast with the material in it.** `LandPanelModel.works_view()` (asking `EconomySystem.works_yield_band()`, so this file authors no fraction) → `LandPanel._render_works`, three label rows between the advantages and the note. **(a) `Typically returns · $1,310 – $2,940`**, shown on land NOBODY OWNS — the other half of the `Est. development` sentence §2.8 has always printed, from the same table, so a retune of `data/economy.json` moves the quote the same hour it moves the money. **(b) `Recovered so far · $2,900`**, only once the block is the player's, off `LandBlock.works_yield_total`; before the first phase completes it reads *"Nothing yet — the crews have not broken ground."* **(c) `Materials yard · $410`**, the CITY's stock, whenever there is any. Under a PENDING `road_install` or `utility_corridor` the phase row grows a second line, `Yard materials −$1,160`, quoting what the yard would take off **if it were charged now** — never on a phase already charged, because re-quoting today's yard against yesterday's invoice would print a discount the player did not get. The receipt is `ui/land_works_model.gd` → `UIRoot.report_land_works()`, spent through `_spend_feedback` exactly as `StreetModel`'s bounty is: `Timber — $540`, or `Fill and aggregate — $297, $153 to the yard` when part of the find was kept, because a player told $450 who sees $297 land has been lied to by rounding. **No haptic** (`StreetModel`'s rule: §2.14's cues answer something the player DID, and nobody pressed anything here); the coin is `data/audio.json`'s `cash` rule on the event, guarded on the CASH half so a find that went entirely to the yard rings nothing. Two `event_log` rows (`land_works_find`, `land_works_stockpile_spent`), 17 strings, one preview state — **`land_yield`, same commit**. | §2.8, §2.15, §2.26, doc 03 §2.8b, doc 93 §AZ3 | The player, 2026-09-04: *"opening up a piece of land will give you resources and money back."* Money that arrives with no sentence attached is money the player does not know they earned — Wave 14's whole finding, applied to a new line before it could repeat. **The band before the purchase is the half that changes a decision**: §2.8's panel could already tell a player that a forest block costs $36,275 to develop and could not tell them that forest is where the timber is, so the terrain premium read as pure punishment. It is derived, not authored: `works_yield_band` reads doc 03's own `terrain_phase_mult`, which is why a forest block quotes 1.90× the clearing timber without a second table existing. **The yard's line under the phase is not a nicety** — ruling §AZ3 — because an invoice that silently got smaller is the one thing a ledger may never contain: without it the player reads `Roads $6,340` against a doc that publishes $7,500 and has nowhere to find the missing $1,160. |
| D-118 | **The world stops lying about a block being dug out.** `game/render/land_works_view.gd` (doc 11 §2.18) dresses every developing block off `LandBlock.development_state` and the running job's own progress: pegs and tape at SURVEY, scrub that goes **clump by clump as the clearing runs**, a graded plane and spoil heaps at GRADING, base laid progressively along doc 10's own block template at ROAD_INSTALL, an open trench toward the block centre at UTILITY_CORRIDOR, kerbs at FINAL_DEVELOPMENT, and nothing at READY. Heavy plant rides doc 11 §2.16's existing layer with a per-phase profile, so the crew type doc 09 §2.3 names is the machine that turns up — two excavators and lorries hauling spoil OUT on the `heavy_equipment_crew` phases, one machine and deliveries IN on `road_crew`, and no plant at all for the two `construction_crew` phases. Six preview states in the same commit (`tools/land_works_preview.gd`, one per phase). | doc 11 §2.18, doc 09 §2.3, doc 93 §AZ4, report 98 §69 RR-211 | **This is not a UI row and it is in this table anyway**, for the reason D-91 was: a delta table that only recorded new pixels would not record the wave in which the player can finally SEE the money they are spending. `cmd_start_development` charges the treasury six times per block, $1.2K–$21K a phase, and until this wave the entire feedback for all six was a progress list on a panel the player had to go and open. The screen said `Grading — about 14m left`; the world said nothing at all. **The player asked for exactly this**, 2026-09-04: *"we want construction animations for that land — to show that the land is being worked: digging it, materials."* The land panel's numbers (D-117) and the block's own appearance now answer the same question, and the one that answers it without being asked is the one in the world. |

### 2.25 S18 — the transformer panel (Wave 25)

*Model `ui/transformer_panel_model.gd`, view `ui/transformer_panel.gd`, tests
`tests/test_ui_transformer.gd`, preview states `transformer`,
`transformer_stressed`, `transformer_troubled`, `transformer_severe`,
`transformer_dark`, `transformer_failed`, `transformer_repairing`,
`transformer_crowded` — **every one of §5.10's six distress bands**, plus the
failed unit with a crew already rolling, plus the longest the customer list ever
gets. Rulings doc 93 §AY2/§AY3. Verbs
doc 04 §2.15. Measured doc 92 §65.*

The player, 2026-09-04: *"You click the transformer and all of that information
pops up just like a building does."* **"Like a building does" is the
specification**, and this screen keeps every part of it: it lives on
`PanelLayer` beside S5 and S4, it is the same width
(`UIWidgets.side_panel_width`), it closes its siblings and is closed by them
(D-16), it carries the same ✕ with the same tooltip, its verbs are pinned in a
footer outside the scroller (PA-47), and every target on it is 48 dp with an
accessibility name.

    Transformer T-03        ▮L2▮ · Working · 96 % condition        ✕
    ████████░░  78 % loaded — getting full. 33 kW spare.
    Carries 96 kW now, 117 kW at the 19:00 peak, of 150 kW.
    ── FED BY ──
    Feeder F-NORTH · 62 % — room to spare, 1.1 MW spare
      [ UPGRADE  $210  →  3.0 MW ]
    Substation SUB-A · 41 % — room to spare, 3.5 MW spare
    ── SERVING 4 BUILDINGS · 117 kW ──
    Apartment L2 · 42 kW · Standard · lit          ›
    Shop L1 · 18 kW · Standard · lit               ›
    …
    [ CALL A CREW  $150 ]   [ UPGRADE  $1,100  →  400 kW ]
    REMOVE TRANSFORMER  refund $275

**Five things it shows and the order is the argument.** Which unit and what
condition; what it carries against what it can carry **at today's ambient** (doc
04 §2.7 derates with the weather, so the nameplate is not the answer and the
panel prints both); what feeds it; **who is behind it**; and the verbs. When the
unit has FAILED — or a crew is already on it — `TransformerPanelModel.focus_of`
draws the REPAIR first, because that is the moment the player tapped it.

**The customer list is the part that could not be drawn before.** `PowerGrid`
had no public way to ask a transformer who its customers were (doc 04 §2.15.1);
each row is a 48 dp target that jumps the camera to that building and carries its
demand, its shed priority class and whether it is lit right now. The list caps at
`data/ui.json.transformer.max_customer_rows` (8) with a `+N more` line, because
doc 92 §65.1 measures a grown city's widest transformer feeding **17** buildings
and 17 rows at 48 dp is 816 dp of a 915 dp display.

**Bands are read, never re-derived.** `PowerGrid.distress_band` is the same
function `game/render/power_infra_model.gd` classifies the pad with, so a
cabinet drawn SEVERE cannot sit under a panel calling it *"getting full"*.

| id | change | doc ref | why |
|---|---|---|---|
| D-114 | **A transformer is selectable, and what selects it is a radius rather than a tile.** `BuildController.pick_at_ground` gains a fourth answer, `PICK_COMPONENT`, between OPPORTUNITY and BUILDING; `component_near(point, radius)` measures the tapped POINT against the pad's tile centre on the same 48 dp `tap_radius_m` §2.21 gave the street collectable, **capped at half a tile** (`COMPONENT_PICK_TILE_FRACTION`). `PowerGrid.buildings_served_by()` and `PowerGrid.damage_fraction()` are the two read-only accessors it needs; `PowerInfraView.set_selected(id)` rings the tapped pad in the world, and `UIRoot.transformer_selected` is the one signal the shell clears it on. | §2.8, §2.21, doc 04 §2.15.1/§2.15.3, report 98 §68 RR-205/RR-207 | **A tap on a transformer used to DESELECT.** Measured on both shipped cities: doc 09 puts every pad on a tile no building occupies, inside a developed block whose stage does not open S4, so `pick_at_ground` fell through all three of its answers and returned `PICK_NONE` — which `game/main.gd` treats as "tap on empty ground", i.e. close whatever is open. 100 % of taps, on the most complicated and most failure-prone object the renderer draws (doc 92 §65.3). **The cap is the safety half**: 48 dp is 16.04 m of ground at full zoom-out, two tiles in every direction, so without it a tap squarely on a house would open the transformer next door. A transformer stands on one tile and a tap further than half a tile from its centre is not on it. |
| D-115 | **The building panel's POWER section becomes one row and a door.** Wave 17's D-70 section — header, draw line, N hop rows each with a title, a reading, an UPGRADE button and a checklist, an armed REMOVE row, and the next-level headroom line — is replaced by `Power · fed by T-03 · 78 % · ›`, tinted with the transformer's own load band, or `Power · NOT SERVED · ›` in the critical state. The row is a 48 dp target that opens S18. `BuildingPanel.grid_upgraded` / `grid_demolished` are **retired** (S18 raises `upgraded` / `demolished` and `UIRoot.grid_action` carries them); `power_row_opened` replaces them. `Fix this →` on a `POWER_CAPACITY` checklist row now opens S18 on the component the headroom binds at — which is what `fix_target.id` has always been — instead of arming a strip. Retired copy: `ui_power_section_title`, `ui_power_draw`, `ui_power_hop`, `ui_power_hop_load`, `ui_power_next_level_ok`, `ui_power_next_level_short`. | §2.9, §2.7, doc 93 §AY3, report 98 §68 RR-208 | The player: *"So we don't clutter the building information … Right now there's too many things there."* **Measured: 76 rows on a shop, 65 on a flat, 2,098 dp of content on a 915 dp display** (doc 92 §65.4). But panel length is the symptom; the defect is OWNERSHIP. A transformer is shared by every building in its service radius, so its REMOVE row was armed from N panels and a player who upgraded it from a house's panel had no way to see the other N−1 buildings they had just helped. After: **42 / 53 / 43 rows** — the worst case in the city is now shorter than the best case was. |
| D-116 | **Three things on that panel were examined and KEPT, and each is a ruling.** (a) The **`Fix this →` strip** (`cmd_fix_power_capacity`) stays: it is quoted against THIS building's next level, moving it to S18 would need a building id S18 does not have, and deleting it would leave doc 04 §4's fix verb with no door at all. (b) The **UNSERVED sentence** and (c) the **shed-feeder sentence** stay: both are facts about this building's power right now, and a player whose shop is dark must not have to open a transformer that is working perfectly to find out why. **The water block is NOT collapsed**, for the same test read the other way: a doc-05 node has no surface of its own to be handed to, so a one-row summary would delete a verb rather than move one. The priority row, the coverage checklist and the progress block stay for the same reason — each is a decision only this panel can make about this building. | §2.9, doc 05 §6, doc 93 §AY3 | The test is not *"how long is the panel"*, it is *"who owns this decision"*. A door to nowhere is worse than a long panel, and it is this project's signature defect (A91-D-19) run backwards — a verb whose only surface was deleted for tidiness. |

### Wave 26 delta — the Water Facility card builds a water facility (2026-09-05)

*(Ruling 93 §BA3, report 98 §70 RR-215, defect row doc 91 A91-D-138. One row, and
it changes no pixel: the sheet's card list, its tabs, its ordering and its prices
are all exactly as §2.7 photographs them.)*

| # | change | reads | why |
|---|---|---|---|
| D-120 | **The plain `Water Facility` build card enters the doc-05 pump placement, not doc 02's bare-shell one.** `BuildController.enter("water_facility")` with no variant now routes to `enter_water_component(catalog.reference_water_variant())` — the same door `Water Facility · Pump` has always used — so the GHOST and the CONFIRM run the same twelve checks (`E_NOT_OWNED`, `E_NOT_DEVELOPED`, `E_CITY_LEVEL`, `E_FOOTPRINT`, `E_NO_WATER`, `E_NO_MAIN`, `E_UNSERVED`, `E_FUNDS`) instead of doc 02's five and doc 05's twelve respectively. The card keeps its id, its `ui_build_card_water_facility` name key, its Utility tab, its 3×3 footprint and its $45,000 price — doc 03 quotes both doors off the same `water_plant` row — so the deck stays at 83 states and no screenshot changes. | §2.7, doc 05 §6, doc 93 §BA1/§BA3 | **The ghost was green and the purchase was a lie.** Before this, tapping the plain card and confirming ran `cmd_place_building`, which stamped a `water_facility` that hosted no doc-05 node: it decayed, it was billed doc 03's `water_works` staffing, it drew 60 kW, it took 3×3 of the player's ground, and it supplied **nothing** (measured: `water.nodes.size()` 22 → 22). The ghost could not warn about that because it was checking a different set of rules from the command it was previewing — §2.7's own rule is that a ghost *asks* the command rather than re-implementing it, and the water component cards have obeyed it since Wave 11 while the building card underneath never did. The visible half of the fix is that `E_NO_MAIN` — *"no water main within reach"* — is now drawn on the ghost, in red, **before** the player pays, on exactly the sites where the old card was happy to take $45,000 for a building that could never join the network. |

### Wave 28 — the panel says WHICH transformer rung the next level needs

*(Ruling 93 §BC-3, report 98 §72 RR-223, defect row doc 91 A91-D-144, measured
doc 92 §68.3. One row, three surfaces, one function — `PowerActions.rung_needed`
— because a rung quoted twice is a rung that can be quoted two ways.)*

| # | change | reads | why |
|---|---|---|---|
| D-123 | **When a building's NEXT level needs a bigger transformer than the one feeding it, three surfaces say which rung.** (a) **S5's POWER row** gains one line under the fed-by summary: *"Level 4 needs a level-5 transformer (2.5 MW). This one is level 1."* — `ui_power_row_needs_rung`, drawn in the WARNING state, and only when the answer is not "the one you have", so an ordinary building's panel gains no row. Its siblings are the two cases where a bigger rung under this pad is not the answer: `ui_power_row_needs_second` — *"Level 2 will not fit on this transformer at any rung. It needs a second one beside it — level 3 (400 kW)."* — for doc 04 §2.9's PARALLEL transformer, which `cmd_fix_power_capacity` sells; and `ui_power_row_no_rung` for the one state that really is a wall, where not even a transformer of its own carries the building. (b) **S18's SERVING block** gains a line in every state that has customers at all — `ui_transformer_customers_need_rung` when one wants a bigger pad, `ui_transformer_customer_needs_second` when one wants a pad of its own, `ui_transformer_customer_stranded` when one wants a pad that does not exist, and `ui_transformer_customers_fit` when none does. (A pad serving NOBODY keeps the `ui_transformer_serving_none` heading and gains nothing: "nothing under this wants a bigger pad" is a strange thing to say about an empty one.) (c) **The fix router's `FIX_POWER` answer** carries `needs` — **unconditionally, not behind `with_quote`** — and `ui/ui_root.gd` hands it to S18 as `show_transformer(binds_at, needs)`, so the panel says the sentence for **the building the player came from** rather than for the hungriest customer on the pad. That distinction is not cosmetic: S18's customer list is capped (`customers_hidden`), so the asked-about building may have no row there at all. All three read `PowerActions.rung_needed`: the smallest rung of doc 04 §2.2's ladder whose **nameplate** carries the host transformer's **post-upgrade peak** load — the whole load, siblings and streetlights included, because that is what doc 04 §5.3's gate is judged on. | §2.9 (S5 POWER), §2.25 (S18), §2.7 (`Fix this →`), doc 04 §2.2/§5.3, doc 93 §BC-3 | **The row was honest about the purchase it quoted and silent about the one the player needed.** `cmd_fix_power_capacity` quotes the NEXT rung and only the next, because `cmd_upgrade_grid_component` charges one rung per call, and it says `clears: false` when that rung is not enough — which is true, complete and unactionable. Measured on a `data_center` L2 on an L1 pad (`tools/measure_envelope.gd --panel`, founding city, seed 1337): `action=upgrade_transformer to_level=2 cost=1100 clears=false`, and no surface anywhere said the load actually wanted **rung 4**. A player reading that could only tap, be refused, and tap again. Every other question about a `POWER_CAPACITY` blocker already had an answer on a panel — *which* component binds (Wave 17, A91-D-55), *how far over* it is, *what it carries at the evening peak* (RR-120), *who else is behind it* (Wave 25) — and the one question the blocker is really asking, "how big does this pad have to be?", had none. **The quote itself is unchanged and deliberately so:** a row naming L4 from L1 would price a purchase the verb cannot charge in one tap. And `needs_rung == 0` keeps its own sentence rather than being clamped to the top rung, because *"nothing you can buy fixes this"* is a different sentence from *"buy the top one"* — doc 93 §BC-1 is what guarantees a shipped table never has to say it. **CORRECTED in the fix pass, twice.** (i) The first cut wrote `needs` inside `if with_quote:` and both production callers pass `false`, so the third of these three surfaces reached none: it is unconditional now, and S18 draws it. (ii) The first cut also gave `needs_rung == 0` the sentence *"nothing on the ladder feeds it"*, which is false in a legal, reachable state — a top-rung pad loaded anywhere in [6,075, 6,750) is un-shed and merely past the 0.90 upgrade gate, and every customer under it was told to give up while `cmd_fix_power_capacity` quoted a parallel transformer that cleared for $42,160. `rung_needed` now answers about the BUILDING (`alone_rung`, from `CitySim.building_peak_demand_kw`) as well as about the pad, and the three states are `needs_bigger` / `needs_second` / `no_rung_carries`. Doc 93 §BC-3's rider. *Merge note (2026-09-05): the verifier found no production fix target carried the building id, so `needs` reached no surface twice. Now `BuildController.building_view` hands `sim_id` into `power_headroom_params`, the row's `fix_target` carries it on top, `FixRouter.route(…, false)` fills `needs` from a REAL checklist row, and S5's POWER row itself hands the building to S18 (`_on_power_row_opened` no longer discards it) — `tests/test_ui_transformer.gd::test_a_real_checklist_row_reaches_s18_with_the_sentence_for_its_own_building`.* |

**Preview states: no new screen, no new state.** The line is drawn inside S5's
existing POWER block and S18's existing SERVING block, both of which
`tools/ui_preview.gd --screen=all` already photographs; the deck's state count is
unchanged. Five new string keys (`ui_power_row_needs_rung`,
...
| D-130 | **The ghost PAINTS where it can legally go.** D-127 gave the build sheet the set and the sentence and left `site_hint()["tiles"]` with no renderer. It has one: `ui/site_paint_model.gd` turns the **same memoised window the bar printed from** into instance rows and `game/render/site_paint_view.gd` uploads them as one translucent MultiMesh of 8 m tile quads over the ground — **+1 draw call while a refused ghost is up, +0 the rest of the game** (`game/shaders/site_paint.gdshader`, doc 11 §2.20). Four decisions carry it. **(1) The lit tile is the ANCHOR, not the origin.** `placement_sites` returns footprint origins and the shell's tap is `move_to_ground`, which *centres* the footprint — `origin_for_ground(p) = tile_at(p) − centre_offset()` — so painting the origin of a 3×3 pump would land the ghost one tile up-left of the site that was verified. Each row is drawn on `origin + centre_offset()`, the one tile whose tap produces exactly that origin, and the door the paint implies is the tap `main.gd`'s `_handle_tap` already had. **(2) The mark is the legend's own.** A clean site takes `normal` (`●`), a site §BD9 warns about takes `warning` (`▲`) — `data/ui.json.state_glyphs` rows 0 and 1, so no fifth glyph and no fifth colour, and the colourblind variants carry (`BuildSheet.set_palette_variant`). Hue, mark and the alpha ramp all say the same thing; the mark alone survives greyscale (A5). **(3) The fade is from the GHOST and the ring is on the site the SENTENCE names** — two true statements about one set, and the second is why *"the nearest is 3 tiles away"* points at something. **(4) It is lit exactly when that sentence is printed**, off one flag set by the one line of `_refresh_bar` that prints it, so the ground and the bar cannot drift apart. The memo gains its other half in the same change: `UIRoot._check_placement_ground` throws the scanned window away when a batch carried one of `BuildSheet.SITE_GROUND_EVENTS`, because §BD8's rule is about the finger moving and says nothing about the settlement that just lifted austerity. | §2.5, §2.7, doc 05 §6, doc 11 §2.20, doc 93 §BG1/§BG2 | **A player told there are three spots and not shown which three has been handed a number, not an answer** — and the number is the part the bar could already say. Measured on the founding city's shoreline: **3 spots, all clean, nearest (35, 49) at 3 tiles, $38,086**, three quads and one draw call. On `tests/fixtures/player_save_0903`, from his own plant: **0 legal in 441 scanned**, nothing lit, and the bar's door carrying the camera 41 tiles to the water. `A91-D-162`. |
| D-131 | **The window's price is the price the TILE was quoted.** `BuildController._site_advice`'s `E_FUNDS` branch read `found["cost"]`, which is the cheapest *legal* site's — and in the one branch where nothing is legal that is **zero**. Every founding-city player who opened the water card was told *"a spot 3 tiles away would take this and it costs **$0**; the treasury holds $24.1K."* The `preview = true` quote that raised `E_FUNDS` carries the real number in `params.cost`, so the scan keeps it on the buyable row and the sentence spends it: **$38.1K**, which is also $286 more than the card's own headline ($37,800) because that tile needs a one-tile lateral. `ui_site_funds` is unchanged; only the argument is. | §2.7, doc 03 §2.4, doc 93 §BG2 | **The sentence that exists to unblock a blocked player told him the thing he could not afford was free.** It shipped in Wave 28 with tests, and no test could see it: every one of them asserted the KEY that was chosen and the SHAPE of the arguments, which were both right. It took driving the real shell for a screenshot to read the words. `A91-D-163` — and the third instrument this project has added for that reason (RR-243's preview deck, RR-242's fold, and now `tools/site_paint_preview.gd`). |
