...
> **Measured against a running city for the first time — Wave 8, 2026-08-20, and the band splits in two.** The table above is arithmetic from doc 10 §2.6's worked examples; dispatch has only ever priced ETAs on a Chebyshev stand-in, so none of it had been checked against a live city. It has now, and the measurement is *forward-looking*: §2.10's Wave-8 note explains why the router is not yet the shipped provider, and the right-hand column below is what the game becomes on the day it is. The doc 92 rig, four agents × 3 seeds × 21 game-days, recording every `unit_dispatched.eta_h` and every `incident_resolved.response_min`, run on a pristine copy of the stand-in tree and on the same tree with the router wired.
>
> | | stand-in (shipped) | **router wired** | |
> |---|---|---|---|
> | **dispatch ETA** (turnout + travel — *this* table's quantity), mean | 5.3 – 7.7 gm | **7.0 – 10.0 gm** | +22 – 32 % |
> | dispatch ETA, p90 | 9.5 – 11.6 gm | **11.8 – 14.7 gm** | |
> | dispatch ETA, max | 13.6 – 20.8 gm | **16.6 – 20.0 gm** | |
> | **response** (incident created → first unit on scene), mean | 6.0 – 8.9 gm | **11.6 – 19.5 gm** | +80 – 145 % |
> | response, p90 | 10.0 – 22.4 gm | **33.2 – 58.7 gm** | |
> | response, max | 26.0 – 27.4 gm | **63.6 – 74.6 gm** | |
> | resolved incidents that peaked above tier 1 | 12.5 – 17.2 % | **21.4 – 29.0 %** | |
> | failed · abandoned · destroyed | 0 · 0 · 0 | **0 · 0 · 0** | |
>
> **The ETA band holds.** Street-true routing costs **+22–32 %** on the travel term, which is doc 10's `detour_factor = 1.25` showing up exactly where it should, and every mean and p90 above stays inside this table's 4–14 gm starter band. Doc 10 §1's own calibration check predicted 15.3 gm for a full 384 m crossing on local roads; the measured maximum is 20.0 gm on a city the agents have grown well past 384 m. **Nothing here needs retuning.**
>
> **The RESPONSE band does not hold, and the reason is not travel.** Response is `created → first on scene`, so it carries the wait for a free unit as well as the drive, and it rose **three to four times as much as the drive did**. A trip that is 25 % longer is 25 % longer in both directions plus the same on-scene time, so an eight-unit founding roster spends materially more of its day committed, and the queue in front of the next incident grows superlinearly. That is a **fleet-sizing** result, not a routing one.
>
...
`grace_days` suppresses it for the first N game-days so the scripted `TUT_TRANSFORMER_FAIL` incident (`data/incidents.json` `tutorial`, requested through `director.request_scripted_incident`) is still the first one a new player ever meets. Only channels with a live candidate source carry a row. ~~`water_main_break` and `traffic_accident` are **omitted** because their candidate sources are unimplemented (doc 92 §18.2, D-14/D-15) and a floor row for them would be dead data~~ — **superseded 2026-08-19 (Wave 7): both candidate sources landed** (`CityIncidentWorld.water_mains()` / `road_intersections()`, doc 91 **D-17/D-18**, renumbered from D-14/D-15), so the floor now carries **five** rows, not three, and `data/incidents.json` authors them. The budget did not move — doc 92 §18.2 ruled in advance that the two new rows come out of the three existing ones (0.20/0.10/0.10 → 0.14/0.08/0.08 + 0.06/0.04, still 0.40/game-day). `storm_damage` is still omitted permanently, because its candidates exist only inside a live doc 07 storm cell. Authored values and the measured A/B: doc 92 §18.3, re-derived across five channels in §18.6; gated by `tests/test_balance_gates.gd::test_gate_19_*`.
...
**All FOUR money terms above are doc 03's** *(report 98 C-07 / C-16 / R-14, and now **RR-78**).* Doc 06's `repair_material_base` column was deleted in Round 1; **`reward_base` is deleted here.** It lived on every type row of `data/incidents.json` and it was a price, which made it the last dollar column standing outside doc 03's currency monopoly — and it was the same dollar as doc 03's own `POLICE_FINE_PER_RESOLVED_INCIDENT = 350`, which is precisely what open question 6 below asked about in Wave 1. **The ruling is RR-78 and the question is CLOSED.** The six values are unchanged and now live in `data/economy.json` under `city_services.dispatch_payout_base`; `IncidentCatalog.FORBIDDEN_KEYS` refuses a doc-06 data file that carries the key back.
...
**The ceiling BINDS on this doc's shipped curve, and that is a finding about §2.7 rather than about doc 03** *(doc 92 §36.3).* `tier_k = 0.35` grows the payout faster than doc 02's residual-damage curve grows the damage (`0.10` per tier), so on a cheap building the reward outruns the value at risk from about tier 4 up. House L1, capital $1,200, tier-5 fire answered at the target response: reward `900 × 2.40 = $2,160`, residual damage 0.40 ⇒ repair `$408`, loss prevented `$792` — **2.73×**. Doc 03 clamps it to `0.75 × 792 = $594`. Nothing in this doc was retuned to fix that; the ceiling is what fixes it, and balance gate 31 is what keeps it fixed.
...
*Worked (recomputed on doc 03's ladder, R-14; re-checked against the RR-78 ceiling):* tier-3 structure fire in a **house L2**, one engine, on scene at minute 5, resolved with `burn_timer = 0`.
- `speed_bonus = clamp(1.5 − 0.5·(5/6), 0.60, 1.50) = 1.083333`
- `shape = (1 + 0.35·2) × 1.083333 = 1.841666`
- `reward = 900 × 1.841666 × 1.00 (auto-dispatched) = **$1,657**` — **unchanged**, which is the point of RR-78's "moved, not retuned"
...
>
> **What was wrong.** `sim/incidents/travel_time_provider.gd` shipped as *both* the interface and a stand-in implementation — a Chebyshev-distance model in whole game-seconds — and `sim/roads/road_travel_time_provider.gd` shipped the real thing, tested, **and nothing constructed it**. Worse, the real thing overrode only `travel_gs` and `route_tiles`: doc 10 §7 publishes **four** things to doc 06, and `access_quality`, `congestion_index` and `condition_hazard_mult` were still answering with the pre-router constants 1.0 / 0.0 / 1.0 behind a provider whose entire purpose is that it knows the streets.
>
> **What shipped.** The seam itself, complete and tested: all four published inputs overridden and unrescaled (C-48 / C-61); the tile→edge join (`RoadNetwork.congestion_index_at` / `condition_hazard_mult_at`) so the three per-position answers describe the same piece of street; **doc 10 §2.14's two-step contract implemented in §2.10's assignment loop** — rank every eligible unit on the O(1) `estimate_eta_practical` carrying the same reassign / reserve / role-fit penalties, then pay a real quote for the top `DISPATCH_CANDIDATES = 3`, with the quote budget published *through the seam* (`TravelTimeProvider.dispatch_candidates()`, **0 = quote everyone**, which is what doc 06's own stand-in answers, so pre-router ranking is reproduced tie-break for tie-break); and `_boot_roads()` moved ahead of `_boot_incidents()` so the router exists when the incident system is built. Wiring it is now literally one argument.
>
> **What is HELD, and the measurement that holds it.** `CitySim` still constructs `IncidentSystem` with the stand-in. Wave 8 wired the router, ran the doc 92 matrix, and took the wiring back out:
>
> | `greedy_growth`, seed 4242, 21 game-days | stand-in | **router wired** |
> |---|---|---|
> | wall clock | **12.6 s** | **> 20 min** |
> | open incidents, game-days 11–21 | 0 – 4 | 42 on day 17, **still climbing on day 18** |
>
> **The cost is not the router.** Five mitigations were implemented and measured, and not one of them moved the cliff: rank-then-quote (planner misses 589 → 197 per 24 coarse hours on the benchmark city), an allocation-free top-K candidate buffer, a per-pass quote allowance, an epoch-keyed skip for incidents nothing can reach, a memo on `access_factor`, and a 16× route cache. What changes is that **a rotting city's incidents stop being ANSWERED**: real ETAs across a collapsed, flood-closed road network push `eta + penalties` past `MAX_ACCEPTABLE_COST = 90`, and §2.10 has **no terminal rule for an incident no unit can answer** — it is re-queued forever. The backlog grows without bound and the integrator's per-sub-step cost grows with it. In the Chebyshev world that state was unreachable, so nobody had to write the rule.
>
> **Two rulings are needed before the argument goes in**, and neither belongs to an implementation branch:
>
...
RR-93 (both FINE baselines moved, both COARSE ones did not, because the coarse
path never spawns).

**What v1 does NOT do, and where each would go.**

- An unanswered `petty_crime` expires **silently, and that is now a RULING
  rather than an omission** (doc 93 §V1, sim q5). `expire_stability_delta` is
  ...
  promises not to punish. Generalised in doc 93 §V1 as *an attention reward may
...
"street": {
  ...
  "live": [ {
    ...
    "born_gm": 1358.0,              // the same instant in game-MINUTES — doc 11 §2.17 anchors
                                    ...
                                    // roster mid-stride instead of restarting it (RR-93)
...
9. ~~**Incident cap.**~~ **ANSWERED YES, and shipped — CLOSED 2026-08-21 (the answer landed in Wave 13; this question was never struck).** The original text is kept because its diagnosis was exactly right and its severity estimate was not: ~~Should there be a hard ceiling on simultaneous active incidents (e.g. 60) in addition to `incident_load_damper`, to bound offline cost on very large cities? None added; the damper alone is asymptotically self-limiting but not bounded. Note the offline budget is now 720 game-hours (C-19), not 480, which makes the question slightly sharper.~~ **The ceiling exists as of Wave 13 and it is §2.13(b) above** — §2.13's own "≤ 40 active" made binding on every AUTOMATIC birth, with ambient generation tapered from a knee placed at the worst backlog the matrix had ever measured. It is not a cap of 60 and not a cap on the roster: the eight `spawn_incident` rows in `data/incidents.json` were the fertility this question could not see, and a branching process with mean offspring three does not need a large city to run away — it needed only 104 game-days of `do_nothing` on `crisis`, where the roster went `103 → 89,055` in eight game-hours. **`tests/test_balance_gates.gd::test_gate_30_a_decayed_city_roster_is_bounded` runs a 200-game-day city on purpose and asserts the bound**; measured peak after the rule is **37** open. Doc 92 §31, doc 93 §M1, report 98 RR-62, audit 91 **A91-D-35**. *(The "very large cities" framing was the one thing this question got wrong, and it is worth keeping as the lesson: the danger was never the size of the city, it was the number of children per incident.)*
...
### Wave 13 — the saturation rule (doc 92 §31, doc 93 §M1, report 98 RR-62, audit 91 A91-D-35)
