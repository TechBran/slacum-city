...
### RR-60 — One difficulty knob per ledger line, never two; and a summary-table row label does not outrank the two formulas that define the knob (docs 03 §2.4/§2.2/§2.9, 05 §9, 91 §17.2, 92 §29.2/§29.3/§29.5/§31, 93 §M1–§M4)
...
**Ruled (doc 93 §M1): `E_roads_repair` takes `M_repair` and is excluded from the
...
advertised −15 % measures **−13.2 %**. **Ruled the other way (doc 93 §M2): the
...
§M3): `cmd_install_backup_generator` is doc 05's interface call, not a player
...
92 §31.1) — the four state hashes on both cities and both paths, all 63 cells of
