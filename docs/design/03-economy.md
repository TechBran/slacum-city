...
**What this buys, measured** (doc 92 §73.2). A 9-tile `service` main is **$2,574** (9 × $286; the merge verifier re-drove the cited test's own run) and takes one building's doc 05 §2.3 tile factor from **0.40 to 1.00**; a 3-tile `trunk` at the plant is **$2,412** and adds **213.0 m³/h** to §2.5's `feed_capacity` — the one term of the supply chain no node purchase can move.
