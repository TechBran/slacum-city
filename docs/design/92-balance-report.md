...
## 23. Pass 9 — the top of the ladder (Wave 10)
...
and measured in §23.8 and §23.11.

### 23.1 The hollow ceiling, measured
...
### 23.2 What was added, and where the line is
...
### 23.3 The reward-pacing ruling — where each sixth rung opens
...
### 23.4 The fit — the sixth row is the fifth row's curve, one step further
...
### 23.5 The meshes — through the same pipeline, and the L1–L5 hashes prove it
...
### 23.6 The seventh rung of the population ladder
...
measured, and §23.9 measures it.

### 23.7 The sixth curriculum row
...
### 23.8 The finding — the tower tier is a POWER purchase, and the student had to be told
...
every L1–L5 game-hour in §23.9's table is the same on all three seeds before and
after it.
...
### 23.9 The curriculum's pacing, re-measured — 3 seeds × 45 game-days
...
rather than the ~2× the first five settle into, and §23.8 is why: it is a saving
...
### 23.11 What moved that was not content
...
2. **`Curriculum`'s copper habit** — §23.8, fires only on `upgrade_to_level`.

### 23.12 Hashes — the starter city does not move, and the bench city moves through EXACTLY ONE KEY
...
commands no upgrades at all, so §23.11's fallback cannot fire either.
...
### 23.13 The frame — the Z2 budget did not move by a single primitive
...
### 23.14 The matrix — 18 of 18, and not one row moved
...
nothing in the matrix moved — which is also why §23.9's `curriculum` run is the
...
### 23.15 What this pass did not do
...
- **It did not extend `water_facility`, the two stations or the two grid shells.**
  §23.2 — those ladders are docs 05 and 06's.
- **It did not fix the `upgrade_time_hours` off-by-one.** §23.11 — reported.
...
- **It did not put `curriculum` in the default matrix.** §22.2 still stands, and
  §23.14's matrix is the six-strategy one this document has always published.
