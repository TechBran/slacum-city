...
## 39. Pass 15 — street polish: one persisted field, and the ding that stays at zero (2026-08-21)
...
### 39.1 One field, two baselines, and both predictions made before the run

Doc 06 §2.16's opportunity row gained `born_gm`, the spawn game-minute (report
98 RR-88 has the argument). **A payload field is not hashed** — `state_hash` is
...
### 39.2 `expire_stability_delta = 0.0` — a balance number that is deliberately not a balance number
...
fit** — doc 93 §U1 rules it zero for v1 on design grounds (an attention reward
...
### 39.3 What this pass costs, for the record

Frame budget, not money, and most of it is a refund. **−13 draw calls at every
pose on a quiet city** (report 98 RR-87: eight empty `VehicleView` buffers and
