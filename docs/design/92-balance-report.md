...
## 33. Pass 13 — the crisis purse is ruled, and two published tables are re-taken at a clean fork (2026-08-21)
...
### 33.1 The founding purse, re-taken and turned into the ratio the ruling needs
...
### 33.2 The ruling — `crisis` keeps its purse, and 0.729 is the knife-edge
...
1. **`crisis` founds POSITIVE.** +$44.47/gh (§33.1). The purse is not what pays
...
four assertions stay word for word what §29.6 shipped. §33.2 item 2 is a
...
### 33.3 §28.2-class hygiene — the two cheapest published tables, re-run at this fork
...
**(a) The founding ledger — reproduces to the cent.** §33.1. Twenty-eight
...
**(c) The insolvency rows — reproduce seed for seed.** §33.2 item 2. `crisis`
...
### 33.4 What this pass did not do, and what it leaves ranked
...
- **It did not retune a digit of `data/difficulty.json`**, and that is now a
  ruling (§33.2) rather than an omission.
...
2. **`Balanced`'s game-hour 0 is the only hour its reserve floor binds** (§32.4,
   §33.2). A harness question, and the smallest thing on this list.
...
4. **The expensive tables have never been re-taken on a merged tree** (§33.3).
