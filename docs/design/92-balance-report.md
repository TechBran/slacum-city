...
## 35. Pass 14 — the money the ledger could not see, and the radius of a finger (2026-08-21)
...
### 35.1 The bounty table doc 06 has been paying all along
...
* **It is not small against the hourly ledger, and the bar is low.** One tier-3
  ...
  (D-6). The point stands either way: A91-D-33 is that none of this money has
...
### 35.2 The toast floor, and why it is 25
...
### 35.3 48 dp of finger, in metres
...
0. **`revenue.bounties` and `revenue.street` belong in `EconomySystem.
   settle_hour`** (A91-D-33). The UI tallies them off the bus today and the
