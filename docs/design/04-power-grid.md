...
**As surfaced (Wave 11) — `route_feeder` is two cards on doc 12 §2.7's drag-path tool** (doc 93 §J2, measured in doc 92 §27). The verb waited five waves for a door because putting it on a card is a balance change: §17.3 named the 2 × 1,200 kW ceiling as the late-game's binding constraint, and a player who can widen the trunk plays a different late game. Four things about the surface, and each is a ruling rather than a layout choice:
