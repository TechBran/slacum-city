...
Its roster lives in `data/grid_components.json` under a new **`routable`** key, kept separate from `placeable` because a line is not a point: `placeable` is the one-tile, one-tap, one-build-sheet-card roster, `routable` is priced per tile and drawn as a path, exactly as doc 10's `cmd_place_road` and doc 05's `cmd_place_water_main` are. Like those two it had **no build-sheet card** until doc 12's drag-path tool took it (Wave 11 — see below); the verb was live and tested underneath for five waves. Boot asserts every listed conductor class is one §2.2 rates (the P0-01 pattern). §2.1 source connectivity resolves to a **substation with a free §2.2 slot**, preferring the pad the assist aimed at over a trunk underfoot, and `E_NO_SLOT` is the blocker that turns "I need more copper" into "I need another substation".
...
**As surfaced (Wave 11) — `route_feeder` is two cards on doc 12 §2.7's drag-path tool** (doc 93 §J2, measured in doc 92 §28). The verb waited five waves for a door because putting it on a card is a balance change: §17.3 named the 2 × 1,200 kW ceiling as the late-game's binding constraint, and a player who can widen the trunk plays a different late game. Four things about the surface, and each is a ruling rather than a layout choice:

- **The tab is `infrastructure`, beside the transformer the run roots** — not `roads`. Doc 12 §2.7 files a run card by what it is made of, not by the tool that draws it, which is why doc 05's mains sit there too.
- **One card per conductor class** (`Feeder` class 1, `Heavy Feeder` class 2), read from `routable.feeder.conductor_classes` so a class this build does not ship never gets a card and `E_CLASS_UNAVAILABLE` stays a defensive branch. Class 3 is still deferred here and therefore still absent there.
...
**Wave 1.5 placement tests** (`tests/test_player_verbs.gd`, shipped):

26. `test_transformer_unblocks_the_tutorial_lot` — doc 09's `tutorial_lot_a` refuses a house with `E_UNSERVED`; one `cmd_place_grid_component("transformer", …, 1)` inside its L1 radius charges `500 + lateral × 110`, the same command re-serves the lot, and the house placed there is attached, energized and lit. **This is the tutorial's connect-power beat, end to end.**
27. `test_grid_placement_rejections` / `test_grid_placement_needs_a_feeder` / `test_grid_placement_not_developed` — every code in §4's shipped order fires on its own case, including `E_UNKNOWN_COMPONENT` for `substation` and `feeder` (C-30: a substation is a *building*), and `E_NO_FEEDER` when every feeder is FAILED.
28. `test_feeder_tap_radius_covers_the_authored_city` — every transformer doc 09 authored sits within `feeder_tap_radius_tiles` of some feeder route, and the worst of them is **exactly** that radius; and the radius equals half a land block. The number is therefore pinned by shipped data twice over, not chosen.
29. `test_grid_rules_mirror_the_power_ladder` — `data/grid_components.json`'s service-radius column equals §2.2's, and a drifted column is a **boot error**, not a silent mismatch (the P0-01 pattern).
30. `test_transformer_adopts_orphaned_buildings` — a building orphaned by a FAILED transformer is re-attached by the next transformer that covers it, inside the same command.
