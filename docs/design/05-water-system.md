...
Additional rules:
- **Upgrade gate (spec §9.4):** doc 02 refuses an upgrade if the building's tile pressure `P_tile < upgrade_min_pressure (0.55)` or if `water.zone_headroom_m3h(building_id) < delta_water × 1.10` (doc 02's check **E_WATER_HEADROOM**, §2.11 there). `zone_headroom_m3h(z) = max(0, S(z) − D(z))`. The answer names the arm that refused (`limit`) **and the remedy** (`reason` ∈ `BLOCKED_WATER_DISTANCE` / `BLOCKED_WATER_CAPACITY`), because the two want opposite purchases — §4 and doc 93 §BH.
  - *(Wave 30: this bullet read **"the building's 30-game-day average `P`"** from Wave 5 until doc 93 §BH struck it. No such average has ever existed in `sim/water/` — `can_upgrade_water` reads `pressure_at(tile)` live — and it is struck rather than implemented, because an average would leave the refusal standing for thirty game-days after the player laid the main that answers it. §10.)*
...
Water **provides**: every per-variant number for `water_facility` — `base_kw`, `kw_required`, capacity/volume/head, `coverage_frac`, and the per-variant footprint of the four non-reference variants (§8); `get_water_service(building) -> {pressure, output_mult, happiness_penalty, contaminated}`; `zone_headroom_m3h(building_id)`; `can_upgrade_water(building, delta_water_m3h) -> {ok, reason, deficit_m3h, limit, zone_key, pressure, zone_pressure, main_distance_tiles, tile_factor}` (doc 02's check **E_WATER_HEADROOM**), where **`limit` ∈ {`none`, `no_zone`, `capacity`, `pressure`} is the LINE that refused** and **`reason` ∈ {`BLOCKED_WATER_CAPACITY`, `BLOCKED_WATER_DISTANCE`} is the REMEDY** — supply, or a main laid closer. Neither derives from the other: `no_zone` and `pressure` each split across both remedies (Wave 30, doc 93 §BH, §10). `pressure_remedy_at(tile)` is the one place that split is decided, and every reader — the checklist row, the building panel's water row, the harness agent — asks it rather than re-deriving it.
...
### Wave 28 — the whole system audited, and §2.8's own sentence made reachable (report 98 §73)

*(Rulings doc 93 §BD. Measured doc 92 §69, on `tools/measure_water_chain.gd` and
on a real player save. Defect rows doc 91 A91-D-145..A91-D-149. Surfaces doc 12
§2.26 D-124..D-126. **No constant, ladder, curve, threshold or worked example in
this document changes.** Every amendment below is a rule this doc already wrote
being made reachable, or a read this doc already implied being published.)*

| Ruling | Change |
|---|---|
| **RR-224** | **§2.12's repair gets a player verb, and this doc still authors no dollar.** §2.12 has published `damage_fraction` and `work_minutes` since C-16 and says plainly *"the system exposes no cost method"*; doc 03 §2.5 has published `repair_cost = capital_value × damage_fraction × 0.85 × M_repair` and doc 03 §8 `water` the two capitals — and **nothing in the project ever called either of them** (doc 91 A91-D-148). The coordinator verb is `CitySim.cmd_repair_water_asset(asset_id, preview)`, which serves BOTH of §2.12's target kinds because this doc's own job table already keys on `target_kind ∈ {edge, node}`. It reads this doc's `damage_fraction` (§2.12's own conversion, and at least `1 − condition` for a standing asset, which is §2.12's `overhaul`), this doc's `repair.base_minutes` work content and this doc's `post_repair_condition`, and hands doc 03 the rest. **A `junction` is not an asset** (§2.1: it is where mains meet), so a crew cannot be sent to one. |
| **RR-225** | **§2.8's hold is released on EVERY terminal exit of doc 06's incident, not only on resolution — and the sentence that says so was already in §2.8.** §2.8 rules that doc 06's tiered `zone_pressure_delta` *"is held until the incident resolves"* and, four lines later, that this doc's `break_pressure_penalty_fallback` fires *"only for a broken segment with **no owning doc-06 incident**"*. Doc 06's lifecycle has three exits and only RESOLVED reached `set_segment_repaired`, so a break nobody answered left `WaterEdge.owning_incident` naming an incident that had been erased, with the fallback unreachable behind it. **This doc's reading is that a terminal incident is not an owning incident**, and the verb is `WaterSystem.release_segment_incident(edge_id)`: it drops `owning_incident` and `incident_pressure_penalty` and **leaves `state`, `severity` and the leak untouched** — the pipe is still broken, which is the consequence of nobody coming. §2.8's own `0.12 × severity` then applies, which is the number this document authored for exactly that case. Doc 06's call site is its own `_release_finished_units`, filed to doc 06 rather than amended here. **Measured** (doc 92 §69.1): three mains on a real save, three held 0.80s clamped to `break_penalty_cap`, a zone pinned at P = 0.50 with 31.3 m³/h spare and 89 of 89 buildings under doc 02's 0.55 gate. |
| **RR-226** | **§2.11's upgrade gate answers WHICH of its two arms refused, and a zero-delta ask is not refused at all.** `can_upgrade_water` returns `limit` ∈ {`none`, `no_zone`, `capacity`, `pressure`} beside the existing `{ok, reason, deficit_m3h}`, with the zone key, the tile's pressure, the zone's pressure and the tile's distance to the nearest live main. **The two arms want opposite purchases**: capacity is answered by the stage §2.5's chain binds on, and pressure is answered by a main laid closer, because §2.3's factor is distance and no supply moves it. And `delta_water_m3h <= 0.0` returns ok before the zone is looked for — this doc's twin of doc 04's Wave-17 rule (*"a building that adds no load cannot overload anything"*), which doc 02's water column being 0.0 for a `substation` and a `power_facility` made necessary and nobody wrote (doc 91 A91-D-139). Neither is a change to §2.11's thresholds: `upgrade_min_pressure` 0.55 and `upgrade_headroom_safety` 1.10 are exactly as authored. |
| **RR-227** | **§2.5's chain is published as ONE read, and the stage that binds is named.** `WaterSystem.supply_chain(zone_key)` returns the four terms of §2.5's supply expression, the two numbers they produce, the binding stage, the stage that would bind next, the node ids of the binding stage (cheapest rung first) and `stranded_m3h`. **No new arithmetic**: every term is §2.5's own, computed the way `WaterTopology._resolve_supply_chain` and `_solve_zone` already compute it. It exists because §2.5's supply is four numbers wearing one name and until this wave three separate readers each re-derived a subset — which is what doc 92 §67.4 measured 118 pumps being bought for the want of. `SUPPLY_STAGES` fixes the four names (`source`, `treatment`, `pump`, `mains`), and `mains` is `feed_capacity` — the only term in §2.5 that is not a node. |
| **RR-228** | **§6.1's "there is no water-node panel" is withdrawn.** That ruling (Wave 11, doc 93 §J1) argued from a verb list of three, and this wave's list is six: upgrade, repair, remove, isolate, restore, and a CHAIN read that belongs to the zone rather than to any building. Doc 93 §BD1 records the overturning and doc 04's identical precedent (§AY2, Wave 25). §6.1's two surfaces are kept and not duplicated — the shell's node block still draws on S5, the drawer's valve row is unchanged — and S19 draws the valve pair only on a break row, **because the drawer cannot**: a break whose incident has gone terminal has no drawer row, which is the state A91-D-145 found three of. |
| **§6.1's roster line, corrected** | §6.1's *"The surfaces, as shipped (Wave 10)"* paragraph is accurate and is restated here against doc 92 §67.9 item 2, which reads *"nothing in `tools/playtest.gd` lays a water main … `cmd_place_water_main` is probed, listed and never called"*. **That is true of the harness and not of the game**: `ui/path_tool.gd` has carried `water_main_service` and `water_main_trunk` on the Utility tab since Wave 10 and reaches the command directly. The player can lay a main; the scripted agent cannot. Recorded so a later lane does not build a second door beside the one §6.1 already describes. Doc 92 §69.5. |
| **§6's MVP ladder — examined and NOT raised** | Doc 93 §BD6. At the top of the shipped roster (`source` L2 = 253.5 after `cond_factor`, `treatment` L2 = 196.0, `pump` L3 × 2 = 480.0) the binding term is **`feed_capacity` = 214.0**, four `service` mains at the plant — the one term in §2.5 that is not a node. Raising `treatment` to L3 (480) moves the binder to 214.0; raising `source` to L3 (642) leaves it at 214.0. The purchase that moves it is a **trunk main** (213 per segment), which §6.1 already offers. A `placeable_levels` change would therefore buy nothing measurable while moving every doc 92 row fitted to the curriculum arc. Filed as the open question for the lane that teaches the agent to lay a trunk main: raise the ladder and the harness together, and measure them together. |
| **§2.5's share split — CORRECTED (A91-D-149)** | §2.5 says the upstream capacity is *"split among pumps in proportion to `rated_flow_m3h`"*, and the implementation split it over the LIVE pumps on topology change while whether a pump RUNS is decided per tick (power fraction, §2.6's restart lockout). A pump that was dark, tripped or in its lockout kept its slice and delivered none of it. **This doc's reading is that §2.5's sentence is about sharing what the chain makes among the pumps that can move it, and a pump that is moving nothing is not one of them** — so `_solve_zone` re-splits the SAME `upstream_cap` over the running set. `share_m3h` keeps its topology-time value: it is what §3.2 persists and what `WaterTopology` publishes, and with every live pump running the two figures are identical, which is why both shipped cities' `profile_sim` baselines do not move. **Measured** on the curriculum arc at game-day 25: a zone in deficit at supply **128.6** against demand **135.3**, with `source 257.3 / treatment 385.9 / pumps rated 480.0 / feed_capacity 214.0` and **105.5 m³/h — most of its own chain — stranded**; after, supply **214.0** (which is `feed_capacity`, the honest binder) and headroom **68.4**. `supply_chain` still publishes `stranded_m3h`, now normally zero, so the next regression of this shape is a number on a table. Doc 92 §69.4. |
| **§6's placement quote — THREE ADDITIONS, no new rule (Wave 28 fix pass)** | `cmd_place_water_component`'s twelve-check order gains nothing and loses nothing; its QUOTE gains three things it should always have carried. (a) **doc 03 §2.10 layer 2**: the quote now refuses `E_AUSTERITY` where the commit would, instead of quoting `valid` and letting `Treasury.spend` say no — doc 12 §2.7's own rule that the preview and the commit are one code path (doc 91 A91-D-151, report 98 RR-233). The same arm goes on `cmd_place_water_main`; `cmd_repair_water_asset` deliberately gets none, because its category is `repair` and layer 2 does not block it: a city in austerity must still be allowed to stop a leak. (b) **doc 04 §2.1's capacity read**, as a WARNING and never a refusal: the quote publishes `power_ok` / `deficit_kw` / `transformer` / `ratio` from `PowerGrid.can_serve_tile` at the site's own `kw_required`. §6 has always required a transformer to REACH the site (`E_UNSERVED`); it has never asked whether that transformer can carry 60 kW of pump, and doc 92 §69.11 measures one that could not — `power_fraction 0.00`, `flow_m3h 0.0`, for a game-week (A91-D-153, RR-234b). (c) **the shell is born at `pending_level`, not `level`**, so a site under construction is doc 02 §2.12's NEW BUILD and a fire inside its build window destroys it rather than stranding its node `offline_manual` for ever (A91-D-152, RR-234a). |
| **§6 gains a READ, and it is the one the player asked for** | Doc 93 §BD8. §6 has always said which tiles a component may stand on; nothing in the project could answer WHICH TILES THOSE ARE. `BuildController.placement_sites()` runs the shipped preflight over a window and returns the set, the refusal histogram and the cheapest purchase that opens one — doc 12 D-127. **No siting rule in this document changes**: the window's legality test is `cmd_place_water_component(preview = true)`, unchanged, called once per tile. Measured doc 92 §69.10 on the player's own committed save: *"create a water source"* went from no legal tile within forty to three within ten, and the whole source → treatment → pump chain costs **$204,677** and takes one game-day. |

### Wave 30 — the refusal names the truth, and the agent can lay the pipe (report 98 §77)

*(Ruling doc 93 §BH. Measured doc 92 §73, on `tools/measure_utility_plan.gd` and
on the player's committed save. Defect rows doc 91 A91-D-165..A91-D-168.
**No constant, curve, threshold, ladder or worked example in this document
changes.** `upgrade_min_pressure` is 0.55 and `upgrade_headroom_safety` is 1.10,
exactly as authored, and doc 93 §BH is the derivation that says so.)*

| Ruling | Change |
|---|---|
| **RR-250** | **§2.11's gate names its REMEDY, not only its arm, and §4's published `reason` stops lying.** §4 has said since Wave 5 that `can_upgrade_water` answers `reason = "BLOCKED_WATER_CAPACITY"` — the ONLY value it could return — and doc 92 §67.8 measured a level-7 high-rise refused with that word inside a zone at pressure 1.00 holding 57.1 m³/h spare. The wall was §2.3's per-tile factor, which is Chebyshev steps to the nearest live main, and the sentence in front of the player said *buy supply*. `reason` is now one of two: **`BLOCKED_WATER_DISTANCE`** (§2.3's factor is the wall — lay a main closer) and **`BLOCKED_WATER_CAPACITY`** (the zone is short — raise the stage §2.5 binds on). RR-226's `limit` is unchanged and is NOT a second spelling of it: `limit` says which LINE refused and `reason` says what to BUY, and neither folds into the other — `no_zone` splits across both remedies (a tile out of every main's reach wants a main; a zone whose supply nodes are all dead wants supply) and so does `pressure` (a tile under the gate inside a zone that is above it is distance; inside a zone that is under it too, it is the zone's shortage). §4's interface line is amended to match. |
| **RR-251** | **The classification lives in this doc, once.** `WaterSystem.pressure_remedy_at(tile)` is the single rule, and it exists because three UI surfaces were each deciding it with a predicate of their own: `ui/build_controller.gd` on `limit == "pressure"`, `ui/water_panel_model.gd` on `pressure < gate and zone.pressure >= gate`, and `ui/requirement_formatter.gd` on a third arrangement of the same two tests. The three could disagree with each other and with the gate — measurably: a tile at 0.32 inside a zone at 0.50 is a CAPACITY refusal that the panel row called *"too far from a main"*. All three read `reason` now. Doc 91 A91-D-165. |
| **RR-252** | **The `no_zone` refusal says HOW FAR, and doc 12's row can print it.** `topology.distance_at_tile` answers `−1` outside §2.2's BFS cutoff, because the BFS stops there — so the one refusal whose whole content is *"no main reaches you"* was the one that could not say a distance. The gate falls back to this doc's own whole-map `nearest_main_tile` **on the refusal path only** (O(main tiles), once per refused upgrade), which is the read `CitySim.cmd_place_water_component` already does for `E_NO_MAIN`. Doc 91 A91-D-166. |
| **§2.11's "30-game-day average P" — STRUCK** | §2.11's upgrade-gate bullet has read *"the building's 30-game-day average `P`"* since Wave 5, and **no such average has ever existed** anywhere in `sim/water/`: `can_upgrade_water` reads `pressure_at(tile)` live, and `WaterServiceLedger` keeps an HOURLY factor for doc 03, not a monthly one for this gate. Doc 93 §BH rules the sentence struck rather than implemented, and the derivation is doc 12 §2.7's: an average would leave the refusal standing for thirty game-days after the player laid the main that answers it — a door that does not open. The bullet now reads *"the building's tile pressure `P_tile`"*. **No behaviour changes**; a claim the code never made is withdrawn. Doc 91 A91-D-167. |
| **§2.3's gate READS THE TILE, and the alternative is refuted rather than declined** | Doc 93 §BH's ruling in full. The tabled alternative was to gate on the ZONE's pressure with the tile factor applied as a discount on the delta the building asks for. §2.4 refutes it: the demand aggregator sums `W_b` **undiscounted** — a far building draws its full doc-02 magnitude out of the zone, and §2.3's factor is a DELIVERY term, never a demand term — so a discount would invent a rule §2.4 does not have. And it fails at the boundary: at `d > max_service_distance_tiles` the factor is 0.0, the discounted ask is 0.0, and a high-rise with no water at all would clear a headroom check. The gate stays on `P_tile` at 0.55, which is also where §2.11's own effect table puts it — one band under the 0.60 "Normal" row, i.e. *a building already below normal service may not be given more load*. |
| **§6.1's roster line, closed** | Wave 28 recorded that the PLAYER could lay a main and the scripted agent could not. `tools/playtest.gd` lays both tiers now (`Api.water_main_run` / `place_water_main`, `Balanced._lead_mains`), through the same `cmd_place_water_main` §6.1 already describes and with no second door added. The route is this doc's own `WaterSystem.lateral_tiles`, so an agent's main and the game's component lateral are the same tiles and the same price over the same span. Doc 92 §73.2, report 98 §77 RR-253. |
| **A main is a DEMAND purchase, and that is now measured** | Not a rule change — a consequence of §2.2 and §2.4 that no section had ever stated and that cost a measured quarter of a city. §2.2's BFS enrols every tile within `max_service_distance_tiles` of a live main tile, and §2.4 bills the zone for every building whose access tile lands in it. So a run laid into unserved ground moves a neighbourhood's demand onto a works that was not sized for it. Measured over 45 game-days with an agent that laid mains on the refusal alone: seed 1337 ended **8,559 → 6,283** residents and its zone **1.00 → 0.21** pressure. The harness rule stands its SERVICE arm down while the zone is at or past doc 04 §5.10's `OVERLAY_WARNING_R`, **and lays a main only for a building that is ALREADY IN the zone** — §2.11's `pressure` arm, whose demand §2.4 is already billing — never for one that no main reaches at all, which is the run that enrols a district. It takes no such guard on the TRUNK arm: a zone that binds on `mains` is short by definition and the trunk is the remedy. Doc 92 §73.4. |
