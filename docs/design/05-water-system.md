...
| **§2.5's share split — a hole recorded, not closed** | §2.5 splits `upstream_cap` among a zone's pumps *"in proportion to `rated_flow_m3h`"*, and the implementation does it on TOPOLOGY change while whether a pump runs is decided per TICK. A pump that is dark, tripped or inside its `pump_restart_minutes` lockout is `is_live()` and keeps its share, so the pumps that could use it never see it: a zone with an L1 (40) and an L3 (240) pump on 100 m³/h of upstream splits 14.3 / 85.7, and tripping the L3 delivers 14.3 where the chain could carry 40.0. `supply_chain` publishes it as `stranded_m3h` so it can never again be lost inside a difference of two numbers, and `tools/measure_water_chain.gd --losses` prints it as the one row in the volume ledger with no section behind it. **0.1 m³/h on the measured save** (two same-size pumps), which is why it is filed rather than fixed: the fix is a tick-time re-split and it is hash-affecting. Doc 91 A91-D-149, doc 92 §69.4. |
