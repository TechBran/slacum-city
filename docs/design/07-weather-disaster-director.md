...
| **10** Roads, routing & traffic | road graph edges, per-edge `condition`, `road_redundancy`, `roads_impassable_pct`, the per-edge `accident_hazard(e)` rate | **the city-wide `get_state()` string** — doc 10 §8's table is indexed by it and derives its own `slowdown` / `cong_add` / `wear` rows from it; **`wx_slowdown() = 1 − road_speed_mult`** (doc 10 §2.11 consumes exactly this), per-edge flood speed multiplier, `flood_saturation(tile)` for its ground term, edge removal at ≥350 mm, debris road closures. Weather is **global**; roads samples one city-wide state (C-59) |
...
> #### Wave 14 — doc 10 became a real consumer of `get_state()` (report 98 RR-69, doc 91 A91-D-32)
>
> The row above described a publication with no subscriber. `RoadNetwork.weather_state_of` was a `Callable` field **`CitySim` never assigned**, so roads' `weather_state` was the literal string `"clear"` for the life of every city ever played and doc 10 §8's eleven weather rows were unreachable: `wx_cong_add` 0.00, `wx_slowdown` 0.00, `wx_wear_day` 0.00. **Rain had never slowed traffic and no road had ever worn faster for being wet.**
>
> The wire is `CitySim._road_weather_state → weather.get_state().to_lower()` — the same fold doc 05's `WaterHourlySystem` has always used for `weather_kind`. Three properties of it are this doc's to guarantee, and all three hold:
>
> 1. **It is the GLOBAL state (C-59).** Roads never calls `state_at(tile)`, and it deliberately does not consume `precip01` — interpolating a slowdown would break the quantised snapshot doc 10 §4 guarantee 2 rests on. Only the storm cell is spatial and it does not touch the global effect channels.
> 2. **The fold is total for every state this doc authors.** `data/weather.json` authors CLEAR, CLOUDY, RAIN, HEAVY_RAIN, THUNDERSTORM and HEAT_WAVE; lower-cased, all six are rows of doc 10 §8's table. `snow` / `blizzard` / `fog` / `high_wind` / `extreme_cold` are authored in doc 10 and wait on **this doc's** state list, not on the seam.
> 3. **The catch-up path sees the same sky the online path would.** `_advance` is one function for both paths and `_sync_clock` derives `now_min` from `ctx.tick_index` alone, so at the top of any game-hour the fine path and the coarse path agree on `get_state()` to the character. Roads samples at exactly those instants for its daily wear accumulator, which is why `wx_wear_day` is **bit-identical** between modes rather than merely close; doc 10 §2.10's within-hour congestion carries the one bounded difference (a segment boundary inside a coarse hour is priced at the hour's opening state), which is sanctioned per-system exactly as doc 06 §2.6's Poisson counts and doc 04 §2.12's one-step thermal integration are (doc 93 §E2).
>
> §3.2 already persists the timeline and the segment cursor, so a save taken in the rain restores into the same rain and doc 10's roads come back wet. `tests/test_city_sim.gd` and `tests/test_roads_integration.gd` carry the assertions.
