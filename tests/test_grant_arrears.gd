...
func test_a_v8_body_is_seeded_from_its_own_binarys_table() -> void:
	# `_v9_to_v10` is the rung that stamps. A body with a treasury block and no
	# ledger gets the version it came from; one that already has a ledger is
	# left exactly as it is.
	var stamped := CitySim._v9_to_v10({"treasury": {"treasury": 100}}, 8)
	...
	var already := CitySim._v9_to_v10(
	...
	var fragment := CitySim._v9_to_v10({}, 8)
