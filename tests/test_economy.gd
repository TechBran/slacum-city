...
## Doc 93 §M1 and §M2, and the reason both rulings exist: **one knob per line,
...
## And on the revenue side, `M_rev` reaches `tax` and no other line (§M2).
func test_one_difficulty_knob_per_ledger_line() -> void:
	...
	for preset in Difficulty.PRESETS:
		...
		assert_almost_eq(float(expenses["roads_repair"]),
				...
				"roads_repair on %s takes M_repair alone (doc 93 §M1)" % preset)
