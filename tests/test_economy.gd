...
func test_founding_ledger() -> void:
	...
	# RR-77 / RR-78 move this row. `fines` (a held $3.00/gh that stood in for a
	...
	# RR-78 moves this row and only this row's revenue side: +$172.00 of founding
...
func test_settlement_is_deterministic_and_offline_uses_one_code_path() -> void:
	...
	# 100 × $488.372396 = $48,837 (was $31,937 before RR-78's founding assistance
...
func _founding_inputs() -> Dictionary:
	...
	return {
		...
		# RR-77: no incident resolved in this synthetic hour, so the live
		...
		# RR-78: the founding hour is game-day 0, so the assistance taper is at
