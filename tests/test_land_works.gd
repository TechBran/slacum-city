...
func test_a_yard_draw_is_announced_and_the_treasury_only_pays_the_net() -> void:
	...
	for entry: Variant in spends:
		...
		assert_eq(int(spend["gross_cost"]) - int(spend["amount"]),
				_charge_for(run["charges"], phase),
				"the charge the treasury saw is the gross less the yard draw")
	sim.dispose()


## The `development_phase_charged` amount for one phase of the run's single
## block: -1 if the phase was never charged, -2 if it was charged twice (a
## second block in the rig would make the answer ambiguous).
func _charge_for(charges: Array, phase: String) -> int:
	var found := -1
