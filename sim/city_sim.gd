...
## **v11 — 2026-09-04, the celebration-grant LEDGER (Wave 24, doc 03 §2.5a,
...
## answerable against a record of what was received. A body below v11 has no such
...
##
## `_v10_to_v11` therefore takes v2 → v3's line — **mark, do not answer** — for
...
##
## It is NOT a rules rung. Every key a v10 body carries means what it meant, no
## draw order moves, and a v10 city advanced under v11 sees the same city v10
...
func migrate_save_section(body: Dictionary, from_version: int) -> Dictionary:
	...
	while version < SAVE_SECTION_VERSION:
		match version:
			...
			# says which. See `_v10_to_v11`. (Both Wave 24 and Wave 25 claimed v10
...
## v10 → v11: **mark, do not answer** — v2 → v3's line, for v2 → v3's two
...
static func _v10_to_v11(body: Dictionary, from_version: int) -> Dictionary:
...
##    `CostCurves.superseded_level_up_grant` keyed on the version `_v10_to_v11`
...
func _settle_grant_arrears() -> void:
	...
	# A NATIVE body (no migration mark) whose ledger is EMPTY while the city
	# claims earned levels is a body somebody stripped the ledger out of: a v11
	# binary records the level-1 payment at the live site the hour it is earned,
	# so no honest v11 body has levels and no ledger. Fail SAFE — seed the ledger
	# as paid at today's table and pay nothing (the merge verifier's farm vector
	# F, the one of eight that paid: $27,839,000 for deleting one key). A fresh
	# city is `earned_level 0` and never enters here; a migrated body was marked.
	if seed_from < 0 and treasury.grant_paid_by_level.is_empty() \
			and goals.earned_level >= 1:
		for level in range(1, goals.earned_level + 1):
			treasury.note_grant_paid(level, econ_curves.level_up_grant(level))
		return
...
##
## Read off `PopulationSystem.settled_occ` — what the LAST SETTLE counted — and
## not the live product, because the live product runs ahead of the chip by up
## to 59 game-minutes (RR-203's 56-second window, closed at the Wave 24 merge):
## a shell that completes at minute 120 is a full house to
## `state_occupancy × ramp × attractiveness` and nobody to `city_population`
## until the hour-3 settle at minute 176. A building the settle has never seen
## (placed since the last hour boundary) is `0 of N`; after a boot or a restore
## `settle_aggregates` has seen every building, so the panel and the chip agree
## from the first frame. `occupancy`/`occ_of` cannot serve: persisted sparsely,
## `1.0` for an id it has never seen, and not rewritten by `settle_aggregates`.
func settled_residents(sim_id: String) -> int:
	...
	if authored <= 0 or catalog.category(String(b.archetype)) != "residential":
		return -1
	return roundi(float(authored) * float(population.settled_occ.get(sim_id, 0.0)))
...
func cmd_repair_grid_component(component_id: String, preview: bool = false) -> Dictionary:
	...
	var cost := econ_curves.repair_cost_grid(kind, level, damage,
			float(treasury.difficulty().get("M_repair", 1.0)))
	# …and a third: a hair of wear. The lane gated this on doc 03's rounding
	# (`cost <= 0`) and the merge verifier found every transformer on the founding
	# city offering `CALL A CREW  $1` at game-hour 6 with the panel reading 100 %:
	# half-up rounding makes $0.89 a dollar, and a dollar job that parks a
	# heavy-equipment crew to move a condition by two thousandths is not a
	# purchase. So a STANDING component needs doc 03's own floor of wear
	# (`GRID_REPAIR_MIN_DAMAGE_FRACTION`, 0.05 — the first wear the panel shows as
	# less than 100 %) before a crew is offered; a FAILED one is always damaged.
	if damage <= 0.0 or cost <= 0 or (not failed and (float(c["condition"]) >= target
			or damage < econ_curves.grid_repair_min_damage())):
	...
	quote["job_id"] = job_id
	# Consumers, named: `data/ui.json.event_log` logs it under `power`, and
	# `ui/transformer_panel.gd` re-reads the job for its ETA. No toast, by
	# `data/ui.json`'s own ruling — the panel the player is standing in shows
	# the crew and its clock; the ARRIVAL is what raises a banner.
...
func _complete_grid_repair(job: Dictionary) -> void:
	...
	# refresh, and the arrival banner. `was_failed` is what decides whether
...
func _charge_development_phases() -> void:
	for charge in development.take_phase_charges():
		...
		bus.emit(&"development_phase_charged", {"block": block_id,
				...
				"deferred": deferred})
		# The yard's share of this invoice is its OWN row on the bus —
		# `land_works_stockpile_spent`, emitted above with block, phase and amount —
		# and the event log draws it (data/ui.json). A `stockpile_offset` key on
		# this payload had exactly one reader, a test; removed at the Wave 25 merge.
	_publish_treasury_events()
