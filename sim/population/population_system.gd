...
## What the LAST SETTLE counted, per residential building — written by BOTH
## `advance` and `settle_aggregates`, never persisted, never hashed. This is the
## map `CitySim.settled_residents` reads, so a building panel can never claim a
## resident the chip has not counted: between settles the live product
## (`state_occupancy × ramp × attractiveness`) runs AHEAD of `city_population`
## by up to 59 game-minutes — a shell that completes at minute 120 is a full
## house to the live product and nobody to the chip until minute 176 (Wave 24
## merge verifier, RR-203's 56-second window). `occupancy` cannot serve here:
## it is persisted sparsely and `settle_aggregates` must not rewrite it.
var settled_occ: Dictionary = {}  # building id -> occ_b at the last settle
